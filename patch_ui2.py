...
# Replace Closest to Action block
old_block_closest = """        <template x-if="rankedOpps.rows.length > 0">
...
new_block_closest = """        <template x-if="rankedOpps.rows.length > 0 && rankedOpps.rows.find(r => r.action !== 'AVOID')">
...
content = content.replace(old_block_closest, new_block_closest)

# Replace Headwinds block
old_block_headwinds = """        <template x-if="rankedOpps.rows.length > 0 && !rankedOpps.rows.find(r => r.action === 'TRADE')">
...
new_block_headwinds = """        <template x-if="rankedOpps.rows.length > 0 && !rankedOpps.rows.find(r => r.action === 'TRADE')">
...
content = content.replace(old_block_headwinds, new_block_headwinds)
