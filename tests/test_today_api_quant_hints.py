...
def test_today_router_contract_quant_cluster_hints():
    ...
    assert '"cc_os": cc_os' in src
    assert "build_cc_operating_system_context" in src
    assert "positions=pf_holdings" in src
    assert "enrich_opportunity_quality_rows" in src
    assert '"tracker_wave": tracker_wave' in src
    assert '"safe_automation": safe_automation' in src
...
def test_playbook_e2e_selectors_present():
    ...
    assert 'data-cc="playbook-opportunity-quality-pill"' in html
    assert 'data-cc="playbook-strategy-decay-line"' in html
    assert 'data-cc="cc-os-console"' in html
    assert 'data-cc="cc-os-curve-strip"' in html
    assert 'data-cc="monitor-upgrade-alerts"' in html
    ...
    assert "playbookOpportunityQualityPill" in js
    assert "opportunityQualityPillClass" in js
    assert "ccOsCurveStrip" in js
