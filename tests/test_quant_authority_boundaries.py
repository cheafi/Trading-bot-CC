...
def test_tracker_wave_and_automation_envelopes():
    wave = build_tracker_wave_context(tradeability="TRADE")
    auto = build_safe_automation_context(tradeability="TRADE", deployable_count=1)
    os_ctx = build_cc_operating_system_context(tradeability="TRADE", deployable_count=1)
    assert wave["may_authorize_deploy"] is False
    assert auto["may_authorize_deploy"] is False
    assert os_ctx["may_authorize_deploy"] is False
    assert_no_deploy_from_signals(
        [
            {"signal_type": wave["signal_type"]},
            {"signal_type": auto["signal_type"]},
            {"signal_type": os_ctx["signal_type"]},
        ]
    )
