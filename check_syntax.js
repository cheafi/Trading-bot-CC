function cc() {
	return {
		...
		init() {
			...
			if (!localStorage.getItem("cc_guide_seen")) {
		...
		async regenerateBrief() {
			...
			try {
				...
				if (j.ok) {
					alert(
						"✓ Brief regenerated: " + (j.after?.date || "") + " — " + (j.after?.size_bytes || 0) + " bytes",
					)
				} else {
		...
		dosVerdict() {
			...
			if (!d) return { label: "—", pill: "pw", color: "border", conf: 0, reason: "" }
			...
			if (conf >= 0.7 && conflict === "LOW" && leader)
				return {
					label: "TRADE",
					pill: "pg",
					color: "green",
		...
		async fetchPortfolio() {
			...
			try {
						...
						if (r2.ok) {
							...
							const brokerPositions = (d2.positions || []).map((p) => ({
								...
								...((this.pf.positions || []).find(
									(m) => (m.ticker || m.symbol) === (p.symbol || p.ticker),
								) || {}),
								source: "broker",
							}))
				...
				if (this.pf.positions.length > 0) {
					const enrichTasks = this.pf.positions
						.filter((p) => p.entry_price && p.ticker)
						.map((p) =>
							fetch("/api/dossier/" + p.ticker + "/trade-advice?buy_price=" + p.entry_price)
						...
						if (r.status === "fulfilled" && r.value) {
							...
							posWithEntry[i]._alpha_spy =
								r.value.pnl_pct != null
									? Math.round((r.value.pnl_pct - (this.today7.regime?.spy_change_pct || 0)) * 100) /
										100
									: null
						}
					})
		...
		async addPosition() {
			...
			try {
				const r = await fetch("/api/portfolio/position", {
					...
					headers: { "Content-Type": "application/json" },
					body: JSON.stringify({
						ticker: this.pf.addTicker,
						shares: this.pf.addShares || 0,
						entry_price: this.pf.addEntry || 0,
						stop_price: this.pf.addStop || 0,
						target_1r: this.pf.addT1r || 0,
						target_2r: this.pf.addT2r || 0,
		...
		async fetchDossier() {
			...
			try {
				...
				this.dos.data._p9 = {
					fundamentals: f9fund.value || null,
					earnings: f9earn.value || null,
					structure: f9struct.value || null,
				}
				// Peer comparison (async, non-blocking)
				this.dos.peers = null
				this.dos.peersLoading = true
				fetch("/api/dossier/" + t + "/peers")
					.then((r) => (r.ok ? r.json() : null))
					.then((d) => {
						this.dos.peers = d
					})
		...
		async ibkrConnect() {
			...
			try {
				...
				alert("IBKR Connect failed: " + e.message)
			} finally {
				this.ibkr.loading = false
			}
		},
		...
		async ibkrPlaceOrder() {
			...
			try {
				...
				const r = await fetch("/api/ibkr/order", {
					method: "POST",
					headers: { "Content-Type": "application/json", "X-API-Key": window._apiKey || "dev-secret-local" },
					body: JSON.stringify(body),
				})
		...
		async fetchToday7() {
			try {
				const r = await fetch("/api/v7/today")
				if (!r.ok) throw 0
				const d = await r.json()
				this.today7.regime = d.market_regime || null
				this.today7.top_ranked = d.top_5 || []
				this.today7.filter_funnel = d.filter_funnel || null
				this.today7.avoid_list = (d.avoid || []).map((a) =>
					typeof a === "string" ? { ticker: "⚠", reason: a } : a,
				)
				this.today7.tradeability = (d.market_regime || {}).tradeability || ""
		...
		async fetchRanked() {
			...
			try {
				let u = "/api/v7/playbook/ranked?limit=30"
				...
				if (!this.rankedOpps.rows.length && !this.rankedOpps.actionFilter && !this.rankedOpps.sectorFilter) {
					try {
						const sr = await fetch(
							"/api/v7/opportunity-scanner?regime=" + (this.regime.label || "BULL") + "&top_n=30",
						)
						if (sr.ok) {
							const sd = await sr.json()
							const cands = (sd.candidates || []).slice(0, 30)
							if (cands.length) {
								this.rankedOpps.rows = cands.map((c, i) => ({
				...
				if (!this.rankedOpps.rows.length) {
					try {
						const br = await fetch("/api/brief")
						if (br.ok) {
							const bd = await br.json()
							const all = [...(bd.actionable || []), ...(bd.watch || [])].slice(0, 30)
							if (all.length) {
								this.rankedOpps.rows = all.map((s, i) => ({
									ticker: s.ticker,
									score: s.rs_score || 0,
									action: s.conviction || "WATCH",
									sector_type: "",
									entry_price: s.entry || s.price,
									stop_price: s.stop,
									target_price: s.target_3r || s.target_2r,
									risk_reward:
										s.target_3r && s.stop && s.entry
											? ((s.target_3r - s.entry) / (s.entry - s.stop)).toFixed(1)
											: 3.0,
									setup: "brief",
									grade: s.conviction === "TRADE" ? "A" : s.conviction === "LEADER" ? "B" : "C",
									thesis_conf: s.near_52w_high ? 0.7 : 0.5,
									timing_conf: s.vol_ratio >= 1.2 ? 0.7 : 0.5,
									exec_conf: 0.6,
									data_conf: 0.7,
									why_now:
										"RS:" + s.rs_score + " · ATR:" + s.atr_pct + "% · Vol:" + s.vol_ratio + "x",
								}))
								this.rankedOpps.source = "brief"
							}
						}
					} catch (e) {}
				}
		...
		async fetchScanners(cat) {
			...
			try {
				...
				const r = await fetch(u)
				if (!r.ok) throw new Error("HTTP " + r.status)
				this.scannerHub.data = await r.json()
				const d = this.scannerHub.data || {}
				let uni = d.universe_size || d.universe || 0
				if (!uni) {
					let s = 0
					if (d.scanners) {
						for (const k of Object.keys(d.scanners || {})) {
							const grp = d.scanners[k] || {}
							for (const sk of Object.keys(grp)) {
								s += (grp[sk] || []).length || 0
							}
						}
					} else if (d.hits) {
		...
		async autoScheduleExperiments() {
			try {
				...
				console.warn("auto-schedule failed", e)
