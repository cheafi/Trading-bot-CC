...
## Expectancy-First Architecture (2026-09)

**North star:** optimize expectancy per trade × frequency ÷ drawdown — **not** win rate.  
**Constraints preserved:** `TRADE_RR_THRESHOLD=2.5` · human `deploy_open` gate · Research ≠ Deploy · LLM in review layer only · Tier-1 bandit safe / Tier-2 human-gated.

Seven-week build order maps professional trading-system blueprint → CC X backlog. Each phase must pass the **Four Questions** PR gate before sprint eligibility.

|  Week | Phase                                  | Scope                                                                                                                            | CCX IDs                   | IDOS gate                                                                                                                                                  |
| ----: | -------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- | ------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **1** | Strategy Fitness Matrix + DSR          | `strategy_fitness.py`; OOS eligibility gates; `GET /api/v7/strategy-fitness/matrix`; persist `data/strategy_fitness_matrix.json` | **CCX-180**               | **Q1** Know: per-ticker OOS edge evidence · **Q3** Doubt: DSR deflates selection bias · **Q4** Act: eligibility hints only — `may_authorize_deploy: false` |
| **2** | Walk-forward wire + per-ticker fitting | Optimizer → fitness matrix nightly; attach OOS grade to playbook rows                                                            | CCX-094, CCX-029, CCX-181 | **Q1** Know: IS/OOS split on every strategy row · **Q3** Doubt: walk-forward degradation visible · **Q4** Remove ad-hoc backtest labels                    |
| **3** | Strategy library expansion             | VWAP, ORB, PEAD, momentum, pairs, overnight gap, micro-futures modules                                                           | CCX-182                   | **Q1** Know: which setup class fits which regime · **Q3** Doubt: per-strategy sample floors · **Q4** Consolidate `STRATEGY_REGISTRY` sprawl                |
| **4** | Candidate store + universe filters     | Persistent candidate JSONL; universe filter SSOT; Alpha Factory chain                                                            | CCX-020–023, CCX-183      | **Q1** Know: full opportunity funnel audit · **Q3** Doubt: filter rejection reasons logged · **Q4** Dedupe scanner vs playbook candidate paths             |
| **5** | CPCV + overfitting controls            | Combinatorial Purged CV; extend DSR with CPCV variance                                                                           | CCX-184                   | **Q1** Know: CPCV distribution not point estimate · **Q3** Doubt: multiple-testing correction beyond DSR stub · **Q4** Retire single-fold OOS claims       |
| **6** | Contextual bandit Tier-1               | Thompson sizing wire; `autonomous_learning_loop` allocation context from fitness matrix                                          | CCX-043, CCX-132, CCX-185 | **Q1** Know: arm context includes fitness eligibility · **Q3** Doubt: n&lt;30 hidden · **Q4** Act: bandit allocation only — no parameter auto-apply        |
| **7** | Execution + counterfactual logging     | IBKR/Futu closed-trade ≥95%; kill rules hardening; "what if I'd taken all" candidate log                                         | CCX-040, CCX-092, CCX-186 | **Q1** Know: realized vs counterfactual expectancy · **Q3** Doubt: slippage-adjusted PF in fitness · **Q4** Act: human deploy gate unchanged               |

### Alignment snapshot (exists vs gap)

| Blueprint area                                     | Exists (code)                                                                                                    | Gap                                                                                           |
| -------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- |
| Opportunity discovery / universe / candidate store | `opportunity_scanner.py`, `opportunity_scan_filters.py`, `alpha_factory.py`, `research_queue.py`                 | No persistent candidate JSONL store; Alpha Factory not wired on all scan paths (CCX-020–023)  |
| Strategy library (VWAP, ORB, PEAD, …)              | `strategy_optimizer.STRATEGY_REGISTRY` (SWING/BREAKOUT/MEAN_REVERSION); `strategies/momentum.py`, `breakout.py`  | Named institutional setups (VWAP, ORB, PEAD, pairs, overnight gap, micro-futures) **missing** |
| Walk-forward / optimizer / enhanced backtester     | `strategy_optimizer.py` (4-fold WF), `enhanced_backtester.py`, `backtest_lab.py`, `main.py` `_walk_forward_test` | Not wired to playbook rows or fitness matrix persistence (Week 2)                             |
| Strategy fitness / per-ticker fitting              | **CCX-180 shipped** — `strategy_fitness.py`                                                                      | CPCV; nightly batch; playbook UI chips                                                        |
| DSR / CPCV                                         | **DSR stub** in `strategy_fitness.py`                                                                            | CPCV not implemented (CCX-184)                                                                |
| Thompson / contextual bandit                       | `thompson_sizing.py`, `autonomous_learning_loop.py`, `self_learning.py`                                          | Fitness matrix not yet bandit context; n&lt;30 hide (CCX-043)                                 |
| Broker adapters (IBKR, Futu)                       | `brokers/ib_broker.py`, `brokers/futu_broker.py`, `ibkr_service.py`                                              | Closed-trade capture &lt;95% target (CCX-040); reconciliation gaps                            |
| Kill rules / position_manager / risk_limits        | `risk_limits.py`, `position_manager.py`, `decision_truth_model.py`                                               | Fitness eligibility not yet in sizing pipeline                                                |
| Candidate logging ("what if all")                  | `rejection_audit.py`, `shadow_account.py`, A/B shadow in `self_learning.py`                                      | No unified counterfactual candidate JSONL (CCX-186)                                           |

**Week 1 evidence:** `src/engines/strategy_fitness.py`, `tests/test_strategy_fitness.py`, `GET /api/v7/strategy-fitness/matrix`

---

## Autonomous Learning Loop — phased roadmap (2026-09-15)

Bounded automation under human deploy gate. **Research ≠ Deploy** at every phase.

| Phase            | Scope                                                                                                                                                                                                                                                              | Automation tier                             | IDOS gate                                                                                                                                                                |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **P0** (shipped) | `autonomous_learning_loop.py`; `GET /api/v7/meta-intelligence/summary`; `POST /api/v7/learning-loop/run` (gated `AUTONOMOUS_LEARNING=0` default); ai_usage in usage summary; scheduler 4:50 PM observe/label/calibrate/propose (env-gated); Sunday weekly IC cache | Tier 1 automated when enabled               | **Q1** Know: forward marks + usage counts · **Q2** Believe: belief queue due count · **Q3** Doubt: Brier drift flag · **Q4** Act: explicit `may_authorize_deploy: false` |
| **P1**           | Wire meta-intelligence into Ops Evolution Dashboard (CCX-134); restore `/api/v7/self-learn/*` router if missing; IBKR → closed_trades ≥95% (CCX-040)                                                                                                               | Tier 1 ingest + Tier 2 human gate for apply | Q1+Q4: complete outcome chain; no deploy bypass                                                                                                                          |
| **P2**           | Belief Review full ritual (CCX-135); weekly IC email/Discord; replay cache warming on EOD; Intelligence Engine daily report                                                                                                                                        | Tier 2 IC review for belief edits           | Q2+Q3: conviction vs calibration                                                                                                                                         |
| **P3**           | Monthly Evolution Report (CCX-137); Top-20 self-improvements ranker; walk-forward CI                                                                                                                                                                               | Tier 2 monthly CIO packet                   | Q4: deletion ceremony for zero-use surfaces                                                                                                                              |

**Files (P0):** `src/services/autonomous_learning_loop.py`, `src/services/usage_log.py`, `src/api/routers/decision.py`, `src/scheduler/main.py`, `tests/test_roadmap_p0.py`
