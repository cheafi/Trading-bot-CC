...
**Master roadmap（三層 AOS + 四 Sprint + 30 項清單）：** [`ALPHA_OPERATING_SYSTEM_DEVELOPER_PROMPT_ZH.md`](./ALPHA_OPERATING_SYSTEM_DEVELOPER_PROMPT_ZH.md)
...
- [x] `avoid_now` API（`build_avoid_now_engine` — categories: regime, earnings, failed_breakout…）
- [ ] `avoid_now` **獨立 Today panel**（category chips + drill）
...
| P1  | Portfolio risk cockpit          | `portfolio_risk_cockpit.py` ✅           |
| P1  | Catalyst calendar               | `catalyst_calendar.py` ✅                |
| P1  | PM memo generator               | `pm_memo.py` ✅                          |
