...
class VCPStrategy(IStrategy):
    ...
    def __init__(self, config: Optional[StrategyConfig] = None):
        ...
        self.volume_contraction_threshold = params.get(
    ...
    def populate_indicators(
        ...
        dataframe["dist_from_52w_high"] = (
            dataframe["high_52w"] - dataframe["close"]
        ) / dataframe["high_52w"]
        dataframe["dist_from_52w_low"] = (
            dataframe["close"] - dataframe["low_52w"]
        ) / dataframe["low_52w"]
        ...
        dataframe["vcr"] = IndicatorLibrary.volatility_contraction_ratio(
            dataframe, 10, 50
        )
        ...
        dataframe["rs_rating"] = (
            dataframe["close"]
            .rolling(window=min(252, len(dataframe)))
            .apply(
        ...
        tight_results = dataframe.apply(
            lambda row: (
                self._detect_tight_range(dataframe.loc[: row.name].tail(10))
                if len(dataframe.loc[: row.name]) >= 10
                else (False, 0)
            ),
            axis=1,
        )
    ...
    def populate_entry_trend(
        ...
        conditions = (
            ...
            (
                (dataframe["tt_score"] >= 7)
                if self.trend_template_enabled
            ...
            (dataframe["tight_consolidation"])
            &
            # Volume was dry RECENTLY (prior 5 days avg below average)
            (
                dataframe["volume"].rolling(5).mean().shift(1)
                < dataframe["vol_sma_50"] * 0.8
            )
            &
    ...
    def custom_stoploss(
        ...
        **kwargs,
        ...
        current_atr_pct = (
            dataframe["atr_pct"].iloc[-1] if "atr_pct" in dataframe.columns else 0.02
        )
    ...
    def scan_universe(
        self, universe_data: Dict[str, pd.DataFrame], min_score: int = 6
    ) -> List[Dict[str, Any]]:
        """
                    ...
                    candidates.append(
                        {
                            ...
                            "volume_contraction": vcp_details.get(
        ...
        candidates.sort(
            key=lambda x: (x["is_vcp"], x["vcp_score"], x["rs_rating"]), reverse=True
        )
...
class SEPAScreener:
    ...
    def screen(df: pd.DataFrame) -> Dict[str, Any]:
        ...
        criteria = {
            "1_above_150_200_ma": current_price > sma_150.iloc[-1]
            and current_price > sma_200.iloc[-1],
            "2_150_above_200": sma_150.iloc[-1] > sma_200.iloc[-1],
            "3_200_rising": sma_200.iloc[-1] > sma_200.iloc[-21],  # Rising over 1 month
            "4_50_above_150_200": sma_50.iloc[-1] > sma_150.iloc[-1]
