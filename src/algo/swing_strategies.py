...
from .base_strategy import IStrategy, TimeFrame
...
class ShortTermTrendFollowingStrategy(IStrategy):
    ...
    def populate_entry_trend(
        ...
        # Entry triggers
        reversal_trigger = dataframe["bullish_reversal"] == 1

        breakout_trigger = dataframe["breaks_above_ema20"] == 1
    ...
    def populate_exit_trend(
        ...
        trend_break = (dataframe["close"] < dataframe["sma_50"]) & (
        ...
        # MACD bearish cross
        macd_cross_down = (dataframe["macd"] < dataframe["macd_signal"]) & (
            dataframe["macd"].shift(1) >= dataframe["macd_signal"].shift(1)
        )

        # Combined exit signal
        dataframe.loc[
            trend_break | (momentum_exhaustion & macd_cross_down), "exit_long"
        ] = 1
...
class ClassicSwingStrategy(IStrategy):
    ...
    def populate_indicators(
        ...
        dataframe["rsi_rising"] = (dataframe["rsi"] > dataframe["rsi"].shift(1)).astype(
            int
        )
    ...
    def populate_entry_trend(
        ...
        short_conditions = (
            ...
            & ((dataframe["rsi"] > 60) | (dataframe["stoch_k"] > 70))
            & ((dataframe["stoch_cross_down"] == 1) | (dataframe["rsi_falling"] == 1))
        )
    ...
    def populate_exit_trend(
        ...
        exit_long_conditions = (
            (dataframe["near_resistance"] == 1)
            | ((dataframe["rsi"] > 70) & (dataframe["rsi_falling"] == 1))
        ...
        exit_short_conditions = (
            (dataframe["near_support"] == 1)
            | ((dataframe["rsi"] < 30) & (dataframe["rsi_rising"] == 1))
            | (dataframe["stoch_cross_up"] == 1)
        )
...
class MomentumRotationStrategy(IStrategy):
    ...
    def populate_indicators(
        ...
        # Momentum metrics
        dataframe["mom_4w"] = (
            dataframe["close"].pct_change(self.momentum_lookback_short) * 100
        )
        dataframe["mom_8w"] = (
            dataframe["close"].pct_change(self.momentum_lookback_long) * 100
        )
        ...
        dataframe["pct_from_sma20"] = (
            (dataframe["close"] - dataframe["sma_20"]) / dataframe["sma_20"] * 100
        )
        ...
        dataframe["is_rebalance_day"] = (
            dataframe["day_count"] % self.rebalance_period_days == 0
        ).astype(int)
    ...
    def populate_entry_trend(
        ...
        # Entry conditions
        high_momentum = dataframe["momentum_score"] > dataframe[
            "momentum_score"
    ...
    def populate_exit_trend(
        ...
        close_below_20ma = (dataframe["close"] < dataframe["sma_20"]) & (
        ...
        momentum_weak = (
            (
                ...
                < dataframe["momentum_score"].rolling(50).quantile(0.40)
            )
            & (
                dataframe["momentum_score"] < dataframe["momentum_score"].shift(5)
            )  # Getting weaker
        )
        ...
        exit_on_rebalance = (dataframe["is_rebalance_day"] == 1) & momentum_weak

        dataframe.loc[
            close_below_20ma | trend_break | exit_on_rebalance, "exit_long"
        ] = 1
...
class ShortTermMeanReversionStrategy(IStrategy):
    ...
    def populate_indicators(
        ...
        # Distance from mean
        dataframe["pct_from_sma10"] = (
            (dataframe["close"] - dataframe["sma_10"]) / dataframe["sma_10"] * 100
        )
        dataframe["pct_from_sma20"] = (
            (dataframe["close"] - dataframe["sma_20"]) / dataframe["sma_20"] * 100
        )
    ...
    def populate_entry_trend(
        ...
        in_uptrend = (dataframe["close"] > dataframe["sma_200"]) & (
            dataframe["sma_50"] > dataframe["sma_200"]
        )

        # 2. Sharp selloff (3-5 consecutive down days)
        valid_selloff = (dataframe["pullback_days"] >= self.min_down_days) & (
            dataframe["pullback_days"] <= self.max_down_days
        )

        # 3. Drawdown in target range (6-12%)
        valid_drawdown = (dataframe["pullback_depth"] >= self.min_drawdown_pct) & (
        ...
        # Combined entry
        entry_conditions = (
            in_uptrend & valid_selloff & valid_drawdown & oversold & reversal_signal
        )
    ...
    def populate_exit_trend(
        ...
        back_to_sma20 = (dataframe["close"] >= dataframe["sma_20"]) & (
            dataframe["close"].shift(1) < dataframe["sma_20"].shift(1)
        )

        # Exit 3: RSI normalized
        rsi_normalized = (dataframe["rsi"] > 55) & (dataframe["rsi"].shift(1) <= 55)
        ...
        back_to_bb_upper_half = (
            ...
            < ((dataframe["bb_middle"].shift(1) + dataframe["bb_upper"].shift(1)) / 2)
        )
...
def get_swing_strategy(strategy_id: str) -> IStrategy:
    ...
    if strategy_id not in SWING_STRATEGIES:
        raise ValueError(
            f"Unknown strategy: {strategy_id}. Available: {list(SWING_STRATEGIES.keys())}"
        )
