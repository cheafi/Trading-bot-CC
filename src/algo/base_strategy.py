...
class IStrategy(ABC):
    ...
    def leverage(
        self, ticker: str, current_time: datetime, current_rate: float, **kwargs
    ) -> float:
        """
    ...
    def get_all_signals(
        ...
        signal_mask = (df.get("enter_long", 0) == 1) | (df.get("exit_long", 0) == 1)
        if self.can_short:
            signal_mask |= (df.get("enter_short", 0) == 1) | (
