...
class TrendFollowingStrategy(IStrategy):
    ...
    def populate_indicators(
        ...
        dataframe["donchian_high"] = (
            dataframe["high"].rolling(self.breakout_period).max()
        )
        ...
        dataframe["atr_stop"] = dataframe["close"] - (
            self.atr_multiplier * dataframe["atr"]
        )
        ...
        # MA alignment
        dataframe["ma_aligned"] = (dataframe["sma_20"] > dataframe["sma_50"]) & (
            dataframe["sma_50"] > dataframe["sma_100"]
        )

        # Breakout detection
        dataframe["breakout_up"] = dataframe["close"] > dataframe[
            "donchian_high"
        ].shift(1)
        dataframe["breakout_down"] = dataframe["close"] < dataframe[
            "donchian_low"
        ].shift(1)
    ...
    def populate_entry_trend(
        ...
        conditions = (
            ...
            ((dataframe["ma_aligned"]) | (dataframe["close"] > dataframe["sma_50"]))
            &
