...
class PreEarningsMomentumStrategy(IStrategy):
    ...
    def populate_exit_trend(
        ...
        # Exit on trend break
        trend_break = (dataframe["close"] < dataframe["sma_20"]) & (
...
class PostEarningsDriftStrategy(IStrategy):
    ...
    def populate_indicators(
        ...
        for i in range(len(dataframe)):
            ...
            dataframe.iloc[i, dataframe.columns.get_loc("days_since_gap_up")] = (
                days_since_up
            )
            dataframe.iloc[i, dataframe.columns.get_loc("days_since_gap_down")] = (
                days_since_down
            )
            dataframe.iloc[i, dataframe.columns.get_loc("gap_up_price")] = gap_up_price
            dataframe.iloc[i, dataframe.columns.get_loc("gap_down_price")] = (
                gap_down_price
            )
        ...
        for i in range(len(dataframe)):
            if 0 < dataframe["days_since_gap_up"].iloc[i] <= self.pullback_entry_days:
                ...
                dataframe.iloc[
                    i, dataframe.columns.get_loc("pullback_from_gap_high")
    ...
    def populate_exit_trend(
        ...
        gap_filled_long = (dataframe["close"] < dataframe["gap_up_price"]) & (
            dataframe["close"].shift(1) >= dataframe["gap_up_price"].shift(1)
        )

        # Momentum fading
        momentum_fade_long = (dataframe["rsi"] > 70) & (
        ...
        # === EXIT SHORT ===
        gap_filled_short = (dataframe["close"] > dataframe["gap_down_price"]) & (
            dataframe["close"].shift(1) <= dataframe["gap_down_price"].shift(1)
        )

        momentum_fade_short = (dataframe["rsi"] < 30) & (
            dataframe["rsi"] > dataframe["rsi"].shift(1)
        )
...
class EarningsBreakoutStrategy(IStrategy):
    ...
    def populate_indicators(
        ...
        for i in range(len(dataframe)):
            ...
            dataframe.iloc[i, dataframe.columns.get_loc("post_earnings_high")] = (
                post_earnings_high
            )
            dataframe.iloc[i, dataframe.columns.get_loc("consolidation_low")] = (
                consolidation_low
            )
            dataframe.iloc[i, dataframe.columns.get_loc("days_since_earnings")] = (
                days_since
            )
        ...
        # Trend
        dataframe["sma_20"] = IL.sma(dataframe["close"], 20)
        dataframe["above_sma20"] = (dataframe["close"] > dataframe["sma_20"]).astype(
            int
        )
    ...
    def populate_exit_trend(
        ...
        failed_breakout = (dataframe["close"] < dataframe["post_earnings_high"]) & (
            dataframe["close"].shift(1) >= dataframe["post_earnings_high"].shift(1)
        )

        # RSI overextended
        overextended = (dataframe["rsi"] > 80) & (
            dataframe["rsi"] < dataframe["rsi"].shift(1)
        )
...
def get_earnings_strategy(strategy_id: str) -> IStrategy:
    ...
    if strategy_id not in EARNINGS_STRATEGIES:
        raise ValueError(
            f"Unknown strategy: {strategy_id}. Available: {list(EARNINGS_STRATEGIES.keys())}"
        )
...
class EarningsCalendar:
    ...
    def get_upcoming_earnings(
        ...
        self.logger.info(
            f"Would fetch earnings for {len(tickers)} tickers, {days_ahead} days ahead"
        )
        return {}

    def get_recent_earnings(
        self, tickers: List[str], days_back: int = 30
    ) -> Dict[str, EarningsEvent]:
        """
        ...
        # Placeholder - would call API in production
        self.logger.info(
            f"Would fetch recent earnings for {len(tickers)} tickers, {days_back} days back"
        )
        return {}
