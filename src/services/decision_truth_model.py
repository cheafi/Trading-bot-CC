...
PLAYBOOK_FUNNEL_LAYER_DEFINITIONS: Dict[str, str] = {
    ...
    "setup_qualified": "Met setup bar — monitor / near-miss pool; not deploy-ready.",
    "trade_qualified": "Thesis + timing bar — still needs execution path.",
    "execution_qualified": "Execution-ready on card — board gate still required.",
    "deploy_qualified": "All gates pass — only tier that authorizes deploy.",
    "watch_qualified": "Alias for setup_qualified — monitor pool; not deploy-ready.",
...
def playbook_funnel_layer_note() -> str:
    ...
    return (
        "Scanned = universe · Setup-qualified = monitor pool · Trade-qualified = thesis+timing · "
        "Execution-qualified = card-ready · Deploy-qualified = all gates pass · "
        "Near-miss = upgrade layer (not deploy)."
    )
...
def format_board_quality_detail(
    ...
    *,
    ...
    notes = [str(n).strip() for n in (degradation_notes or []) if str(n).strip()]
    if scanner_degraded and not notes:
        notes = ["rank input: cache stale"]
...
def build_honest_funnel(
    *,
    ...
    setup_qualified = max(near_miss, watch_count)
    trade_qualified = sum(
        1
        for cr in council_results
        if refine_action(cr) in _TRADE_ACTIONS
        or (
            refine_action(cr) == "PILOT"
            and is_pilot_eligible(cr)
        )
        or (
            refine_action(cr) == "WATCH"
            and _score(cr) >= 7.0
            and float(_pr(cr).confidence.thesis) >= 0.65
            and float(_pr(cr).confidence.timing) >= 0.50
        )
    )
    ...
    return normalize_playbook_funnel(
        {
            ...
            "setup_qualified_setups": setup_qualified,
            "trade_qualified_setups": trade_qualified,
            "execution_qualified_setups": execution_ready,
            "watch_qualified_setups": setup_qualified,
...
def normalize_playbook_funnel(
    ...
    *,
    ...
    deploy_authority: Optional[bool] = None,
) -> Dict[str, Any]:
    """Canonical qualification funnel counts for Playbook + unlock_deploy."""
    f = dict(funnel or {})
    scanned = int(f.get("universe_scanned") or f.get("universe") or 0)
    execution = int(
        f.get("execution_qualified_setups")
        or f.get("execution_ready_setups")
        or 0
    )
    setup = f.get("setup_qualified_setups")
    if setup is None:
        setup = f.get("watch_qualified_setups")
    if setup is None:
        setup = f.get("near_miss_setups")
    if setup is None:
        setup = f.get("watch_setups")
    if setup is None and near_miss is not None:
        setup = len(near_miss)
    setup = int(setup or 0)
    trade = int(f.get("trade_qualified_setups") or 0)
    deploy_raw = int(f.get("deploy_qualified_setups") or 0)
    if deploy_authority is False:
        deploy = 0
    elif deploy_authority is True:
        deploy = execution if execution > 0 else deploy_raw
    else:
        deploy = deploy_raw
    return {
        **f,
        "universe_scanned": scanned,
        "setup_qualified_setups": setup,
        "trade_qualified_setups": trade,
        "execution_qualified_setups": execution,
        "watch_qualified_setups": setup,
        "deploy_qualified_setups": deploy,
        "execution_ready_setups": execution,
        "qualification_line": format_qualification_funnel_line(
            setup_qualified=setup,
            deploy_qualified=deploy,
        ),
    }


def format_qualification_funnel_line(
    *,
    ...
    """Playbook / Today funnel strip — never label setup as deploy."""
    parts: List[str] = []
    if setup_qualified:
...
def finalize_funnel_qualification(
    funnel: Optional[Dict[str, Any]],
    *,
    ...
    """Apply deploy_qualified only when SystemTruth deploy authority passes."""
    from src.services.system_truth import resolve_deploy_authority

    deploy_ok = resolve_deploy_authority(
