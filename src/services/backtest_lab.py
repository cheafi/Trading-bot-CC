...
async def build_backtest_lab(
    ...
    *,
    ...
    curve_health = _attach_curve_health(sym, wf, trade_review, attribution)
...
def _attach_curve_health(
    ticker: str,
    walk_forward: Dict[str, Any],
    trade_review: Dict[str, Any],
    attribution: Dict[str, Any],
) -> Optional[Dict[str, Any]]:
    """Walk-forward → strategy curve health envelope (research only)."""
    try:
        from src.services.backtest_curve_bridge import build_curve_health_envelope

        ranked = (attribution or {}).get("ranked") or []
        best = ranked[0].get("name") if ranked else "lab_best"
        return build_curve_health_envelope(
            walk_forward,
            trade_review,
            strategy_id=str(best or "lab_best"),
            ticker=ticker,
        )
    except Exception as exc:
        logger.debug("curve health bridge failed: %s", exc)
        return None
