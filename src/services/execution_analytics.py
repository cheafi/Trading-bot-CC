...
def normalize_ibkr_fills(fills: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """Map IBKR execDetails rows to analytics-friendly fill records."""
    ...
    for raw in fills or []:
        ...
        if qty <= 0:
            continue
        slip = raw.get("slippage_bps")
        if slip is None:
            slip = 8.0 if raw.get("price") else 0.0
        normalized.append(
            {
                "symbol": raw.get("symbol") or "",
                "filled": True,
                "quantity": qty,
                "slippage_bps": float(slip or 0),
...
def build_execution_analytics_from_ibkr(
    fills: List[Dict[str, Any]],
    *,
    ibkr_connected: bool = True,
) -> Dict[str, Any]:
    """Build execution analytics from session-local IBKR fills."""
    rows = normalize_ibkr_fills(fills)
    if not rows:
        return build_execution_analytics(
            orders_sampled=0,
            ibkr_connected=ibkr_connected,
    ...
    slips = [float(r.get("slippage_bps") or 0) for r in rows]
    partials = [1.0 if r.get("partial") else 0.0 for r in rows]
    partial_pct = sum(partials) / len(partials) * 100.0 if partials else 0.0
    degraded = len(rows) < 5
    return build_execution_analytics(
        orders_sampled=len(rows),
        median_slippage_bps=sum(slips) / len(slips) if slips else 0.0,
        fill_rate_pct=100.0,
...
def build_recent_fills_summary(fills: List[Dict[str, Any]]) -> Dict[str, Any]:
    """Aggregate user-supplied or IBKR fill rows for router payloads."""
    if not fills:
        return build_execution_analytics(degraded=True)
    if fills and fills[0].get("exec_id") is not None:
        return build_execution_analytics_from_ibkr(fills, ibkr_connected=True)
