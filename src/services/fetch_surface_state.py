...
def _export_escape_html(text: object) -> str:
    import html

    return html.escape(str(text or ""))


def _export_truncate(text: object, max_len: int = 160) -> str:
    s = str(text or "").strip()
    if len(s) <= max_len:
        return s
...
def export_issues_list_html(items: Optional[list] = None) -> str:
    """Bulleted issues list — parity with cc-helpers exportIssuesList."""
    rows = items or []
    if not rows:
        return '<div class="cc-export-muted">None flagged</div>'
    lis = "".join(f"<li>{_export_escape_html(_export_truncate(item, 200))}</li>" for item in rows)
    return f"<ul>{lis}</ul>"


def export_error_log_rows_html(entries: Optional[list] = None, limit: int = 12) -> str:
    """Error log table — parity with cc-helpers exportErrorLogRows."""
    list_rows = entries or []
    if not list_rows:
        return '<div class="cc-export-muted">No session errors logged</div>'
    body = []
    for row in list_rows[:limit]:
        r = row or {}
        body.append(
            "<tr><td>"
            f"{_export_escape_html(r.get('severity') or '—')}</td><td>"
            f"{_export_escape_html(_export_truncate(r.get('source') or r.get('category') or '—', 40))}</td><td>"
            f"{_export_escape_html(_export_truncate(r.get('message') or r.get('detail') or r.get('summary') or '—', 160))}</td><td class=\"mono\">"
            f"{_export_escape_html(_export_truncate(r.get('timestamp') or r.get('ts') or r.get('time') or '—', 24))}"
            "</td></tr>"
        )
    return (
...
def export_probe_rows_html(rows: Optional[list] = None) -> str:
    """Ops probe vs runtime table — FAIL / UNAVAILABLE rows prioritized."""
    list_rows = rows or []
    flagged = []
    for row in list_rows:
        r = row or {}
        probe = str(r.get("probe") or r.get("label") or "").upper()
        runtime = str(r.get("runtime") or r.get("runtime_evidence") or r.get("evidence") or "").upper()
        if "FAIL" in probe or "FAIL" in runtime or "UNAVAILABLE" in runtime:
            flagged.append(r)
    show = flagged or list_rows
    if not show:
        return '<div class="cc-export-muted">No probe/runtime rows available</div>'
    body = []
    for row in show[:8]:
        r = row or {}
        name = str(r.get("name") or "—").replace("_", " ")
        body.append(
            "<tr><td>"
            f"{_export_escape_html(name)}</td><td>"
            f"{_export_escape_html(r.get('probe') or r.get('label') or '—')}</td><td>"
            f"{_export_escape_html(_export_truncate(r.get('runtime') or r.get('runtime_evidence') or r.get('evidence') or '—', 120))}"
            "</td></tr>"
        )
    return (
...
def build_export_issues_page(snapshot: Optional[Dict[str, Any]] = None) -> str:
    """Page 1 — issues, disconnects, error log (parity with cc-helpers buildExportIssuesPage)."""
    s = snapshot or {}
    truth = s.get("system_truth") or s.get("systemTruth") or {}
    blockers = today_mission_system_blockers(system_truth=truth) if truth.get("reason_codes") else list(s.get("system_blockers") or [])
    truth_line = system_truth_line(truth) if truth else str(s.get("system_truth_line") or "")
    funnel = s.get("filter_funnel") or {}
    qual = s.get("qualification") or {}
    setup_count = int(qual.get("setup") or funnel.get("watch_qualified_setups") or funnel.get("setup_qualified_setups") or 0)
    deploy_count = int(
        qual.get("deploy")
        or funnel.get("deploy_qualified_setups")
        or funnel.get("execution_ready_setups")
        or truth.get("deploy_qualified_count")
        or 0
    )
    qual_line = str(qual.get("line") or playbook_qualification_funnel_line(funnel) or f"{setup_count} setup-qualified · {deploy_count} deploy-qualified")
    header = s.get("cc_header") or {}
    disconnects = s.get("disconnects") or {}
    generated = str(s.get("generated_at") or "")
    error_log = s.get("error_log") or []
    deploy_open = truth.get("deploy_authority") is True or s.get("deploy_authority") is True

    parts = [
        '<section class="cc-export-page cc-export-issues" data-cc-export="issues">',
        "<h1>CC · Issues / errors / disconnect summary</h1>",
        f'<p class="cc-export-meta mono">Generated {_export_escape_html(generated)} · review export · not trade authority</p>',
        "<h2>SystemTruth</h2>",
    ]
    if truth_line:
        parts.append(f'<p class="cc-export-lead">{_export_escape_html(truth_line)}</p>')
    parts.extend(
        [
            export_issues_list_html(blockers),
            "<h2>cc-header status</h2>",
            "<table><tbody>",
            f"<tr><th>Freshness</th><td>{_export_escape_html(header.get('freshness') or s.get('data_tier') or '—')}</td></tr>",
            f"<tr><th>Engine</th><td>{_export_escape_html(header.get('engine') or ('ON' if s.get('engine_running') else 'OFF'))}</td></tr>",
            f"<tr><th>IBKR</th><td>{_export_escape_html(header.get('ibkr') or s.get('ibkr_label') or '—')}</td></tr>",
            f"<tr><th>Breaker</th><td>{_export_escape_html(header.get('breaker') or ('TRIPPED' if s.get('breaker') else 'clear'))}</td></tr>",
            f"<tr><th>API mode</th><td>{_export_escape_html(header.get('mode') or s.get('cc_mode') or '—')}</td></tr>",
            "</tbody></table>",
            "<h2>Disconnect / degraded states</h2>",
            "<table><tbody>",
            "<tr><th>IBKR gateway</th><td>"
            + _export_escape_html(
                "reachable" if disconnects.get("ibkr_gateway") else "offline" if disconnects.get("ibkr_gateway") is not None else "—"
            )
            + "</td></tr>",
            f"<tr><th>API health mode</th><td>{_export_escape_html(disconnects.get('health_mode') or '—')}</td></tr>",
            f"<tr><th>instant_degraded</th><td>{_export_escape_html('yes' if disconnects.get('instant_degraded') else 'no')}</td></tr>",
            f"<tr><th>brief-fallback</th><td>{_export_escape_html('yes' if disconnects.get('brief_fallback') else 'no')}</td></tr>",
            "</tbody></table>",
            "<h2>Error log summary</h2>",
            f'<p class="cc-export-muted">{_export_escape_html(str(s.get("error_log_total") or 0) + " buffered · showing last " + str(len(error_log)) + " entries")}</p>',
            export_error_log_rows_html(error_log, int(s.get("error_log_limit") or 12)),
            "<h2>Ops probe vs runtime (FAIL / UNAVAILABLE)</h2>",
            export_probe_rows_html(s.get("probe_rows")),
            "<h2>Deploy authority &amp; qualification</h2>",
            "<table><tbody>",
            f"<tr><th>Deploy authority</th><td>{_export_escape_html('open' if deploy_open else 'blocked / monitor only')}</td></tr>",
            f"<tr><th>Setup-qualified</th><td>{_export_escape_html(str(setup_count))}</td></tr>",
            f"<tr><th>Deploy-qualified</th><td>{_export_escape_html(str(deploy_count))}</td></tr>",
            f"<tr><th>Summary</th><td>{_export_escape_html(qual_line)}</td></tr>",
            "</tbody></table>",
...
def build_export_all_surfaces_page(snapshot: Optional[Dict[str, Any]] = None) -> str:
    """All CC surfaces snapshot — parity with cc-helpers buildExportAllSurfacesPage."""
    s = snapshot or {}
    ...
    console = funds.get("console") or {}
    if console.get("research_lab_title") or console.get("title"):
        funds_lines.append(str(console.get("research_lab_title") or console.get("title")))
    fs = console.get("funds_first_screen") or {}
    for key, label in (
        ("guardrail", "Guardrail"),
        ("now", "NOW"),
        ("why", "WHY"),
        ("core_index_posture", "Core index posture"),
        ("live_allocation_label", "Live allocation"),
    ):
        if fs.get(key):
            funds_lines.append(f"{label}: {fs[key]}")
    best = fs.get("best_research_sleeve") or {}
    if best.get("headline"):
        funds_lines.append(f"Best research sleeve: {best['headline']}")
    if funds.get("error"):
        funds_lines.append(f"Load error: {funds['error']}")
    if not funds_lines:
        funds_lines.append("Funds not loaded — open Funds tab before export for sleeve summary.")

    agent_lines: list[str] = []
    if agent.get("summary"):
        agent_lines.append(str(agent["summary"]))
    if agent.get("regime_gate"):
        agent_lines.append(f"Regime gate: {agent['regime_gate']}")
    if agent.get("conviction_tier"):
        agent_lines.append(f"Conviction tier: {agent['conviction_tier']}")
    if agent.get("blocker"):
        agent_lines.append(f"Blocker: {agent['blocker']}")
    if agent.get("error"):
        agent_lines.append(f"Error: {agent['error']}")
    if not agent_lines:
        agent_lines.append("Agent council not loaded — open Agent tab before export.")

    pos_count = len(portfolio.get("positions") or [])
    portfolio_summary = portfolio.get("summary") or (f"{pos_count} positions" if pos_count else "No portfolio positions loaded")

    sections = [
        ("Today / Dashboard", _export_escape_html(_export_truncate(today.get("morning_decision") or today.get("board_message") or today.get("tradeability") or "Dashboard snapshot unavailable", 400))),
        ("Playbook", _export_escape_html(playbook.get("funnel_label") or playbook.get("board_message") or "Playbook not loaded")),
        ("Discovery", export_issues_list_html([])),  # placeholder replaced below
        ("Dossier", _export_escape_html("No dossier ticker loaded") if not (s.get("dossier") or {}).get("ticker") else _export_escape_html((s.get("dossier") or {}).get("summary") or "Dossier loaded")),
        ("Portfolio", _export_escape_html(portfolio_summary)),
        ("Funds", export_issues_list_html(funds_lines)),
        ("Agent", export_issues_list_html(agent_lines)),
        ("Ops", _export_escape_html(ops.get("verdict") or ops.get("intro") or "Ops console snapshot")),
        ("IBKR", _export_escape_html(ibkr.get("label") or ibkr.get("status") or "IBKR not connected")),
        ("Guide", _export_escape_html(guide.get("workflow") or "Core workflow: Dashboard → Playbook → Discovery → Dossier → Portfolio / IBKR.")),
    ]

    html_parts = ['<section class="cc-export-page cc-export-surfaces" data-cc-export="surfaces">', "<h1>CC · All surfaces snapshot</h1>"]
    list_sections = {"Funds", "Agent", "Discovery"}
    for title, body in sections:
        if title == "Discovery":
            html_parts.append("<h2>Discovery</h2>")
            verdict = ((s.get("discovery") or {}).get("data") or {}).get("discovery_verdict") or {}
            disc_lines = []
            if verdict.get("best_scanner_today"):
                disc_lines.append(f"Best scanner: {verdict['best_scanner_today']} ({verdict.get('best_scanner_hits') or 0} hits)")
            if not disc_lines:
                disc_lines.append("Discovery not loaded — open Discovery tab before export for funnel summary.")
            html_parts.append(export_issues_list_html(disc_lines))
            continue
        if title in list_sections:
            html_parts.append(f"<h2>{title}</h2>{body}")
        else:
            html_parts.append(f"<h2>{title}</h2><p>{body}</p>")
    html_parts.append('<footer class="cc-export-footer">End of CC review export · not trade authority</footer></section>')
    return "".join(html_parts)


def build_export_review_html(snapshot: Optional[Dict[str, Any]] = None) -> str:
    """Full printable review bundle — issues page + all surfaces."""
...
def sanitize_blocked_candidate_copy(
    ...
    *,
    blocked: bool = True,
    blocker: str = "no pilot entry — deploy authority blocked",
) -> str:
    """Blocked cards — monitor-only copy, no pilot/sizing/handoff language."""
    r = row or {}
    if not blocked:
        return str(r.get("display_copy") or r.get("summary") or r.get("why_now") or "")
    ticker = str(r.get("ticker") or "—").upper()
    bucket = r.get("primary_bucket") or r.get("lifecycle_bucket") or "Watch"
    return (
        f"{ticker} · {bucket} candidate · State: monitor only · "
        f"Blocked: {blocker}"
    )


def remove_trade_language_when_blocked(text: str, *, blocked: bool = True) -> str:
    """Strip deploy/sizing/handoff phrases when authority is blocked."""
    if not blocked:
        return str(text or "")
    out = str(text or "")
    replacements = (
...
def execution_repair_one_liner(truth: Optional[Dict[str, Any]] = None) -> str:
    """One-line execution repair banner for non-Ops surfaces."""
    t = truth or {}
    broker = str(t.get("broker_freshness") or "").lower()
    exec_gate = str(t.get("execution_gate") or "").lower()
    if broker in ("offline", "blocked") or exec_gate == "blocked":
        detail = "broker offline" if broker == "offline" else "execution blocked"
        return f"Execution blocked: {detail}. Open Repair Console."
    return ""
