...
def _trade_plan_human(
    ...
    *,
    ...
    """Human-readable structure snapshot / trade plan rows for dossier UI."""
    from src.services.dossier_mode import build_structure_snapshot_rows
    ...
    note = _dossier_trade_plan_note_value(tp, unified)
    invalidation = str(tp.get("invalidation") or unified.get("invalidation") or "")
    return build_structure_snapshot_rows(
        mode=mode,
        entry_zone=ez,
        stop=stop,
        target_1r=t1,
...
async def _build_intel_payload(
    ...
    *,
    ...
    from src.services.dossier_mode import resolve_dossier_mode, build_dossier_mode_block

    trust = dossier.get("trust") or {}
    trust_src = str(trust.get("source") or "")
    instant_degraded = "instant-degraded" in trust_src or trust.get("instant_degraded") is True
    brief_backed = "brief" in trust_src.lower() or trust.get("brief_backed") is True
    conf_line = str(confidence_metrics.get("decision_confidence_label") or "").lower()
    pending_calibration = "pending calibration" in conf_line
    has_quote = bool(dossier.get("price")) and float(dossier.get("price") or 0) > 0
    has_risk = bool(dossier.get("why_stop"))
    playbook_deploy_allowed = bool(decision_stack.get("board_gate") == "NOW")
    broker_online = bool(
        isinstance(ibkr, dict)
        and (ibkr.get("connected") or ibkr.get("online") or ibkr.get("ready"))
    )
    dossier_mode = resolve_dossier_mode(
        unified_label=str(unified.get("label") or ""),
        load_phase=load_phase,
        partial=partial,
        failed_fetch=bool(module_errors.get("dossier")),
        instant_degraded=instant_degraded,
        brief_backed=brief_backed,
        research_only=research_only,
        has_quote=has_quote,
        pending_calibration=pending_calibration,
        rr_unavailable=_rr_unavailable(unified),
        module_errors=module_errors,
    )
    freshness = str(trust.get("as_of") or trust.get("freshness") or "")
    trade_plan_human = _trade_plan_human(
        dossier.get("trade_plan") or {},
        unified,
        mode=dossier_mode,
        source=trust_src or "live_dossier",
        freshness=freshness,
        live_validated=dossier_mode == "usable" and not _rr_unavailable(unified),
    )
    dossier_mode_block = build_dossier_mode_block(
        mode=dossier_mode,
        ticker=ticker,
        unified_label=str(unified.get("label") or ""),
        instant_degraded=instant_degraded,
        brief_backed=brief_backed,
        has_quote=has_quote,
        has_technicals=bool(dossier.get("technicals")),
        has_peers=bool(peers_block.get("rows")),
        has_options=bool(options_block.get("has_data")),
        has_catalysts=bool(catalysts.get("items")),
        has_risk=has_risk,
        playbook_deploy_allowed=playbook_deploy_allowed,
        evidence_quality=str(principles_memo.get("evidence_quality") or "") if isinstance(principles_memo, dict) else "",
        calibration=conf_line or "Pending",
    ...
    return sanitize_for_json(
        {
            ...
            "dossier_mode": dossier_mode_block,
            "dossier_mode_key": dossier_mode,
            "dossier_operator_block": dossier_mode_block.get("dossier_operator_block"),
            "hide_trade_plan": dossier_mode_block.get("hide_trade_plan"),
            "hide_paper_draft": dossier_mode_block.get("hide_paper_draft"),
            "hide_sizing": dossier_mode_block.get("hide_sizing"),
            "evidence_status": dossier_mode_block.get("evidence_status"),
            "deploy_authority": {
                "allowed": playbook_deploy_allowed,
                "board_gate": decision_stack.get("board_gate"),
            },
            "broker_state": ibkr if isinstance(ibkr, dict) else {},
