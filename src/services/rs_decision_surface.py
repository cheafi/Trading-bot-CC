    ...
    *,
    ...
    return {
        ...
        "regime": {
            ...
            "stance": (
                "Funnel context — cross-check Playbook before promotion"
                if tradeability in ("TRADE", "STRONG_TRADE", "SELECTIVE")
                else "Funnel context only — board WAIT/NO_TRADE"
            ),
        },
        ...
        "warning": (
            None
            if live_rows
            else "Live RS unavailable — cached names below are funnel context only"
        ),
        "unavailable_fallback": (
            None
            if live_rows
            else {
                "reason": error_note or "live_compute_failed",
                "why": (
                    "Live RS universe could not be computed — "
                    "rankings below are cached brief fallback, not live relative strength."
                ),
                "next_safe_action": (
                    "Use Discovery scanners and Playbook for authority; "
                    "retry Live when market data feed recovers."
                ),
                "strongest_cached": [
                    {
                        "ticker": r.get("ticker"),
                        "sector": r.get("sector_short") or r.get("sector"),
                ...
                "label": "Stale / fallback — not live RS",
            }
        ),
    }
