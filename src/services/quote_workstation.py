...
def _operator_panel_error(exc: Exception, label: str) -> str:
    """Translate backend exceptions into operator-safe partial-state copy."""
    msg = str(exc).strip()
    lower = msg.lower()
    if "unexpected keyword argument" in lower:
        return f"{label} unavailable — chart service mismatch; use dossier chart tab"
    if "nonetype" in lower and "has no attribute" in lower:
        return f"{label} unavailable — upstream quote data not loaded yet"
    if "timeout" in lower or isinstance(exc, asyncio.TimeoutError):
        return f"{label} timed out — retry or continue with core dossier"
    if "connection" in lower or "unreachable" in lower:
        return f"{label} unavailable — market data path offline"
    if msg:
        return f"{label} unavailable — partial load"
    return f"{label} unavailable — partial load"
...
async def _panel(coro, label: str, timeout: float = _PANEL_TIMEOUT):
    try:
        ...
        return {"error": _operator_panel_error(asyncio.TimeoutError(), label), "partial": True}
    except Exception as exc:
        return {"error": _operator_panel_error(exc, label), "partial": True}
...
async def build_quote_workstation(request, ticker: str) -> Dict[str, Any]:
    ...
    async def _chart():
        ...
        return await live_chart_data(sym, request, period="6mo")
    ...
    for key, res in zip(keys, results):
        if isinstance(res, Exception):
            panels[key] = {
                "error": _operator_panel_error(res, key),
                "partial": True,
            }
        else:
            panels[key] = res
