...
class BacktestConfig:
    ...
    earnings_gap_extra_bps: float = (
        30.0  # extra slippage if entry within 2d of earnings
    )
...
class BacktestEngine:
    ...
    def _open_position(
        ...
        # Dynamic slippage
        slippage_frac = self._calculate_slippage(symbol, price, price_data or {}, date)
        ...
        self.logger.debug(
            f"Opened {side.value} position in {symbol} at {exec_price:.2f}"
        )
    ...
    def _calculate_results(
        ...
        equity_df = pd.DataFrame(
            self.equity_history, columns=["date", "equity"]
        ).set_index("date")
        ...
        if benchmark_data is not None and not benchmark_data.empty:
            try:
                ...
                if len(common_dates) > 20:
                    ...
                    benchmark_return = (
                        bench_aligned.iloc[-1] / bench_aligned.iloc[0]
                    ) - 1
                    benchmark_annual = (
                        (1 + benchmark_return) ** (1 / years) - 1 if years > 0 else 0
                    )
        ...
        avg_slippage_bps = np.mean(self.slippage_audit) if self.slippage_audit else 0
...
class WalkForwardOptimizer:
    ...
    def optimize(
        ...
        signal_generator: Callable[
            [Dict[str, pd.DataFrame], Dict[str, Any]], Dict[str, pd.DataFrame]
        ],
    ...
    def _label_single(
        ...
        if df is None or df.empty:
            return {
                ...
                "hit_stop": False,
                "hit_time_stop": False,
                "time_to_t1_days": None,
                "time_to_t2_days": None,
                "mae_pct": 0,
                "mfe_pct": 0,
        ...
        for i, (dt, row) in enumerate(window.iterrows()):
            ...
            if not hit_t2:
                if (is_long and high >= t2_price) or (not is_long and low <= t2_price):
                    ...
                    time_to_t2 = days_in
            if not hit_stop:
                if (is_long and low <= stop_price) or (
...
class AttributionEngine:
    ...
    def trade_management_attribution(
        ...
        [o["pnl_pct"] for o in outcomes if o.get("pnl_pct") is not None]
        ...
        return {
            ...
            "metrics": {
                ...
                "avg_capture_ratio": float(np.mean(capture_ratios))
                if capture_ratios
                else 0,
    ...
    def full_attribution(
        ...
        return {
            ...
            "sector": AttributionEngine.sector_attribution(trades, sector_map or {}),
            "factor": AttributionEngine.factor_attribution(
                trades, factor_exposures or {}
            ),
            "event": AttributionEngine.event_attribution(trades, calendar_events or []),
