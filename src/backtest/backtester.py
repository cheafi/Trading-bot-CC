...
from src.core.models import BacktestResult, Signal
...
class Backtester:
                ...
                if exit_result:
                    ...
                    trades.append(
                        Trade(
                            ...
                            pnl_pct=pnl_pct,
                            holding_period=(
                                current_date - pos["entry_date"].date()
                            ).days,
    ...
    def monte_carlo(
        self, trades: List[Trade], n_simulations: int = 1000
    ) -> Dict[str, Any]:
        """
    ...
    def detect_overfit(
        self, in_sample_result: BacktestResult, out_sample_result: BacktestResult
    ) -> Dict[str, Any]:
        """
        ...
        sharpe_degradation = (sharpe_ratio_is - sharpe_ratio_oos) / max(
            sharpe_ratio_is, 0.01
        )
    ...
    def _get_price(
        self, price_data: pd.DataFrame, ticker: str, date: datetime
    ) -> float:
    ...
    def _check_exit(
        self, position: Dict, current_price: float, current_date: datetime
        ...
        if direction == "LONG":
            if current_price <= stop:
                ...
                # current_price
                gap_fill = min(stop, current_price) * (1 - self.slippage)
                return (
                    gap_fill,
                    "stop_loss_gap"
                    if current_price < stop * (1 - self.gap_risk_pct)
                    else "stop_loss",
                )
            if current_price >= tp:
                return (
            ...
            if current_price >= stop:
                gap_fill = max(stop, current_price) * (1 + self.slippage)
                return (
                    gap_fill,
                    "stop_loss_gap"
                    if current_price > stop * (1 + self.gap_risk_pct)
        ...
        holding = (current_date - position["entry_date"].date()).days

        if holding >= self.max_holding_days:
            slip = 1 - self.slippage if direction == "LONG" else 1 + self.slippage
    ...
    def _aggregate_walk_forward(self, results: List[Dict]) -> Dict[str, Any]:
    ...
    def _calculate_pbo(
        self, sharpe_degradation: float, win_rate_degradation: float
    ) -> float:
        ...
        degradation_score = (
            abs(sharpe_degradation) * 0.6 + abs(win_rate_degradation) * 0.4
        )
