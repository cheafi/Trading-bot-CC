...
class FeatureEngine:
    ...
    def calculate_features(
        self, ohlcv: pd.DataFrame, sentiment_data: Optional[pd.DataFrame] = None
    ) -> pd.DataFrame:
        """
    ...
    def _calculate_single_ticker(
        self, df: pd.DataFrame, sentiment: Optional[pd.DataFrame] = None
    ) -> pd.DataFrame:
        ...
        features["dist_from_sma200"] = (close - features["sma_200"]) / features[
            "sma_200"
        ]
        ...
        features["bb_width"] = (features["bb_upper"] - features["bb_lower"]) / features[
            "sma_20"
        ]
        ...
        features["dist_from_52w_high"] = (features["high_52w"] - close) / features[
            "high_52w"
        ]
        features["dist_from_52w_low"] = (close - features["low_52w"]) / features[
            "low_52w"
        ].replace(0, 1)
        ...
        features["stoch_rsi"] = (
            (rsi_series - rsi_min) / (rsi_max - rsi_min).replace(0, 1)
        ) * 100
    ...
    def _calculate_trend_score(self, features: pd.DataFrame) -> pd.Series:
        ...
        dist_high = features.get(
            "dist_from_52w_high", pd.Series(0.5, index=features.index)
        )
    ...
    def get_latest_features(self, features: pd.DataFrame) -> pd.DataFrame:
        ...
        if isinstance(features.index, pd.MultiIndex):
            ...
            latest = (
                latest.droplevel("date") if "date" in features.index.names else latest
            )
            return latest
        else:
