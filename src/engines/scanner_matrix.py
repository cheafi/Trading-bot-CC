...
class ScannerMatrix:
    ...
    def enrich_hit_for_ui(
        ...
        *,
        ...
        sane = sanitize_score_display(
            score,
            sample_size=hit.metadata.get("calibration_n") if hit.metadata else None,
        )
        payload.update(sane)
        if not sane.get("valid"):
            payload["strength"] = None
            payload["score"] = None
            payload["score_display"] = "invalid"
            payload["score_display_label"] = "invalid"
        return payload
    ...
    def normalize_scanner_bucket(
        ...
        *,
        ...
        name = str(scanner_name or "").lower()
        if name in _DEBUG_ONLY_SCANNERS:
            return {
                "count": min(len(hits), DISCOVERY_QUALIFIED_WATCH_CAP),
                "top_hits": hits[:DISCOVERY_QUALIFIED_WATCH_CAP],
                "display_count": min(len(hits), DISCOVERY_QUALIFIED_WATCH_CAP),
                "debug_total_hits": len(hits),
                "debug_only": True,
                "label_note": "Validation cluster — debug field only, not primary card",
            }
        top = hits[:DISCOVERY_QUALIFIED_WATCH_CAP]
    ...
    def get_grouped_by_scanner(
        ...
        *,
        ...
        for cat, scanners in raw.items():
            grouped[cat] = {
                name: self.normalize_scanner_bucket(hits, scanner_name=name)
