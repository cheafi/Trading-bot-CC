...
class PerformanceTracker:
    ...
    async def record_signal(
        ...
        logger.info(
            f"Recorded signal {signal_id}: {ticker} {direction} @ {entry_price}"
        )

        return outcome

    async def update_signal(
        self, signal_id: str, current_price: float, timestamp: datetime = None
        ...
        if outcome.direction == TradeDirection.LONG:
            ...
            if current_price >= outcome.target_price:
                return await self._close_signal(
                    signal_id, outcome.target_price, SignalStatus.TARGET_HIT, timestamp
                )

            # Check stop hit
            if current_price <= outcome.stop_loss:
                return await self._close_signal(
                    signal_id, outcome.stop_loss, SignalStatus.STOP_HIT, timestamp
                )
        else:
    ...
    async def _close_signal(
        ...
        if outcome.direction == TradeDirection.LONG:
            outcome.pnl_pct = (
                (exit_price - outcome.entry_price) / outcome.entry_price * 100
            )
        else:
            outcome.pnl_pct = (
                (outcome.entry_price - exit_price) / outcome.entry_price * 100
            )

        # Calculate hold time
        outcome.hold_time_hours = (
        ...
        if risk > 0:
            outcome.risk_reward_actual = outcome.pnl_pct / (
                risk / outcome.entry_price * 100
            )
        ...
        logger.info(
            f"Closed signal {signal_id}: {status.value} | P&L: {outcome.pnl_pct:.2f}%"
        )
    ...
    async def close_signal_manual(
        ...
        return await self._close_signal(signal_id, exit_price, status, datetime.now())
    ...
    def get_performance_stats(
        ...
        if winners:
            stats.avg_winning_hold_time = sum(s.hold_time_hours for s in winners) / len(
                winners
            )
        if losers:
            stats.avg_losing_hold_time = sum(s.hold_time_hours for s in losers) / len(
                losers
            )

        # Streaks
        stats.current_streak, stats.max_win_streak, stats.max_lose_streak = (
            self._calculate_streaks(signals)
        )
    ...
    def format_stats_report(self, stats: PerformanceStats) -> str:
        ...
        lines.append(
            f"📊 **Performance Report** - {stats.period.replace('_', ' ').title()}"
        )
        lines.append("")

        # Summary
        win_emoji = (
            "🟢" if stats.win_rate >= 50 else "🟡" if stats.win_rate >= 40 else "🔴"
        )
        lines.append(f"**Win Rate:** {win_emoji} {stats.win_rate:.1f}%")
        lines.append(
            f"**Total P&L:** {'+' if stats.total_pnl_pct >= 0 else ''}{stats.total_pnl_pct:.2f}%"
        )
        lines.append("")
        ...
        lines.append(
            f"  Expectancy: {'+' if stats.expectancy >= 0 else ''}{stats.expectancy:.2f}%"
        )
        lines.append("")

        # Streaks
        streak_text = (
            f"+{stats.current_streak} wins"
            if stats.current_streak > 0
            else f"{stats.current_streak} losses"
            if stats.current_streak < 0
            else "neutral"
        )
        ...
        if stats.strategy_breakdown:
            ...
            for strat, data in sorted(
                stats.strategy_breakdown.items(),
                key=lambda x: x[1]["total_pnl"],
                reverse=True,
            ):
                emoji = "📈" if data["total_pnl"] > 0 else "📉"
                lines.append(
                    f"  {emoji} {strat}: {data['win_rate']}% WR, {'+' if data['total_pnl'] >= 0 else ''}{data['total_pnl']}%"
                )
    ...
    async def load_from_db(self):
        ...
        try:
            ...
            for row in rows:
                ...
                outcome = SignalOutcome(
                    signal_id=str(row.get("trade_id", row.get("signal_id", ""))),
                    ...
                    else SignalStatus.TARGET_HIT,
                    initial_confidence=float(row.get("confidence", 0)),
                    pnl_pct=float(row.get("pnl_pct", 0)),
                    hold_time_hours=float(row.get("hold_hours", 0)),
                )
            ...
            logger.info(
                "Loaded %d signals from DB (%d active, %d completed)",
                len(rows),
    ...
    def save_to_json(self, path: str = "") -> str:
        ...
        if not path:
            path = str(pathlib.Path.home() / ".tradingai" / "signal_outcomes.json")
        ...
        data = {
            "active": {
                k: self._outcome_to_dict(v) for k, v in self.active_signals.items()
            },
            "completed": [
                self._outcome_to_dict(o) for o in self.completed_signals[-500:]
            ],
    ...
    def load_from_json(self, path: str = "") -> int:
        ...
        try:
            ...
            for item in data.get("completed", []):
                try:
                    td = TradeDirection(item.get("direction", "long").lower())
                except ValueError:
                    td = TradeDirection.LONG
                ...
                exit_time = datetime.fromisoformat(xt) if isinstance(xt, str) else None
                outcome = SignalOutcome(
                    ...
                    exit_time=exit_time,
                    exit_price=float(item.get("exit_price", 0)) or None,
                    status=SignalStatus.TARGET_HIT,
                    pnl_pct=float(item.get("pnl_pct", 0)),
                    hold_time_hours=float(item.get("hold_hours", 0)),
                )
                self.completed_signals.append(outcome)
                count += 1
