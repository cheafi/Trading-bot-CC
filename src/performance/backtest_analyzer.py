...
class BacktestAnalyzer:
    ...
    def analyze_result(self, result: BacktestResult) -> Dict[str, any]:
        """
        ...
        if result.profit_factor > 2:
            analysis["strengths"].append(
                f"Strong profit factor ({result.profit_factor:.2f})"
            )

        if result.max_drawdown < 15:
            analysis["strengths"].append(
                f"Low max drawdown ({result.max_drawdown:.1f}%)"
            )

        if result.alpha > 5:
            analysis["strengths"].append(
                f"Significant alpha over benchmark (+{result.alpha:.1f}%)"
            )

        # Analyze weaknesses
        if result.sharpe_ratio < 0.5:
            analysis["weaknesses"].append(
                "Poor risk-adjusted returns - consider adding filters"
            )

        if result.max_drawdown > 30:
            analysis["weaknesses"].append(
                "Large drawdowns - implement better risk management"
            )
        ...
        if result.win_rate < 45 and result.profit_factor > 1.5:
            analysis["recommendations"].append(
                "Strategy relies on large winners - ensure you can hold through volatility"
            )

        if result.total_trades > 500:
            analysis["recommendations"].append(
                "High trade frequency - monitor slippage and commission impact"
            )
        ...
        score += (
            min(10, (result.profit_factor - 1) * 5) if result.profit_factor > 1 else -10
        )
...
""".strip()
    ...
    def monte_carlo_simulation(
        ...
        result = MonteCarloResult(
            simulations=simulations, original_return=original_return
        )
        ...
        variance = sum((r - mean) ** 2 for r in simulated_returns) / len(
            simulated_returns
        )
        ...
        result.probability_of_loss = (
            len([r for r in simulated_returns if r < 0]) / n * 100
        )
        result.expected_max_drawdown = sum(simulated_drawdowns) / len(
            simulated_drawdowns
        )
    ...
    def walk_forward_analysis(
            ...
            results["folds"].append(
                {
                    ...
                    "degradation": round(
                        ((in_return - out_return) / abs(in_return) * 100)
                        if in_return != 0
                        else 0,
                        1,
                    ),
        ...
        results["consistency"] = (
            profitable_oos / len(results["out_sample_returns"]) * 100
        )

        # Calculate efficiency (avg OOS return / avg IS return)
        avg_is = sum(results["in_sample_returns"]) / len(results["in_sample_returns"])
        avg_oos = sum(results["out_sample_returns"]) / len(
            results["out_sample_returns"]
        )
    ...
    def parameter_sensitivity(
        self, results: List[BacktestResult], parameter: str
    ) -> Dict[str, any]:
        """
        ...
        if sensitivity["sharpe_ratios"]:
            max_idx = sensitivity["sharpe_ratios"].index(
                max(sensitivity["sharpe_ratios"])
            )
        ...
        if sensitivity["returns"]:
            ...
            variance = sum(
                (r - mean_return) ** 2 for r in sensitivity["returns"]
            ) / len(sensitivity["returns"])
    ...
    def format_backtest_report(self, result: BacktestResult) -> str:
        ...
        years = (result.end_date - result.start_date).days / 365
        lines.append(
            f"**Period:** {result.start_date.strftime('%Y-%m-%d')} to {result.end_date.strftime('%Y-%m-%d')} ({years:.1f} years)"
        )
        lines.append("")
        ...
        lines.append(
            f"  Net P&L: ${result.final_capital - result.initial_capital:,.0f}"
        )
        lines.append("")
        ...
        lines.append(
            f"  Total: {'+' if result.total_return >= 0 else ''}{result.total_return:.2f}%"
        )
        lines.append(
            f"  Annualized: {'+' if result.annualized_return >= 0 else ''}{result.annualized_return:.2f}%"
        )
        if result.benchmark_return:
            lines.append(
                f"  Benchmark: {'+' if result.benchmark_return >= 0 else ''}{result.benchmark_return:.2f}%"
            )
            lines.append(
                f"  Alpha: {'+' if result.alpha >= 0 else ''}{result.alpha:.2f}%"
            )
        lines.append("")
        ...
        lines.append(
            f"  Avg Trade: {'+' if result.avg_trade >= 0 else ''}{result.avg_trade:.2f}%"
        )
        lines.append("")
        ...
        if result.monthly_returns:
            ...
            lines.append(
                f"**Monthly Win Rate:** {positive_months}/{total_months} ({positive_months / total_months * 100:.0f}%)"
            )
    ...
    def format_monte_carlo_report(self, result: MonteCarloResult) -> str:
        ...
        lines.append(
            f"  Original: {'+' if result.original_return >= 0 else ''}{result.original_return:.2f}%"
        )
        lines.append(
            f"  Mean: {'+' if result.mean_return >= 0 else ''}{result.mean_return:.2f}%"
        )
        lines.append(
            f"  Median: {'+' if result.median_return >= 0 else ''}{result.median_return:.2f}%"
        )
