...
class PerformanceAnalytics:
    ...
    def calculate_strategy_metrics(
        self, returns: List[float], strategy_name: str, trades_per_year: int = 252
    ) -> StrategyMetrics:
        """
    ...
    def compare_strategies(
        ...
        for i, m in enumerate(by_return):
            scores[m.strategy_name] = (
                scores.get(m.strategy_name, 0) + (len(strategy_metrics) - i) * 2
            )  # Weight: 2
        for i, m in enumerate(by_sharpe):
            scores[m.strategy_name] = (
                scores.get(m.strategy_name, 0) + (len(strategy_metrics) - i) * 3
            )  # Weight: 3
        for i, m in enumerate(by_dd):
            scores[m.strategy_name] = (
                scores.get(m.strategy_name, 0) + (len(strategy_metrics) - i) * 1
            )  # Weight: 1
    ...
    def format_metrics_report(self, metrics: StrategyMetrics) -> str:
        ...
        lines.append(
            f"  Total Return: {'+' if metrics.total_return >= 0 else ''}{metrics.total_return:.2f}%"
        )
        lines.append(
            f"  Annualized: {'+' if metrics.annualized_return >= 0 else ''}{metrics.annualized_return:.2f}%"
        )
        lines.append(
            f"  Avg per Trade: {'+' if metrics.avg_trade_return >= 0 else ''}{metrics.avg_trade_return:.2f}%"
        )
        lines.append("")
