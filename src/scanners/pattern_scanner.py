...
class PatternScanner:
    ...
    def scan_patterns(self, df: pd.DataFrame, ticker: str) -> List[ChartPattern]:
        """
        ...
        patterns = [
            p for p in patterns if p.pattern_quality >= self.min_pattern_quality
        ]
    ...
    def detect_trendlines(self, df: pd.DataFrame, ticker: str) -> List[Trendline]:
        """
    ...
    def calculate_support_resistance(self, df: pd.DataFrame, ticker: str) -> Dict:
        """
    ...
    def _detect_double_patterns(
            ...
            if abs(peak1_price - peak2_price) / peak1_price < 0.03:
                ...
                if current_price < neckline * 1.01:  # Breaking neckline
                    ...
                    pattern = ChartPattern(
                        ...
                        historical_success_rate=self.PATTERN_SUCCESS_RATES.get(
                            PatternType.DOUBLE_TOP, 0.70
                        ),
                        detected_at=datetime.now(),
                        start_date=df.index[last_two_peaks[0]]
        ...
        if len(troughs) >= 2:
            ...
            if abs(trough1_price - trough2_price) / trough1_price < 0.03:
                neckline_idx = np.argmax(
                    high[last_two_troughs[0] : last_two_troughs[1]]
                )
                ...
                if current_price > neckline * 0.99:
                    ...
                    pattern = ChartPattern(
                        ...
                        detected_at=datetime.now(),
                        start_date=df.index[last_two_troughs[0]]
                        if hasattr(df.index[0], "date")
                        else datetime.now(),
                        end_date=df.index[-1]
                        if hasattr(df.index[0], "date")
                        ...
                        pattern_quality=self._calculate_pattern_quality(
                            df, "double_bottom", last_two_troughs
                        ),
                        volume_confirmation=self._check_volume_confirmation(
                            df, last_two_troughs
                        ),
                        ...
                        risk_reward_ratio=abs(target - current_price)
                        / abs(current_price - trough1_price * 0.98),
    ...
    def _detect_head_shoulders(
        ...
        self._find_swing_lows(df, window=7)
        ...
        if len(peaks) >= 3:
            ...
            if (
                head_price > ls_price * 1.02
                and head_price > rs_price * 1.02
                and abs(ls_price - rs_price) / ls_price < 0.05
            ):
                # Find neckline
                ...
                if current_price < neckline * 1.02:
                    ...
                    pattern = ChartPattern(
                        ...
                        confidence=self._calculate_hs_confidence(
                            ls_price, head_price, rs_price, current_price, neckline
                        ),
                        historical_success_rate=self.PATTERN_SUCCESS_RATES.get(
                            PatternType.HEAD_AND_SHOULDERS, 0.83
                        ),
                        ...
                        risk_reward_ratio=abs(current_price - target)
                        / abs(rs_price * 1.02 - current_price),
    ...
    def _detect_triangles(self, df: pd.DataFrame, ticker: str) -> List[ChartPattern]:
    ...
    def _detect_flags(self, df: pd.DataFrame, ticker: str) -> List[ChartPattern]:
    ...
    def _detect_candlestick_patterns(
        ...
        if (
            lower_wick > body * 2 and upper_wick < body * 0.5 and close[-1] < close[-5]
        ):  # After downtrend
            pattern = ChartPattern(
                ticker=ticker,
        ...
        if len(df) >= 2:
            ...
            # Bullish Engulfing
            if (
                close[-2] < open_[-2]  # Previous red
                and close[-1] > open_[-1]  # Current green
                and close[-1] > open_[-2]  # Engulfs previous
                and open_[-1] < close[-2]
            ):
                pattern = ChartPattern(
                    ticker=ticker,
                    ...
                    pattern_quality=60,
                    volume_confirmation=df["volume"].values[-1]
                    > df["volume"].values[-2],
                    trend_alignment=True,
    ...
    def _detect_cup_handle(self, df: pd.DataFrame, ticker: str) -> List[ChartPattern]:
        ...
        if (
            0.12 < cup_depth < 0.35  # Cup 12-35% deep
            and abs(left_rim - right_rim) / left_rim < 0.05  # Rims similar
            and handle_low > cup_bottom  # Handle above cup bottom
            and current > handle_low
        ):  # Currently rising from handle
            pattern = ChartPattern(
                ticker=ticker,
    ...
    def _calculate_pattern_confidence(
        self, df: pd.DataFrame, key_points: List[int]
    ) -> float:
    ...
    def _calculate_pattern_quality(
        self, df: pd.DataFrame, pattern_type: str, key_points: List[int]
    ) -> float:
    ...
    def _fit_trendline(
        self, df: pd.DataFrame, points: List[int], line_type: str, ticker: str
    ) -> Optional[Trendline]:
        ...
        return Trendline(
            ...
            end_price=prices[-1],
            start_date=df.index[points[0]]
            if hasattr(df.index[0], "date")
            else datetime.now(),
            end_date=df.index[points[-1]]
            if hasattr(df.index[0], "date")
            else datetime.now(),
            touches=len(points),
    ...
    def _find_horizontal_levels(self, df: pd.DataFrame, ticker: str) -> List[Trendline]:
