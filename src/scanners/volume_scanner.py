...
class VolumeScanner:
    ...
    def scan_universe(self, market_data: Dict[str, pd.DataFrame]) -> List[VolumeAlert]:
    ...
    def _analyze_volume(self, ticker: str, df: pd.DataFrame) -> List[VolumeAlert]:
        ...
        price_change = (
            (close[-1] - close[-2]) / close[-2] * 100 if len(close) >= 2 else 0
        )

        # Calculate indicators
        obv = self._calculate_obv(close, volume)
        obv_trend = (
            "up" if obv[-1] > obv[-10] else "down" if obv[-1] < obv[-10] else "neutral"
        )
    ...
    def _calculate_smart_money(self, close: np.ndarray, volume: np.ndarray) -> float:
        """
            ...
            scores.append(
                {
                    ...
                    "price": close[-1],
                    "price_change_21d": (close[-1] - close[-21]) / close[-21] * 100
                    if len(close) >= 21
                    else 0,
                }
            )
