...
class SectorScanner:
    ...
    async def _analyze_sector(
        self, sector: Sector, market_data: Dict[str, pd.DataFrame]
    ) -> SectorMetrics:
        ...
        for ticker in stocks:
            ...
            volume = (
                df["volume"].values if "volume" in df.columns else np.ones(len(close))
            )
        ...
        return SectorMetrics(
            ...
            avg_rsi=avg_rsi,
            pct_overbought=sum(1 for r in sector_rsi if r > 70) / len(sector_rsi) * 100
            if sector_rsi
            else 0,
            pct_oversold=sum(1 for r in sector_rsi if r < 30) / len(sector_rsi) * 100
            if sector_rsi
            else 0,
    ...
    def _calculate_rankings(self, metrics: Dict[Sector, SectorMetrics]) -> None:
        ...
        sorted_sectors = sorted(
            metrics.items(), key=lambda x: x[1].return_21d, reverse=True
        )

        for rank, (sector, metric) in enumerate(sorted_sectors, 1):
            metric.rs_rank = rank

    def analyze_rotation(self, metrics: Dict[Sector, SectorMetrics]) -> SectorRotation:
        """
        ...
        rotation.leading_sectors = [
            s for s, m in sorted_by_rs[:4] if m.return_5d > 0 and m.return_21d > 0
        ]

        # Lagging: Bottom 3 by RS with negative momentum
        rotation.lagging_sectors = [
            s for s, m in sorted_by_rs[-4:] if m.return_5d < 0 or m.return_21d < 0
        ]
        ...
        defensive_avg = (
            np.mean(
                [metrics[s].return_21d for s in self.DEFENSIVE_SECTORS if s in metrics]
            )
            if any(s in metrics for s in self.DEFENSIVE_SECTORS)
            else 0
        )

        cyclical_avg = (
            np.mean(
                [metrics[s].return_21d for s in self.CYCLICAL_SECTORS if s in metrics]
            )
            if any(s in metrics for s in self.CYCLICAL_SECTORS)
            else 0
        )

        rotation.defensive_offensive_ratio = (
            defensive_avg / cyclical_avg if cyclical_avg != 0 else 1
        )
    ...
    def _generate_rotation_analysis(
        self, rotation: SectorRotation, metrics: Dict[Sector, SectorMetrics]
    ) -> str:
        """Generate human-readable rotation analysis."""
        lines = []

        lines.append(
            f"📊 **Sector Rotation Analysis** - {rotation.timestamp.strftime('%Y-%m-%d %H:%M')}"
        )
        lines.append("")
        lines.append(
        ...
        if rotation.leading_sectors:
            leaders = [
                s.value.replace("_", " ").title() for s in rotation.leading_sectors
            ]
            lines.append(f"🚀 **Leading Sectors:** {', '.join(leaders)}")

        if rotation.lagging_sectors:
            laggards = [
                s.value.replace("_", " ").title() for s in rotation.lagging_sectors
            ]
    ...
    def get_sector_heatmap(self, metrics: Dict[Sector, SectorMetrics]) -> pd.DataFrame:
        """
