...
class MomentumScanner:
    ...
    def scan_universe(
        ...
        if spy_data is not None and len(spy_data) >= 21:
            spy_return = (
                (spy_data["close"].iloc[-1] - spy_data["close"].iloc[-21])
                / spy_data["close"].iloc[-21]
                * 100
            )
    ...
    def _scan_ticker(
        self, ticker: str, df: pd.DataFrame, spy_return: float = 0
    ) -> List[MomentumAlert]:
        ...
        return_21d = (
            (close[-1] - close[-22]) / close[-22] * 100 if len(close) >= 22 else 0
        )
        ...
        if current_price > high_20d * 0.99:  # Within 1% of 20-day high
            ...
            alert = MomentumAlert(
                ...
                confidence=self._calculate_breakout_confidence(
                    df, volume_ratio, breakout_strength
                ),
        ...
        if current_price < low_20d * 1.01:
            ...
            alert = MomentumAlert(
                ...
                confidence=self._calculate_breakout_confidence(
                    df, volume_ratio, breakdown_strength
                ),
                volume_confirmation=volume_ratio >= 1.5,
                trend_aligned=return_21d < 0,
                detected_at=datetime.now(),
        ...
        if len(open_) >= 2:
            ...
            if abs(gap_percent) >= self.min_gap_percent:
                ...
                signal_type = (
                    MomentumSignalType.GAP_UP
                    if gap_percent > 0
                    else MomentumSignalType.GAP_DOWN
                )
                ...
                alert = MomentumAlert(
                    ...
                    volume_confirmation=volume_ratio >= 1.5,
                    trend_aligned=(gap_percent > 0 and return_21d > 0)
                    or (gap_percent < 0 and return_21d < 0),
                    detected_at=datetime.now(),
                    ...
                    targets=[current_price * 1.05, current_price * 1.10]
                    if gap_percent > 0
                    else [current_price * 0.95, current_price * 0.90],
                )
                alerts.append(alert)
    ...
    def _calculate_breakout_confidence(
        self, df: pd.DataFrame, volume_ratio: float, breakout_strength: float
    ) -> float:
    ...
    def get_top_movers(self, market_data: Dict[str, pd.DataFrame], n: int = 10) -> Dict:
        """
        ...
        return {
            "gainers": [
                {"ticker": t, "change": c, "price": p} for t, c, p in gainers[:n]
            ],
            "losers": [
                {"ticker": t, "change": c, "price": p} for t, c, p in losers[:n]
            ],
        }

    def get_volume_leaders(
        self, market_data: Dict[str, pd.DataFrame], n: int = 10
    ) -> List[Dict]:
