...
class UniverseProvider:
    ...
    def get_universe(self, filter_config: Optional[UniverseFilter] = None) -> List[str]:
        """
    ...
    def _apply_filters(self, tickers: List[str], config: UniverseFilter) -> List[str]:
        ...
        if config.include_sectors:
            filtered = [
                ...
                for t in filtered
                if (
                    t in self.SECTOR_MAPPING
                    and self.SECTOR_MAPPING[t].value in config.include_sectors
                )
...
class DynamicUniverseProvider(UniverseProvider):
    ...
    async def _passes_technical_filter(
        self, ticker: str, filters: Dict[str, Any]
    ) -> bool:
