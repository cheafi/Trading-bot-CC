...
from .momentum_scanner import MomentumAlert, MomentumScanner
from .pattern_scanner import ChartPattern, PatternScanner
from .sector_scanner import Sector, SectorMetrics, SectorRotation, SectorScanner
from .volume_scanner import VolumeAlert, VolumeScanner
...
class MarketMonitor:
    ...
    async def scan_market(
        ...
        momentum_task = asyncio.create_task(
            asyncio.to_thread(
                self.momentum_scanner.scan_universe, market_data, spy_data
            )
        )
        ...
        breadth = (
            ...
            else MarketBreadth(timestamp=timestamp)
        )
        ...
        rotation = (
            self.sector_scanner.analyze_rotation(sector_metrics)
            if sector_metrics
            else None
        )
    ...
    def _calculate_breadth(self, market_data: Dict[str, pd.DataFrame]) -> MarketBreadth:
    ...
    def _assess_market_status(
        self, breadth: MarketBreadth, sector_metrics: Dict[Sector, SectorMetrics]
    ) -> str:
    ...
    def _generate_observations(
        ...
        if breadth.ad_ratio > 2:
            observations.append(
                f"🟢 Strong breadth: {breadth.advances} advances vs {breadth.declines} declines"
            )
        elif breadth.ad_ratio < 0.5:
            observations.append(
                f"🔴 Weak breadth: {breadth.advances} advances vs {breadth.declines} declines"
            )

        if breadth.new_highs > breadth.new_lows * 3:
            observations.append(
                f"📈 New highs dominating: {breadth.new_highs} highs vs {breadth.new_lows} lows"
            )
        elif breadth.new_lows > breadth.new_highs * 3:
            observations.append(
                f"📉 New lows dominating: {breadth.new_lows} lows vs {breadth.new_highs} highs"
            )
        ...
        if rotation:
            ...
            observations.append(
                f"📊 Market phase: {rotation.market_phase.replace('_', ' ').title()}"
            )
        ...
        if patterns:
            ...
            observations.append(
                f"📐 Patterns detected: {bullish_patterns} bullish, {bearish_patterns} bearish"
            )
        ...
        if momentum:
            ...
            if breakouts > 5:
                observations.append(
                    f"💥 Multiple breakouts detected: {breakouts} stocks"
                )

        # Volume observations
        if volume:
            accumulation = sum(
                1 for v in volume if v.signal_type.value == "accumulation"
            )
            distribution = sum(
                1 for v in volume if v.signal_type.value == "distribution"
            )
            if accumulation > distribution * 2:
                observations.append(
                    f"💰 Accumulation dominance: {accumulation} vs {distribution} distribution"
                )
            elif distribution > accumulation * 2:
                observations.append(
                    f"⚠️ Distribution dominance: {distribution} vs {accumulation} accumulation"
                )
    ...
    def get_market_summary(self) -> str:
        ...
        lines.append(
            f"📊 **Market Summary** - {snap.timestamp.strftime('%Y-%m-%d %H:%M')}"
        )
        lines.append("")
        ...
        lines.append(
            f"  • New Highs/Lows: {snap.breadth.new_highs}/{snap.breadth.new_lows}"
        )
        lines.append("")
    ...
    def get_actionable_alerts(self, min_confidence: float = 65) -> List[Dict]:
