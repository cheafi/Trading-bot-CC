...
class AdvancedPatternScanner:
    ...
    def _detect_pattern(
        self, symbol: str, df: pd.DataFrame, pattern_type: PatternType
    ) -> List[PatternMatch]:
    ...
    def _detect_vcp(self, symbol: str, df: pd.DataFrame) -> List[PatternMatch]:
        ...
        pattern = PatternMatch(
            ...
            volatility_contraction=contractions[-1][1] / contractions[0][1]
            if contractions[0][1] > 0
            else 1.0,
            prior_uptrend=True,
        )
    ...
    def _detect_flat_base(self, symbol: str, df: pd.DataFrame) -> List[PatternMatch]:
        ...
        prior_advance = (prior["close"].iloc[-1] - prior["close"].iloc[0]) / prior[
            "close"
        ].iloc[0]
    ...
    def _detect_bull_flag(self, symbol: str, df: pd.DataFrame) -> List[PatternMatch]:
        ...
        for pole_len in range(5, 15):
            ...
            pole_gain = (
                pole_section["close"].iloc[-1] - pole_section["close"].iloc[0]
            ) / pole_section["close"].iloc[0]
            ...
            if pole_gain >= 0.15:  # At least 15% pole
                ...
                flag_trend = np.polyfit(
                    range(len(flag_section)), flag_section["close"].values, 1
                )[0]
                ...
                flag_depth = (
                    flag_section["high"].max() - flag_section["low"].min()
                ) / flag_section["high"].max()
    ...
    def _detect_ascending_triangle(
        ...
        low_prices = [pt[1] for pt in lows]
        if not all(
            low_prices[i] <= low_prices[i + 1] for i in range(len(low_prices) - 1)
        ):
            return results
    ...
    def _find_contractions(self, df: pd.DataFrame) -> List[Tuple[int, float]]:
        ...
        for i in range(0, len(df) - window, window // 2):
            ...
            range_pct = (section["high"].max() - section["low"].min()) / section[
                "high"
            ].max()
    ...
    def _find_local_minima(
        self, series: pd.Series, min_separation: int = 5
    ...
    def _find_horizontal_level(
        self, prices: np.ndarray, tolerance: float = 0.02
    ) -> List[float]:
