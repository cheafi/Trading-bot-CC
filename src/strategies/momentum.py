...
from src.core.models import Direction, Horizon, Signal
...
class MomentumStrategy(BaseStrategy):
    ...
    def generate_signals(
        ...
        # Uptrend filter
        uptrend_mask = (df["close"] > df["sma_50"]) & (df["sma_50"] > df["sma_200"])
        ...
        # Combined filter
        candidates = df[
            uptrend_mask & trend_mask & momentum_mask & rsi_mask & volume_mask
        ]

        self.logger.info(
            f"Found {len(candidates)} momentum candidates from {len(df)} tickers"
        )
