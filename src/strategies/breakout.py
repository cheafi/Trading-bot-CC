...
class BreakoutStrategy(BaseStrategy):
    ...
    DEFAULT_CONFIG = {
        ...
        # 2026 hardening: require an actual close above the prior 20-day high,
        # not a "within 1% of breakout" near-miss. Near-misses are statistically
        # indistinguishable from failed breakouts and erode the edge.
        "require_true_breakout": True,
        "breakout_buffer_pct": 0.0,  # 0% = strict close > high_20d
    }
    ...
    def generate_signals(
        ...
        # Breakout filter: price closes above the prior 20-day high (true break).
        # The previous `close > high_20d * 0.99` admitted near-misses 1% below
        # the breakout level, which is statistically indistinguishable from
        # failed breakouts. Config flag retained for back-compat.
        if self.config.get("require_true_breakout", True):
            buf = float(self.config.get("breakout_buffer_pct", 0.0))
            breakout_mask = df["close"] > df["high_20d"] * (1.0 + buf)
        else:
            breakout_mask = df["close"] > df["high_20d"] * 0.99
