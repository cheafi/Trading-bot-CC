...
class MeanReversionStrategy(BaseStrategy):
    ...
    def generate_signals(
        ...
        # Calculate z-score of 21-day return PER TICKER over its own history.
        # Cross-sectional z-score (the previous implementation) mixes high-vol
        # tech with low-vol utilities and produces spurious "oversold" signals
        # that drift with composition. A per-ticker rolling z-score is the
        # statistically correct way to flag a name as oversold vs. itself.
        if {"ticker", "return_21d"}.issubset(
            df.reset_index().columns
        ) and df.index.name != "ticker":
            # MultiIndex / column 'ticker' case: rolling per ticker
            try:
                if "ticker" in df.index.names:
                    grp = df.groupby(level="ticker")["return_21d"]
                else:
                    grp = df.groupby("ticker")["return_21d"]
                mu = grp.transform(lambda s: s.rolling(63, min_periods=20).mean())
                sd = grp.transform(lambda s: s.rolling(63, min_periods=20).std())
                df["zscore_21d"] = ((df["return_21d"] - mu) / sd).fillna(0)
            except (KeyError, ValueError):
                df["zscore_21d"] = 0
        else:
            # Flat per-ticker snapshot (one row per ticker): there is no history
            # in this frame to compute a rolling z-score, so use the supplied
            # `return_zscore_63d` feature if present, otherwise fall back to a
            # conservative cross-sectional z-score (clearly labeled).
            if "return_zscore_63d" in df.columns:
                df["zscore_21d"] = df["return_zscore_63d"].fillna(0)
            else:
                ...
                if std_return > 0:
                    ...
                    df["zscore_21d"] = 0
                self.logger.warning(
                    "Mean reversion fell back to cross-sectional z-score; "
                    "supply per-ticker history or 'return_zscore_63d' feature."
                )
