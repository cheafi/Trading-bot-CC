...
class MacroAnalyzer:
    ...
    async def analyze_event(self, event_data: Dict) -> MacroEvent:
        """
    ...
    def _analyze_fed_implications(self, event: MacroEvent) -> str:
        ...
        if event.event_type == MacroEventType.CPI:
            ...
            if event.surprise_direction == "above":
                ...
                return (
                    "Weakening labor market may prompt Fed to consider easier policy."
                )
            else:
    ...
    def _analyze_bond_implications(self, event: MacroEvent) -> str:
        ...
        if event.event_type in [MacroEventType.CPI, MacroEventType.PPI]:
            if event.surprise_direction == "above":
                return (
                    "Yields likely to rise on hotter inflation. Duration risk elevated."
                )
            elif event.surprise_direction == "below":
    ...
    def _analyze_equity_implications(self, event: MacroEvent) -> str:
        ...
        if event.event_type == MacroEventType.CPI:
            ...
            if event.surprise_direction == "above":
                return (
                    "Strong jobs data generally positive for cyclicals and financials."
                )
            elif event.surprise_direction == "below":
    ...
    def _analyze_sector_implications(self, event: MacroEvent) -> Dict[str, str]:
        ...
        if event.event_type == MacroEventType.CPI:
            ...
            if event.surprise_direction == "above":
                implications["Consumer Discretionary"] = (
                    "Strong consumer supports spending"
                )
    ...
    def _generate_trading_notes(self, event: MacroEvent) -> str:
        ...
        if event.event_type in [MacroEventType.CPI, MacroEventType.EMPLOYMENT]:
            notes.append(
                "Watch Treasury yields and Fed funds futures for market interpretation"
            )
...
Provide a 2-sentence trading-focused summary of what this means for markets today."""
    ...
    def generate_macro_outlook(self, recent_events: List[MacroEvent]) -> MacroOutlook:
        ...
        employment_events = [
            e for e in recent_events if e.event_type == MacroEventType.EMPLOYMENT
        ]
        ...
        if outlook.inflation_trend == "rising" and outlook.employment_trend == "strong":
            ...
            outlook.inflation_trend == "falling"
            and outlook.employment_trend == "weakening"
        ):
        ...
        # Risk environment
        if (
            outlook.growth_outlook in ["accelerating", "stable"]
            and outlook.inflation_trend != "rising"
        ):
    ...
    def _identify_themes(
        self, events: List[MacroEvent], outlook: MacroOutlook
    ) -> List[str]:
    ...
    def _generate_outlook_summary(self, outlook: MacroOutlook) -> str:
        ...
        lines.append(
            f"📊 **Macro Outlook** - {outlook.generated_at.strftime('%Y-%m-%d')}"
        )
        lines.append("")
        ...
        lines.append("")
        lines.append(
            f"**Risk Environment:** {outlook.risk_environment.replace('_', ' ').title()}"
        )
    ...
    def format_event_brief(self, event: MacroEvent) -> str:
        ...
        if event.actual is not None:
            ...
            lines.append(
                f"Actual: {event.actual} | Forecast: {event.forecast} | Prior: {event.previous}"
            )
