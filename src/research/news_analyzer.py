...
class NewsAnalyzer:
    ...
    async def analyze_news_batch(self, news_items: List[Dict]) -> List[NewsItem]:
        """
    ...
    def _calculate_relevance(
        self, raw: Dict, tickers: List[str], category: NewsCategory
    ) -> float:
...
{{
    ...
    async def generate_brief(
        self, news_items: List[NewsItem], period: str = "morning"
    ) -> NewsBrief:
        """
        ...
        """
        brief = NewsBrief(
            generated_at=datetime.now(), period=period, headline="", market_mood="mixed"
        )
        ...
        # Categorize
        brief.earnings_news = [
            n for n in news_items if n.category == NewsCategory.EARNINGS
        ][:5]
        brief.analyst_actions = [
            n for n in news_items if n.category == NewsCategory.ANALYST
        ][:5]
        brief.macro_events = [
            n for n in news_items if n.category == NewsCategory.MACRO
        ][:5]
        ...
        bullish_items = [
            n
            for n in news_items
            if n.sentiment in [NewsSentiment.BULLISH, NewsSentiment.VERY_BULLISH]
        ]
        bearish_items = [
            n
            for n in news_items
            if n.sentiment in [NewsSentiment.BEARISH, NewsSentiment.VERY_BEARISH]
        ]
    ...
    async def _generate_ai_summary(self, brief: NewsBrief, news_items: List[NewsItem]):
...
Keep it concise and actionable."""
    ...
    def _generate_basic_summary(
        self, brief: NewsBrief, news_items: List[NewsItem]
    ) -> str:
        """Generate basic summary without AI."""
        lines = []

        lines.append(
            f"📰 **{brief.period.title()} Brief** - {brief.generated_at.strftime('%Y-%m-%d %H:%M')}"
        )
        lines.append("")
    ...
    def format_brief_text(self, brief: NewsBrief) -> str:
        ...
        mood_emoji = {"risk_on": "🟢", "risk_off": "🔴", "mixed": "🟡"}.get(
            brief.market_mood, "⚪"
        )
