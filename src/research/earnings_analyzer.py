...
class EarningsAnalyzer:
    ...
    async def analyze_earnings(
        self, ticker: str, earnings_data: Dict
    ) -> EarningsReport:
        """
        ...
        report.eps_result = self._determine_result(
            report.metrics.eps_actual, report.metrics.eps_estimate
        )
        report.revenue_result = self._determine_result(
            report.metrics.revenue_actual, report.metrics.revenue_estimate
        )
    ...
    def _calc_surprise(
        self, actual: Optional[float], estimate: Optional[float]
    ) -> Optional[float]:
    ...
    def _analyze_guidance(self, data: Dict) -> GuidanceChange:
        ...
        if (
            "raise" in guidance_text
            or "increase" in guidance_text
            or "higher" in guidance_text
        ):
            return GuidanceChange.RAISED
        elif (
            "lower" in guidance_text
            or "reduce" in guidance_text
            or "cut" in guidance_text
        ):
    ...
    def _generate_headline(self, report: EarningsReport) -> str:
        ...
        report.revenue_result.value.capitalize() + "s"

        if (
            report.eps_result == EarningsResult.BEAT
            and report.revenue_result == EarningsResult.BEAT
        ):
            ...
            report.eps_result == EarningsResult.MISS
            or report.revenue_result == EarningsResult.MISS
        ):
    ...
    def _estimate_price_reaction(self, report: EarningsReport) -> str:
        ...
        if report.overall_sentiment == "bullish":
            ...
            if report.guidance_change in [
                GuidanceChange.LOWERED,
                GuidanceChange.WITHDRAWN,
            ]:
...
Focus on: What matters most for the stock price? What should traders watch?"""
    ...
    def format_report_brief(self, report: EarningsReport) -> str:
        ...
        emoji = (
            ...
            if report.overall_sentiment == "bearish"
            else "🟡"
        )
        lines.append(
            f"{emoji} **{report.ticker} Earnings** - {report.fiscal_quarter} {report.fiscal_year}"
        )
        lines.append("")
        ...
        if report.metrics.eps_actual:
            ...
            lines.append(
                f"  {eps_emoji} EPS: ${report.metrics.eps_actual:.2f} vs ${report.metrics.eps_estimate:.2f} exp"
            )
        ...
        if report.metrics.revenue_actual:
            ...
            lines.append(
                f"  {rev_emoji} Revenue: ${report.metrics.revenue_actual:.2f}B vs ${report.metrics.revenue_estimate:.2f}B exp"
            )
