...
class FeatureConfig:
    ...
    return_periods: List[int] = field(
        default_factory=lambda: [1, 5, 10, 21, 63, 126, 252]
    )
...
class FeaturePipeline:
    ...
    def _add_volatility_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for period in self.config.volatility_periods:
            ...
            df[f"volatility_{period}d_rank"] = df[f"volatility_{period}d"].rank(
                pct=True
            )

        # Volatility ratio (short vs long term)
        if (
            10 in self.config.volatility_periods
            and 60 in self.config.volatility_periods
        ):
    ...
    def prepare_training_data(
        self, df: pd.DataFrame, dropna: bool = True
    ...
    def generate_cross_sectional_features(
        ...
        if panels:
            ...
            for col, rank_col in [
                ...
                panel[rank_col] = panel.groupby(panel.index)[col].rank(pct=True)
        else:
            panel = pd.DataFrame()
        ...
        for ticker, df in all_data.items():
            ...
            if market_data is not None and len(df_copy) == len(market_data):
                df_copy["relative_strength"] = df_copy["close"].pct_change(
                    20
            ...
            if not panel.empty and ticker in panel["ticker"].values:
                ...
                for col in ["return_rank", "volume_rank", "volatility_rank"]:
                    if col in tk_ranks.columns:
                        df_copy[col] = tk_ranks[col].reindex(df_copy.index)
    ...
    def generate_regime_features(
        ...
        out["vol_regime_z"] = ((vol_20 - vol_mean) / vol_std.replace(0, np.nan)).fillna(
            0
        )
        ...
        if market_df is not None and "close" in market_df.columns:
            ...
            out["sector_momentum_20d"] = (
                (aligned - mkt_ret).reindex(out.index).fillna(0)
            )
