...
class FactorLibrary:
    ...
    def idiosyncratic_volatility(
        stock_returns: pd.Series, market_returns: pd.Series, window: int = 60
    ) -> pd.Series:
        """
    ...
    def sma_cross_signal(
        prices: pd.Series, short: int = 20, long: int = 50
    ) -> pd.Series:
        """
    ...
    def amihud_illiquidity(
        prices: pd.Series, volume: pd.Series, period: int = 20
    ) -> pd.Series:
        """
...
class AlphaFactorEngine:
    ...
    def compute_factors(
        self, price_data: Dict[str, pd.DataFrame], as_of_date: Optional[datetime] = None
    ) -> pd.DataFrame:
        """
    ...
    def _compute_single_factor(self, df: pd.DataFrame, factor: AlphaFactor) -> float:
    ...
    def normalize_factors(
        self, factor_df: pd.DataFrame, method: str = "zscore"
    ) -> pd.DataFrame:
        """
    ...
    def compute_ic_time_series(
        ...
        for period in forward_periods:
            for factor_name in factor_df.columns:
                ...
                results.append({"factor": factor_name, "period": period, "ic": ic})

        return pd.DataFrame(results)

    @staticmethod
    def quantile_returns(
        factor: pd.Series, forward_returns: pd.Series, n_quantiles: int = 5
    ) -> pd.Series:
        """
...
class SimpleMLPredictor(MLPredictor):
    ...
    def feature_importance(self) -> pd.Series:
        ...
        if hasattr(self.model, "feature_importances_"):
            ...
            return pd.Series(
                np.abs(self.model.coef_), index=self.feature_names
            ).sort_values(ascending=False)
        else:
            return pd.Series()
