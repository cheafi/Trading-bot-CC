...
class SimpleQLearningAgent(RLAgent):
    ...
    def initialize_bins(self, states: np.ndarray):
        ...
        for i in range(states.shape[1]):
            bins = np.percentile(
                states[:, i], np.linspace(0, 100, self.n_bins + 1)[1:-1]
            )
...
class RLTrainer:
    ...
    def train(
        self, n_episodes: int = 100, log_interval: int = 10, save_interval: int = 50
    ) -> Dict[str, List[float]]:
        """
...
class RLSignalGenerator:
    ...
    def generate_signal(
        self, df: pd.DataFrame, current_position: float = 0.0
    ) -> Dict[str, Any]:
        """
        ...
        state = np.concatenate(
            [features, [current_position, unrealized_pnl, cash_ratio]]
        )
        ...
        # Map to signal
        action_map = {Action.HOLD: "hold", Action.BUY: "buy", Action.SELL: "sell"}
