...
from src.services.reports_library import create_report_from_shadow, export_report
...
from src.services.research_store import list_reports as list_report_rows
...
async def get_pipeline_steps(_=optional_api_key):
    return sanitize_for_json(
        {
            "steps": pipeline_step_labels(),
            "safety": research_safety_contract(surface="strategy-lab"),
        }
    )
...
async def post_strategy_parse(body: Dict[str, Any], _=optional_api_key):
    ...
    return sanitize_for_json(
        {"draft": draft, "safety": research_safety_contract(surface="strategy-lab")}
    )
...
async def get_strategies(limit: int = 30, _=optional_api_key):
    return sanitize_for_json(
        {
            "drafts": list_strategy_drafts(limit=limit),
            "safety": research_safety_contract(surface="strategy-lab"),
        }
    )
...
async def post_validate(body: Dict[str, Any], request: Request, _=optional_api_key):
    ...
    metrics = (
        (body or {}).get("metrics")
        if isinstance((body or {}).get("metrics"), dict)
        else {}
    )
    ...
    payload = {
        ...
        "report": report,
        "safety": research_safety_contract(),
    }
...
async def post_pipeline(body: Dict[str, Any], request: Request, _=optional_api_key):
    ...
    steps = (
        (body or {}).get("steps")
        if isinstance((body or {}).get("steps"), list)
        else None
    )
    result = run_research_pipeline(
        prompt=prompt, system_state=ss, backtest_metrics=metrics, steps=steps
    )
...
async def post_committee(body: Dict[str, Any], request: Request, _=optional_api_key):
    subject = (
        (body or {}).get("subject")
        if isinstance((body or {}).get("subject"), dict)
        else {}
    )
...
async def post_shadow(body: Dict[str, Any], request: Request, _=optional_api_key):
    ...
    if tags:
        try:
            ...
            push_notice(
                ...
                meta={
                    "tags": ", ".join(tags),
                    "pnl_delta": analysis.get("pnlDifference"),
                },
            )
        except Exception:
            pass
    payload = {
        "shadow": analysis,
        "report": report,
        "safety": research_safety_contract(surface="shadow"),
    }
...
async def get_shadow_runs(limit: int = 20, _=optional_api_key):
    return sanitize_for_json(
        {
            "runs": list_shadow_runs(limit=limit),
            "safety": research_safety_contract(surface="shadow"),
        }
    )
...
async def get_reports(request: Request, limit: int = 30, _=optional_api_key):
    payload = {
        "reports": list_report_rows(limit=limit),
        "safety": research_safety_contract(surface="reports"),
    }
...
async def get_report_detail(report_id: str, _=optional_api_key):
    ...
    return sanitize_for_json(
        {"report": report, "safety": research_safety_contract(surface="reports")}
    )
...
async def get_memory(
    limit: int = 50, item_type: Optional[str] = None, _=optional_api_key
):
    return sanitize_for_json(
        {
            "memory": list_memory(limit=limit, item_type=item_type),
            "safety": research_safety_contract(),
        }
    )
...
async def post_export_pine(body: Dict[str, Any], _=optional_api_key):
    draft = (
        (body or {}).get("draft") if isinstance((body or {}).get("draft"), dict) else {}
    )
