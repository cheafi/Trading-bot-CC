...
;(function (global) {
	...
	function localizeIbkrBracketReason(reason) {
		...
		var map = {
			...
			"Waiting for nextValidId / order queue": "等待 nextValidId／訂單佇列 · Waiting for nextValidId / order queue",
			"Bracket preview pending stop + target fields": "Bracket 預覽待填止損＋目標 · Bracket preview pending stop + target",
			"Bracket builder ready — parent + OCA children on transmit": "Bracket 已就緒 — 送出時 parent + OCA 子單 · Bracket builder ready on transmit",
		}
		return map[r] || r
	}
	...
	function localizeOperatorCopy(text) {
		...
		var map = {
			...
			"review levels; no handoff until gates open": "檢視價位；閘門開啟前勿 handoff · review levels; no handoff until gates open",
			...
			"no sleeve allocation today; repair market data first": "今日不做 sleeve 配置；先修復市場資料 · no sleeve allocation today; repair market data first",
		}
	...
	function localizeOpsRuntimeText(text) {
		...
		var exact = {
			...
			"Probe warming — reachability not confirmed": "探測預熱 — 可達性未確認 · Probe warming — reachability not confirmed",
			...
			"Cycle executed — regime output present": "已執行 cycle — 體制輸出存在 · Cycle executed — regime output present",
			"Engine off — no runtime path": "引擎關閉 — 無執行路徑 · Engine off — no runtime path",
			"Session active — no order test this session": "Session 已連線 — 本 session 未測試下單 · Session active — no order test this session",
			"Gateway reachable — session inactive": "Gateway 可達 — session 未啟動 · Gateway reachable — session inactive",
			"Live handoff exercised this session": "本 session 已演練 live handoff · Live handoff exercised this session",
			...
			"Unreachable, no live handoff possible": "不可達，無法 live handoff · Unreachable, no live handoff possible",
		...
		if (/^Consumed in completed cycle/.test(t)) {
			return "於完成的 cycle 中已消費（" + t.replace(/^Consumed in completed cycle \(/, "").replace(/\)$/, "") + "） · " + t
		}
	...
	function localizeOpsAdvancedSectionDetail(detail) {
		...
		var map = {
			...
			"Start engine or refresh ops console for evidence.":
				_opsBilingual("請啟動引擎或重新整理 Ops 主控台以取得證據", "Start engine or refresh ops console for evidence."),
		}
		if (map[t]) return map[t]
		var need = t.match(/^Need (\d+)\+ observations \(have (\d+)\)$/)
		if (need) {
	...
	function pageOperatorSentence(tab, systemState, ctx) {
		...
		if (tabSafe === "signals") {
			base = {
				now: deployOpen
					? "Deploy review · 可檢視 deploy-qualified"
					: "無可部署名單 · No deploy names",
				blocker: deployOpen ? localizeOperatorCopy("verify execution readiness") : localizeOperatorCopy("board gate " + tb + " + 0 deploy-qualified"),
			...
			base = {
				...
				blocker:
					fetchState === "failed_fetch" ? localizeOperatorCopy("API fetch failed") : blocked,
			...
			base = {
				...
				nextAction: mockOnly ? "今日忽略 flow · Ignore flow today" : localizeOperatorCopy("confirm in Playbook only"),
				scope: "flow",
			}
...
/* CC_APP_BUNDLE_START — cc() Alpine app (source: cc-app.js) */
    ...
    function cc(){return{
      ...
      cc_status:{mode:'PAPER',breaker:false,breaker_reason:'',ibkr_connected:false,ibkr_mode:'paper',ibkr_gateway:false,ibkr_monitoring:false,ibkr_health_label:'',uptime_s:0,last_fetch:0},
      ccHeader:{decision_authority:null,page_authority_mode:'diagnostic',portfolio_context:null,header_summary:null},
      surfaceFetchHints:{},
      live:false,clock:'',regime:{},indices:[],macro:[],sectors:[],asia:[],trust:{},
      marketStrip:{loading:false,error:'',source:'',lastOk:null,snapshotAt:null,renderedAt:null,fromPulse:false},
      sig:{mode:'—',source:'',as_of:'',recs:[],strategy_scores:{},no_trade_reason:null,scan_meta:null},
      dos:{ticker:'',loading:false,loadingEnrich:false,status:'idle_no_query',data:null,intel:null,oppIntel:null,oppIntelLoading:false,error:'',partialNotice:'',fetchDetail:'',failedModule:'',staleAt:null,subTab:'decision',showSignals:true,chartPeriod:'6mo',benchPeriod:'1y',chartSignals:[],benchStats:null,benchSummary:null,benchMonthly:null,benchQuarterly:null,benchYearly:null,benchTab:'monthly',buyPrice:'',advice:null,adviceLoading:false,peers:null,peersLoading:false,conviction:null,convictionLoading:false,optionsData:null,optionsLoading:false},
      playbookFocusTicker:'',
      pf:{loading:false,source:'manual',positions:[],alerts:[],summary:null,corrPreview:null,showAdd:false,showAdvanced:false,addTicker:'',addShares:0,addEntry:0,addStop:0,addT1r:0,addT2r:0,addNotes:'',addSleeve:'',addSector:'',addError:'',addSuccess:'',editTicker:null},
      pfDecision:null,pfDecisionLoading:false,pfQuantSizing:null,
      pfEquity:null,pfEquityLoading:false,
      platformExtras:{catalyst:null,pmMemo:null,loading:false},
      aosMonitors:{rules:[],alerts:[],loading:false},
      vibeAgent:{
        loading:false,error:'',status:null,brief:null,rules:[],alerts:[],intents:[],journal:[],
        intentText:'',intentPlan:null,guardrail:null,page_capability:null,safety:null,
        ruleDraft:{name:'',asset:'',ruleType:'price_zone_touch',condition:'',expiryDays:14},
        paused:false,
      },
      strategyLab:{
        loading:false,error:'',prompt:'',draft:null,validation:null,committee:null,pipeline:null,
        page_capability:null,safety:null,pineExport:'',
      },
      ...
      ledgerView:{loading:false,loaded:false,rows:[],stats:null,filterStrategy:'',filterDirection:'',filterTicker:'',expanded:false},
      ...
      opsPanel:'health',
      changelogPanel:{loading:false,loaded:false,error:'',timeout:false,version:'',product:'CC',entries:[]},
      ...
      scannerHub:{loading:false,data:null,category:null,duration_ms:0,last_run:null,universe:0,error:'',expanded:false},
      rsPanel:{loading:false,data:null,error:'',sectorFilter:'',expanded:false},
      rankedOpps:{loading:false,refreshing:false,fetch_failed:false,rows:[],actionFilter:'',sectorFilter:'',source:'playbook',board_mode:'',board_mode_label:'',board_message:'',board_explanation:'',snapshot_timestamp:'',rejection_clusters:[],rejection_clusters_note:'',unlock_deploy:null,emergency:null,bestAction:null,decision_authority:null,overlapWarning:null,warning:'',avoid_grouped:null,bucket_quality:[],near_miss:[],avoid_collapsed:true,filter_funnel:null,degraded_banner:'',instant_degraded:false,operator_board:null,watch_queues:null,watch_intelligence_summary:null,ai_vibe:null,board_posture:null,paper_automation:null,auto_execution:null,monitor_auto_actions:null,operator_sections_open:false,compact_rows:null,rank_buckets:null,system_state:null},
      ...
      init(){
        ...
        if(typeof window!=='undefined'){
          window.addEventListener('resize',()=>{this.pmStripChipMenuOpen=false});
        }
        this.tick();setInterval(()=>this.tick(),1000);
        this.fetchHealth();setInterval(()=>this.fetchHealth(),15000);
        ...
        setTimeout(()=>{if(!this.playbookBoardHasContent())this.fetchWarmupBriefBoard()},400);
        setTimeout(()=>{
          if(this.rankedOpps.loading) this.rankedOpps.loading=false;
          if(this.rankedOpps.refreshing) this.rankedOpps.refreshing=false;
        },10000);
        try{this.instantDegradedDismissed=sessionStorage.getItem('cc_instant_degraded_dismissed')==='1';}catch(e){}
      },
      ...
      async runTimeTravel(){
        ...
        try{
          ...
          if(!r.ok){
            ...
            this.tt.err=this.formatReplayError(typeof msg==='string'?msg:(msg&&msg[0])||msg);return;
          }
          this.tt.data=await r.json();
        }catch(e){this.tt.err=this.formatReplayError(e.message)}
      ...
      async regenerateBrief(){
        ...
        try{
          ...
          if(j.ok){
            const note=j.rolled_forward?' (rolled forward — live fetch unavailable; re-run when yfinance works)':'';
            alert('✓ Brief updated: '+(j.after?.date||'')+' — '+(j.after?.size_bytes||0)+' bytes'+note);
          }
      ...
      async ccFetch(url, opts={}){
        ...
        for(let i=0;i<tries;i++){
          try{
            ...
            if(/^\/api\//.test(url)){
              const headers=init.headers&&typeof init.headers==='object'&&!Array.isArray(init.headers)?{...init.headers}:{};
      ...
      normalizeFetchError(msg){
        ...
        if(/failed to fetch|networkerror|network error|load failed|fetch failed|HTTP fail|HTTP 0|no-trade HTTP/i.test(s)) return 'failed_fetch';
      ...
      async ccFetchJson(url, opts={}){
        ...
        this.surfaceFetchHints[loadingTab]={...(this.surfaceFetchHints[loadingTab]||{}),loading:true,error:''};
        try{
          ...
          if(!r.ok){
            ...
            try{const j=await r.json();detail=j.detail||j.error||detail;}catch(_e){}
            const state=this.normalizeFetchError(String(detail))||(r.status===503?'stale':'failed_fetch');
            const msg=this.surfaceFetchStateMessage(state, this.normalizeFetchError(String(detail))?null:detail);
            this.surfaceFetchHints[loadingTab]={loading:false,error:msg,failed_fetch:state==='failed_fetch',stale:state==='stale'};
      ...
      surfaceFetchErrorLine(msg,tab){
        ...
        if(/HTTP 503/i.test(s)){
          if(mode==='dossier_research') return 'Live dossier fetch failed from '+this.dosFetchServiceName()+' (API warming up)';
          return this.surfaceWarmupLoadingLine(mode);
        }
        if(mode==='dossier_research'&&(!s||/stock intel unavailable|research unavailable/i.test(s))) return 'Live dossier fetch failed from '+this.dosFetchServiceName();
      ...
      dossierFetchStateCopy(state, detail, service){
        ...
        if(state==='failed_fetch'){
          return {badge:'CONFIRM ONLY',title:'即時研究暫不可用 · confirm-only',explanation:'核心資料暫時未齊 · 只可確認結構，不可 sizing。',next_action:'Retry 重試 live fetch；Load core only 保留 core；Load enrichments 補回擴展研究。',state:'failed_fetch'};
        }
        if(state==='loading'){
          return {badge:'LOADING',title:'載入 Dossier 研究中',explanation:'正在從 '+svc+' 抓取 live research；空面板唔代表結論。',next_action:'等 core 載入完成，或者 Retry。',state:'loading'};
        }
        if(state==='stale'){
          return {badge:'CONFIRM ONLY',title:'顯示快取研究',explanation:'目前只顯示 cached core；live fetch 未完成。',next_action:'Retry 後再確認 levels / verdict；未恢復前不可 sizing。',state:'stale'};
        }
        if(state==='partial'){
          if(svc.includes('instant-degraded')){
            let explanation='Core shell 已載入，但仍未回到 live research。';
            if(detail) explanation+=' ('+String(detail)+')';
            return {badge:'CONFIRM ONLY',title:'核心模式 · confirm-only',explanation,next_action:'Load enrichments 或 Retry；恢復前只可確認結構，不可 sizing / handoff。',state:'partial'};
          }
          let explanation='部分研究 modules 未回來；證據仍未完整。';
          if(detail) explanation+=' ('+String(detail)+')';
          return {badge:'CONFIRM ONLY',title:'部分研究載入',explanation,next_action:'Retry 或 Load enrichments；完整前只可 confirm。',state:'partial'};
        }
      ...
      _dossierIntelDegraded(intel){
        ...
        return !!(intel.partial||intel.degraded||intel.research_only||intel.instant_degraded
          ||trust.stale||src.includes('instant-degraded')||src==='brief-fallback');
      },
      _dossierPartialNoticeFromIntel(intel){
        ...
        if(mod.dossier&&this._dossierIntelDegraded(intel)&&!/^Live dossier fetch failed/i.test(String(mod.dossier))){
      ...
      dosFetchBannerRestoreLine(){
        ...
        if(kind==='partial') return 'Retry / Load enrichments 可補回即時研究 modules；Load core only 只保留 confirm-only core。';
      ...
      dossierGateSnapshotLine(){
        ...
        const tb=String(cs.tradeability_state?.tradeability||this.canonicalTradeability()||'WAIT').toUpperCase();
        ...
        if(es&&['EXEC_BLOCKED','ENGINE_OFF','GATEWAY_DOWN','IBAPI_MISSING','SESSION_INACTIVE','HANDOFF_BLOCKED'].includes(es)) parts.push('執行 '+es.replace(/_/g,' '));
      ...
      dosFetchBannerGrade(){
        const kind=this.dosFetchBannerKind();
        if(kind==='partial') return 'Retry / Load enrichments 補回 live research；Load core only 保留 confirm-only core。';
      ...
      _syncDossierFetchHints(extra={}){
        const loading=!!(this.dos.loading||this.dos.status==='loading_core'||this.dos.status==='loading_enrichments');
        let error='';
      ...
      btLabDegradedLine(){
        ...
        const note=(t.evidence&&t.evidence.trust&&t.evidence.trust.reason)||(t.trust&&t.trust.reason)||(t.walk_forward&&t.walk_forward.note)||'';
        return note||'Full walk-forward lab pending API warm-up — research-only, not deployment authority.';
      },
      formatReplayError(msg){
        const s=String(msg||'');
        if(/could not convert string to float/i.test(s)||/Invalid replay input/i.test(s)||/Invalid research input/i.test(s)){
          const m=s.match(/'([^']+)'/);
      ...
      rrBelowTradeGate(v){const rr=this.parseRR(v);return rr>0&&rr<2.5;},
      ...
      isDevMode(){
        ...
        try{return localStorage.getItem('cc_dev')==='1'}catch(e){return false}
      },
      formatTargetWeights(targetsObj){
        const t=targetsObj||{};
        const entries=Object.entries(t)
          .map(([k,v])=>[String(k||'').toUpperCase().trim(), Number(v)])
      ...
      portfolioPrimaryRiskBlocker(){
        ...
        const msg='stop missing on '+t+(n>1?(' ('+n+' '+this.pluralize(n,'position')+' without stops)'):'');
      ...
      async portfolioSaveStop(){
        ...
        if(!t||!stop||stop<=0){alert('Stop must be > 0');return}
        try{
          const r=await fetch('/api/portfolio/position',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({ticker:t,stop_price:stop})});
          if(r.ok){
      ...
      currentSurfaceCcState(){
        ...
        if(!cs||typeof cs!=='object'){
          cs=this.ccHeader.cc_state||this.today7.cc_state||this.rankedOpps.cc_state||this.dos?.cc_state||this.pfDecision?.cc_state;
        }
      ...
      decisionAuthority(){
        ...
        if(this.tab==='signals') return this.rankedOpps.decision_authority||this.ccHeader.decision_authority||{};
        if(this.tab==='portfolio'&&this.pfDecision?.cc_state?.board_decision_state) return this.pfDecision.cc_state.board_decision_state||{};
        if((this.tab==='dossier'||this.tab==='stock-intel')&&this.dos?.cc_state?.board_decision_state) return this.dos.cc_state.board_decision_state||{};
        return this.ccHeader.decision_authority||this.today7.decision_authority||this.rankedOpps.decision_authority||{};
      },
      executionState(){
        const cs=this.ccState();
      ...
      ccFreshnessState(){
        ...
        const market=trust.stale?'STALE':(String(trust.freshness||'').toUpperCase()==='DEGRADED'?'STALE':'FRESH');
        const da=this.decisionAuthority();
        const src=String(da.source||'');
        const board=(src==='stale_cache'||src==='fallback_brief'||da.degraded||da.gates_active)?'STALE':'FRESH';
        let dossier='';
        if((this.tab==='dossier'||this.tab==='stock-intel'||(this.dos&&this.dos.ticker))&&this.dos){
          ...
          else if(this.dos.status==='failed'||this.dos.status==='stale_fallback'||this.dos.status==='partial_loaded') dossier='STALE';
        }
        const ex=this.executionState();
        const es=String(ex.state||'').toUpperCase();
        const execution=(es==='EXEC_BLOCKED')?'CRITICAL':(['ENGINE_OFF','GATEWAY_DOWN','IBAPI_MISSING','SESSION_INACTIVE','HANDOFF_BLOCKED','DISCONNECTED'].includes(es)?'STALE':'FRESH');
        const tiers=[['market',market],['board',board],['dossier',dossier],['execution',execution]].filter(([,t])=>t);
        let worstTier='FRESH';
        let worstDomain='';
        for(const [d,t] of tiers){
          if(t==='CRITICAL'){worstTier='CRITICAL';worstDomain=d;break;}
          if(t==='STALE'&&worstTier!=='CRITICAL'){worstTier='STALE';worstDomain=worstDomain||d;}
        }
      ...
      systemState(){
        ...
        if(this.today7?.system_state&&typeof this.today7.system_state==='object') return this.today7.system_state;
      ...
      _clientSystemState(){
        ...
        const opts={
          ...
          data_tier:String(fs.worst_tier||ss.data_freshness||(this.pageAuthorityIsDegraded()?'STALE':'FRESH')),
        };
        if(typeof CCHelpers!=='undefined'&&CCHelpers.buildClientSystemState)
          return CCHelpers.buildClientSystemState(opts);
        return {tradeability:opts.tradeability,blocker_compact:opts.blocker,repair_priority:opts.repair,global_strip_active:opts.degraded};
      },
      ...
      pageOperatorSentence(){
        const cap=this.tab==='today'?this.today7?.page_capability
          :this.tab==='signals'?this.rankedOpps?.page_capability
          :this.tab==='agent'?this.vibeAgent?.page_capability
          :this.tab==='strategy-lab'?this.strategyLab?.page_capability
          :this.tab==='shadow'?this.shadowAccount?.page_capability
          :this.tab==='reports'?this.reportsLib?.page_capability
        ...
        const ctx={
          ...
          mock_only:this.tab==='flow'&&(!!this.flowPanel?.decision?.freshness?.synthetic||this.flowOverlayDegraded?.()),
        };
      ...
      _SURFACE_MODES:{today:'dashboard_core',signals:'playbook_core',scanners:'discovery_research',dossier:'dossier_research','stock-intel':'dossier_research',portfolio:'portfolio_manual',funds:'funds_research',flow:'flow_supporting',rs:'rs_supporting',notrade:'rejections_diagnostic',rejections:'rejections_diagnostic',guide:'guide_reference',ops:'ops_diagnostic',ibkr:'ibkr_execution',btlab:'backtest_research',backtest:'backtest_research',command:'command_research'},
      _HEADER_BASE:{
        dashboard_core:{badge:'BOARD GATE',title:'Dashboard — 今日 deploy 姿態',explanation:'Regime + board gate + deploy posture。只有 tradeability 同 execution 對齊先可 sizing。',next_action:'先睇 tradeability，再睇 top ranked。'},
        playbook_core:{badge:'BOARD REVIEW',title:'Playbook — 排序機會',explanation:'Ranked board；只有 board_mode 完整，而且名稱 execution-ready，先可 Deploy。',next_action:'先篩 execution-ready rows，再交叉確認 Dashboard gate。'},
        discovery_research:{badge:'RESEARCH ONLY',title:'Discovery — 全市場掃描',explanation:'只屬 scanner funnel input；呢度出現嘅候選本身唔授權交易。',next_action:'有興趣嘅名稱送去 Playbook 或 Dossier。'},
        dossier_research:{badge:'RESEARCH ONLY',title:'Dossier — 單一標的研究',explanation:'每隻 ticker 嘅證據整理；唔可單獨當成 deploy permission。',next_action:'行動前先去 Dashboard 確認 board posture。'},
        portfolio_manual:{badge:'BOOK CONSTRUCTION',title:'Portfolio — 倉位與風險層級',explanation:'手動 book construction 同風險管理；override 由你決定，唔係由 scanner 自動決定。',next_action:'再平衡前先對齊 manual book 同 IBKR。'},
        funds_research:{badge:'MODEL EVIDENCE',title:'Funds — sleeve 研究',explanation:'Model sleeves 同 backtest 證據；backtest ≠ live track record。',next_action:'落資金前仍要 live validation。'},
        flow_supporting:{badge:'CONFIRMATION ONLY',title:'Flow — options 敘事輔助',explanation:'Flow 只支援 scan-ranked 名稱；唔可作為單獨入場觸發。',next_action:'先確認 Playbook + Dossier 一致。'},
        rs_supporting:{badge:'RESEARCH ONLY',title:'Relative strength — Discovery 輸入',explanation:'RS 只幫手排序 funnel 候選；唔代表 deploy authority。',next_action:'交叉核對 sector regime 同 Playbook；經 Discovery 提升。'},
        command_research:{badge:'RESEARCH ONLY',title:'Command — 進階聚合',explanation:'Terminal aggregate 視圖；只係深度 decision dump，唔係 deploy gate。',next_action:'Deploy authority 仍以 Dashboard + Playbook 為準。'},
        rejections_diagnostic:{badge:'AUDIT TRAIL',title:'Rejections — 閘門失敗紀錄',explanation:'解釋名稱點解過唔到 gates；屬診斷用途，唔係 deploy permission。',next_action:'整理 watchlist，再回到 Playbook 睇可行 board。'},
        backtest_research:{badge:'BACKTEST ONLY',title:'Backtest Lab — walk-forward 研究',explanation:'Walk-forward 研究；backtest ≠ live track record。',next_action:'用 lab 睇 walk-forward diagnostics；落 sizing 前仍要 live validation。'},
        guide_reference:{badge:'GUIDE MODE',title:'Guide — 參考頁',explanation:'文檔同 workflow 參考；呢度唔評估 runtime。',next_action:'準備好先返 Dashboard 睇 live board posture。'},
        ops_diagnostic:{badge:'OPS / CONNECTIVITY',title:'Ops — runtime 診斷',explanation:'Engine 健康同 providers 狀態；workflow status ≠ capital permission。',next_action:'返 Dashboard 前先修好 stale provider 或 breaker。'},
        ibkr_execution:{badge:'EXECUTION GATE',title:'IBKR — broker handoff',explanation:'連線同落單準備度；關鍵錯誤會直接阻止 execution。',next_action:'先連 gateway，再確認 session_usable。'},
      },
      pageSurface(){
        const m=this._SURFACE_MODES||{};
        return m[this.tab]||'discovery_research';
      },
      surfaceShowsDecisionChips(mode){return mode==='dashboard_core'||mode==='playbook_core';},
      sanitizeVisibleText(v){
        if(v==null||v==='') return '';
        const s=String(v);
        if(/led',e\);alert\(/i.test(s)||/\[object Object\]/i.test(s)) return s.includes('[object Object]')?'Evidence unavailable':'';
      ...
      normalizedAuthorityChipForTab(t, rawEntry){
        ...
        const hardBlocked=['EXEC_BLOCKED','ENGINE_OFF','GATEWAY_DOWN','IBAPI_MISSING','SESSION_INACTIVE','HANDOFF_BLOCKED'].includes(es)
          || tb==='NO_TRADE'
          || bs==='SUSPENDED'
          || da.authority_level==='suspended';
        const degraded=this.pageAuthorityIsDegraded()||tb==='WAIT'||bs==='RESEARCH_ONLY'||da.degraded||da.gates_active;
        const needsResearch=mode==='discovery_research'||mode==='rs_supporting'||mode==='funds_research'||mode==='backtest_research'||mode==='command_research';
        ...
        if(mode==='dashboard_core'||mode==='playbook_core'){
          ...
          return {
            ...entry,
            badge:hardBlocked?'BLOCKED':'MONITOR ONLY',
            short:hardBlocked?(this.primaryBlockerChip()+' · no deploy authority'):'盤面未開閘 · ranked for review only',
            authority:hardBlocked?'blocked':'monitor_only',
            authority_label:hardBlocked?'Deploy blocked by board / broker / execution truth':'Board not open — monitor only',
          };
        }
        return {
          ...entry,
      ...
      surfaceFetchStateCopy(state, detail){
        const copy={
          loading:{badge:'LOADING',title:'資料載入中 · Loading',explanation:'即時資料仍在載入；唔好把快取內容當成 deploy authority。',next_action:'等抓取完成，或者 refresh 分頁。'},
          failed_fetch:{badge:'FETCH FAILED',title:'載入失敗 · Fetch failed',explanation:'Network 或 server 錯誤令今次無法 fresh read。',next_action:'先 Retry；如果持續失敗，再睇 Ops diagnostics。'},
          failed_fetch_fallback:{badge:'FETCH FAILED',title:'即時抓取失敗 · 顯示 fallback',explanation:'你而家見到嘅係 fallback watchlist samples，唔係即時 scanner 輸出。',next_action:'Sizing 前先去 Playbook 確認；API 恢復後再 Retry。'},
          stale:{badge:'STALE',title:'資料過時 · Stale',explanation:'上次成功抓取已經過時；目前 ranking 同 posture 未必反映現況。',next_action:'行動前先 refresh 呢個 surface。'},
          fallback:{badge:'FALLBACK',title:'後備畫面 · Fallback',explanation:'即時 scanner 暫不可用；目前只顯示 brief / snapshot fallback，唔係 execution-grade board。',next_action:'等 live API 恢復後返 Dashboard；目前名稱只可 Monitor。'},
          partial:{badge:'PARTIAL',title:'部分載入 · Partial',explanation:'部分 modules 失敗；而家只見到不完整 evidence。',next_action:'Sizing 前先檢查 Ops → Data Providers。'},
          probe_only:{badge:'PROBE ONLY',title:'連線探針 · Probe',explanation:'Ops probe 只證明 wiring，唔係可投資訊號。',next_action:'先修復 provider health，再信 downstream surfaces。'},
          runtime_unknown:{badge:'RUNTIME UNKNOWN',title:'Runtime 未明 · Unknown',explanation:'Engine 或 backend 狀態未能確認。',next_action:'先去 Ops 確認 engine + provider health。'},
          mock_only:{badge:'MOCK ONLY',title:'示意資料 · Mock',explanation:'Synthetic / mock rows，唔係真實市場事件。',next_action:'唔好把 mock rows 提升出 research preview。'},
          research_only:{badge:'RESEARCH ONLY',title:'研究頁面 · Research',explanation:'目前只屬 degraded / model data；唔代表 deploy authority。',next_action:'Sizing 前先回 Dashboard / Playbook 確認。'},
          execution_blocked:{badge:'EXEC BLOCKED',title:'執行封鎖 · Blocked',explanation:'Broker offline、breaker 觸發，或關鍵 IBKR 檢查失敗。',next_action:'落單前先去 IBKR 頁修復連線。'},
          no_data:{badge:'NO DATA',title:'暫無資料 · No data',explanation:'抓取成功，但呢個 surface 今次回傳空集合。',next_action:'WAIT 日呢種情況屬正常；耐心本身就係正確動作。'},
          not_authoritative:{badge:'NOT AUTHORITATIVE',title:'非授權頁面 · Not authoritative',explanation:'Deploy chips 只屬 Dashboard / Playbook。',next_action:'切去 Dashboard 睇 board gate 同 deploy posture。'},
      ...
      opsDegradedCopy(state, detail){
        const copy={
          loading:{badge:'LOADING',title:'載入中 · Loading',explanation:'Ops panel 仲載入緊；空面板唔等於健康。',next_action:'等抓取完成，或者 refresh 呢個 tab。'},
          fallback:{badge:'FALLBACK',title:'後備模式 · Fallback',explanation:'Live Ops 抓取失敗；目前顯示 built-in fallback，唔係權威 release notes。',next_action:'API 恢復後再 Retry；如要改 canonical notes，編輯 data/changelog.json。'},
          unavailable:{badge:'UNAVAILABLE',title:'暫不可用 · Unavailable',explanation:'無法載入 Ops panel；session state 未能確認。',next_action:'幾秒後再 Retry；確認 API 正在運行，再 refresh Ops。'},
          runtime_unknown:{badge:'RUNTIME UNKNOWN',title:'Runtime 未明 · Unknown',explanation:'呢個 panel 未能確認 engine/backend runtime evidence。',next_action:'去 Ops health 重新確認 engine + provider 狀態。'},
          retry_recommended:{badge:'RETRY',title:'建議重試 · Retry',explanation:'Network/server error 阻止 fresh Ops read。',next_action:'幾秒後再 Retry；如持續失敗，用 Ops diagnostics 排查。'},
        };
      ...
      opsInferDegradedState(ctx){
        ...
        if(c.error){
          ...
          if(/failed to fetch|networkerror|load failed|timeout|http 503|http 502|warming up/.test(s)) return 'retry_recommended';
          return 'unavailable';
        }
      ...
      opsApiWarmupMessage(raw){
        const s=String(raw||'');
        return /Failed to fetch|NetworkError|Load failed|fetch failed|ECONNREFUSED|warming up|API warming|HTTP 503|HTTP 502/i.test(s);
      },
      opsAnyPanelLoading(){
        return !!(this.opsConsole?.loading||this.opsRuntime?.loading||this.changelogPanel?.loading||this.errorLog?.loading);
      },
      opsGlobalLoadingLine(){
        ...
        const errs=[this.opsConsole?.error,this.opsRuntime?.error,this.changelogPanel?.error,this.errorLog?.error].filter(Boolean);
        for(const e of errs){
      ...
      headerContext(){
        ...
        const ctx={
          ...
          fallback:((mode==='dashboard_core'&&this.todayUsesBriefFallback())||(mode==='playbook_core'&&this.playbookUsesBriefFallback())||!!hints.fallback),
          mock_only:!!hints.mock_only,
          research_only:!!hints.research_only,
          execution_blocked:!!this.cc_status.breaker||this.degradedExecutionUnavailable()||!!hints.execution_blocked,
        ...
        if(mode==='flow_supporting'){
          ...
          if(this.flowPanel.decision?.degraded||this.flowPanel.decision?.freshness?.synthetic||this.flowPanel.decision?.mock_only){
            ctx.mock_only=true;
            ctx.stale=true;
        ...
        if(mode==='playbook_core'&&this.rankedOpps.loading&&!this.playbookBoardHasContent()) ctx.loading=true;
        ...
        if(mode==='rejections_diagnostic'){
          ...
          if(!ctx.loading&&!ctx.error&&this.rejectionsPanel.data&&!((this.rejectionsPanel.data.no_trade_signals||[]).length)) ctx.empty=true;
        }
        if(mode==='funds_research'){
          ctx.loading=ctx.loading||this.fundMonitor.loading;
          if(this.fundMonitor.data?.degraded||this.fundMonitor.data?.research_only||this.fundMonitor.data?.trust?.stale) ctx.stale=true;
        ...
        if(mode==='backtest_research'){
          ...
          if(this.btLab.data?.degraded||this.btLab.data?.research_only||this.btLab.data?.trust?.stale) ctx.stale=true;
          if(this.btLab.data?.research_only) ctx.research_only=true;
        }
        if(mode==='dossier_research'){
          ctx.loading=ctx.loading||this.dos.loading||this.dos.status==='loading_core'||this.dos.status==='loading_enrichments';
      ...
      headerSummary(){
        ...
        const fetchBadges={loading:'LOADING',stale:'STALE',fallback:'FALLBACK',partial:'PARTIAL',failed_fetch:'FETCH FAILED',failed_fetch_fallback:'FETCH FAILED',mock_only:'MOCK ONLY',research_only:'RESEARCH ONLY',execution_blocked:'EXEC BLOCKED',no_data:'NO DATA',not_authoritative:'NOT AUTHORITATIVE',probe_only:'PROBE ONLY',runtime_unknown:'RUNTIME UNKNOWN',ok:''};
        const chips=[];
        if(this.surfaceShowsDecisionChips(mode)&&['ok','stale','fallback'].includes(fetchState)){
          ...
          if(chipAuth.badge==='DEPLOY'&&ctx.deploy_label) chips.push({label:String(ctx.deploy_label),class:'deploy'});
          else if(ctx.deploy_label&&deployLbl&&!['DEPLOY','TRADE','ALLOCATE'].includes(deployLbl)) chips.push({label:String(ctx.deploy_label),class:'pa'});
          else if(this.pageAuthorityIsDegraded()||this.isWaitDay()) chips.push({label:chipAuth.badge||'MONITOR ONLY',class:'pa'});
          ...
          if((pc.broker_sync||'unavailable')==='unavailable'||this.portfolioHeaderBrokerSyncUnavailable()){
        ...
        const subtitle=(this.surfaceShowsDecisionChips(mode)&&!this.discoveryHeaderMode()&&this.decisionHub)
          ? this.canonicalRegimeLine()
          : (mode==='discovery_research'
            ? ('Research-only · '+this.canonicalRegimeLine())
            : '');
        const showChips=this.surfaceShowsDecisionChips(mode)&&!this.discoveryHeaderMode()&&!!this.decisionHub&&['ok','stale','fallback'].includes(fetchState);
        let badge=mode==='guide_reference'?base.badge:(fetchBadges[fetchState]||base.badge);
        let explanation=fetchState==='failed_fetch_fallback'?this.surfaceFetchStateCopy('failed_fetch_fallback').explanation:base.explanation;
        let nextAction=fetchState==='failed_fetch_fallback'?this.surfaceFetchStateCopy('failed_fetch_fallback').next_action:base.next_action;
        ...
        const normalizedChip=this.normalizedAuthorityChipForTab(this.tab,{badge:base.badge,authority:'monitor_only',short:base.next_action,authority_label:base.explanation});
        if(['ok','not_authoritative'].includes(fetchState) || (mode==='dashboard_core'||mode==='playbook_core')){
      ...
      pageAuthorityMode(){
        ...
        if(this.todayUsesBriefFallback()||this.decisionAuthority().source==='fallback_brief') return 'fallback_board';
        const da=this.decisionAuthority();
        const trust=this.today7.trust||{};
        const cs=this.ccState();
        const bs=String(cs.board_decision_state?.state||'').toUpperCase();
        if(this.isWaitDay()||da.degraded||da.gates_active||bs==='SUSPENDED'||bs==='RESEARCH_ONLY'||da.authority_level==='suspended'||da.authority_level==='research'||trust.stale||this.playbookBoardWait()) return 'degraded_board';
        return 'active';
      },
      pageAuthorityIsDegraded(){return this.pageAuthorityMode()!=='active';},
      ...
      portfolioHeaderMode(){return this.tab==='portfolio'||this.pageSurface()==='portfolio_manual';},
      portfolioHeaderCtx(){
      ...
      portfolioHeaderBookLabel(){return this.portfolioHeaderCtx().book_label||'Manual book';},
      portfolioHeaderPositionsLabel(){return this.portfolioHeaderCtx().positions_label||'No positions';},
      ...
      pfLocalStorageKey(){return 'cc_portfolio_local_holdings';},
      ...
      savePortfolioToLocal(holdings){
        try{
          localStorage.setItem(this.pfLocalStorageKey(),JSON.stringify({holdings,updated_at:new Date().toISOString(),source:'manual'}));
        }catch(e){console.warn('portfolio local save failed',e);}
      },
      ...
      surfaceEmptyState(tab,ctx){
        ...
        const loading=!!(c.loading||hints.loading||(tab==='signals'&&(this.rankedOpps.loading||(!this.playbookBoardHasContent()&&this.rankedOpps.refreshing)))||(tab==='scanners'&&this.scannerHub.loading)||(tab==='notrade'&&this.rejectionsPanel.loading));
        if(loading){
          return {kind:'WARMING',headline:'Surface warming',detail:'API still loading — do not treat empty panels as proof of zero ideas.',badge:'WARMING',cta:'Wait or refresh'};
        }
        const failed=!!(c.failed_fetch||hints.failed_fetch||(tab==='signals'&&this.playbookFetchFailed())||(tab==='scanners'&&this.scannerHub.error&&!this.scannerDiscoveryHasFallbackRows())||(tab==='notrade'&&this.rejectionsFetchFailed()));
        if(failed){
          const err=c.error||hints.error||this.scannerHub.error||'';
          const rejectDetail=tab==='notrade'&&!err?'Fresh rejection data unavailable; no blocked signals shown':((err?String(err)+' — ':'')+'Retry when the badge clears before sizing or handoff.');
          return {kind:'FETCH_FAILED',headline:tab==='notrade'?'Rejections unavailable':'Fetch failed',detail:rejectDetail,badge:'FETCH FAILED',cta:'Retry fetch'};
        }
        const waitOk=!!(c.wait_day_ok||(tab==='signals'&&this.playbookBoardWait())||(tab==='scanners'&&this.discoveryWaitZeroHits())||(tab==='today'&&this.isWaitDay()&&this.dashboardHasNoBoardData()));
        if(waitOk){
          return {kind:'WAIT_DAY_OK',headline:'Empty board — often correct on WAIT',detail:tab==='scanners'?this.discoveryWaitEmptyLine():this.playbookEmptyComment(),badge:'WAIT DAY OK',cta:'Review monitor funnel'};
        }
        return {kind:'NO_DATA',headline:'No rows in this batch',detail:c.detail||(tab==='signals'?this.playbookEmptyComment():(tab==='notrade'?'No blocked signals in current pipeline batch.':'Nothing to show yet.')),badge:'NO DATA',cta:'Refresh'};
      },
      playbookEmptyState(){
        const es=this.surfaceEmptyState('signals',{});
        return {kind:es.kind,badge:es.badge,title:es.headline,detail:es.detail,cta:es.cta||'🔄 Refresh playbook'};
      },
      discoveryEmptyState(){return this.surfaceEmptyState('scanners',{});},
      dashboardEmptyState(){return this.surfaceEmptyState('today',{wait_day_ok:this.isWaitDay()&&this.dashboardHasNoBoardData()});},
      rejectionsEmptyState(){return this.surfaceEmptyState('notrade',{empty:!((this.rejectionsPanel.data?.no_trade_signals||[]).length)});},
      dismissDataContract(){
        this.dataContractDismissed=true;
        try{localStorage.setItem('cc_data_contract_dismissed','1');}catch(e){}
      },
      ...
      severityBadgeClass(state){
        if(typeof CCHelpers!=='undefined'&&CCHelpers.severityBadgeClass) return CCHelpers.severityBadgeClass(state);
      ...
      surfaceWarmupLoadingLine(surfaceMode){
        ...
        if(typeof CCHelpers!=='undefined'&&CCHelpers.surfaceWarmupLoadingLine) return CCHelpers.surfaceWarmupLoadingLine(mode);
      ...
      dataContractFetchStateKey(){
        ...
        const map={'FETCH FAILED':'failed_fetch','EXEC BLOCKED':'execution_blocked','STALE':'stale','FALLBACK':'fallback','LOADING':'loading','PARTIAL':'partial','MOCK ONLY':'mock_only','RESEARCH ONLY':'research_only','NO DATA':'no_data','NOT AUTHORITATIVE':'not_authoritative','PROBE ONLY':'probe_only','RUNTIME UNKNOWN':'runtime_unknown'};
        return map[f]||'ok';
      },
      ...
      todayMissionMonitorsColumnHint(){
        ...
        if(watch>0) return watch+' watch-qualified on funnel — mission tickers are attention queue, not extra KPI count';
        if(opts.monitorCount>0) return 'Fallback monitors — scan / near-miss queue; filter_funnel is authority for watch-qualified';
        return opts.waitDay
      ...
      todayMissionQuantClusterLines(){
        ...
        return hints.slice(0,3).map(h=>{
          ...
          return label?(label+(detail&&detail.length<=72?' — '+detail:' — 只供 Monitor，唔可 Deploy')):'';
        }).filter(Boolean);
      },
      todayBoardHeroSynthesisLine(){
        ...
        if(h0){
          ...
          if(label) parts.push(label+(detail&&detail.length<=48?' ('+detail+')':detail?' — monitor context':''));
        }
      ...
      ibkrLoginToReadyHint(){
        ...
        if(short==='OFFLINE'||st.level==='offline') return 'IBKR OFFLINE — start Gateway/TWS and confirm API port; Connect on IBKR tab when reachable';
        if(short==='LOGIN'||(st.gw&&!st.connected)) return 'IBKR LOGIN — connect session on IBKR tab; READY required before handoff (bracket aligned)';
        if(st.handoff||st.level==='ready') return 'IBKR READY — handoff path verified; confirm bracket alignment before transmit';
      ...
      warmupStatusLine(){
        ...
        const opts={
          healthMode:mode,
          apiReachable:!offline&&!/failed to fetch|networkerror|econnrefused/i.test(String(this.apiError||'')),
          instantDegraded:!!(this.today7.instant_degraded||this.rankedOpps.instant_degraded||this.pageAuthorityIsDegraded()||this.todayUsesBriefFallback()),
          fetchFailed:!!(this.surfaceFetchHints[this.tab]?.failed_fetch),
        };
        ...
        if(mode==='loading'||this.cc_status.mode==='LOADING') return 'WARMING — backend importing modules · brief/monitor queue only until full';
        if(opts.instantDegraded) return 'DEGRADED — instant snapshot · council/scanner may disagree until live ranked loads';
        if(mode==='full') return '';
      ...
      warmupUpgradeQueue(){
        ...
        if(typeof CCHelpers!=='undefined'&&CCHelpers.warmupUpgradeQueue) return CCHelpers.warmupUpgradeQueue(opts);
      ...
      todayMissionBlockersTitle(){
        ...
        return opts.waitDay&&opts.hasSystem?'System blockers · gate flags':(opts.waitDay?'Gate flags':(opts.hasSystem?'System blockers':'Blockers'));
      },
      ...
      todayMissionPanel(){
        ...
        const cardGates=[];
        (td.risk_blockers||[]).forEach(rb=>{const s=String(rb).trim();if(s&&!cardGates.includes(s))cardGates.push(s);});
        (td.why_not_aggressive||[]).slice(0,2).forEach(w=>{const s=String(w).trim();if(s&&!cardGates.includes(s))cardGates.push(s);});
        const system_blockers=this.todayMissionSystemBlockersList();
        const blockers=[];
        system_blockers.forEach(s=>{if(s&&!blockers.includes(s))blockers.push(s);});
        cardGates.forEach(s=>{if(s&&!blockers.includes(s))blockers.push(s);});
        const monitors=[];
        const reject=new Set(['AVOID','NO_TRADE','BLOCKED']);
        const add=t=>{const x=String(t||'').toUpperCase().replace(/[^A-Z0-9.^]/g,'');if(x&&x.length<=8&&!monitors.includes(x))monitors.push(x);};
        const fromApi=(this.today7.dashboard_monitors||[]).filter(Boolean);
        if(fromApi.length){
          fromApi.forEach(add);
        }else{
          (this.today7.near_miss||[]).forEach(nm=>{if(!reject.has(String(nm.action||'').toUpperCase()))add(nm.ticker);});
          (this.today7.top_ranked||[]).forEach(r=>{if(!reject.has(String(r.action||'').toUpperCase()))add(r.ticker);});
      ...
      headerChipGroups(){
        ...
        const opts={
          authorityChip:this.pageAuthorityIsDegraded()?(this.instantDegradedBannerVisible()?'INSTANT DEGRADED':'MONITOR ONLY'):'',
          tradeability:this.canonicalTradeability(),
        ...
        return {primary:(hs.chips||[]).filter(c=>c.class==='deploy'||c.class==='idea'),secondary:(hs.chips||[]).filter(c=>c.class!=='deploy'&&c.class!=='idea'),infra:[]};
      },
      opsRecoveryGuide(){
        const mode=String(this.healthMode||this.healthData?.mode||'').toLowerCase();
        const retry=[];
        ...
        if(mode==='loading'){
          ...
          retry.push('Refresh Ops health · Error log · Updates panels');
        if(!this.ops.running)
          retry.push('Start engine (Ops health) or set CC_AUTO_START_ENGINE=1');
        if(!this.ops.running)
          blocks.push('No engine cycle — Today/Signals may be precomputed only');
        if(this.cc_status.breaker)
          blocks.push('Risk breaker ON — blocks new entries until cleared');
        const st=this.ibkrStateFrom(this.today7.execution_readiness||{});
      ...
      ibkrApplyStatusPayload(d){
        ...
        this.ibkr.host=(window.CCHelpers&&CCHelpers.ibkrSyncHostFromStatus)
      ...
      ibkrApplyStatusFetchFailure(httpStatus,rawMsg){
        ...
        const warming=httpStatus===503||/warming up|API warming|still loading|backend importing|mode=full/i.test(msg);
        let line=isNet
          ...
          :warming
            ...
            :('Status fetch failed'+(httpStatus?(' (HTTP '+httpStatus+')'):'')+(msg?': '+msg.slice(0,140):''));
      ...
      playbookBridgeRowsFromTop5(rows){
        return (rows||[]).filter(r=>r&&r.ticker).map((r,i)=>({
          ticker:r.ticker,
          ...
          why_now:Array.isArray(r.why_now)?r.why_now.join(' · '):(r.why_now||'Brief fallback bridge — monitor only'),
      ...
      playbookBridgeFromCaches(){
        ...
        try{
          ...
          if(today&&(today.top_5||[]).length)
            return this.playbookBridgeRowsFromTop5(today.top_5);
        }catch(e){}
        return [];
      },
      playbookOppsFallbackRows(){
        ...
        return (this.opps||[]).filter(r=>{
          ...
          return ['FALLBACK WATCH','FALLBACK CANDIDATE','WATCH ONLY','WATCH','PILOT','REFERENCE ONLY','RESEARCH ONLY'].includes(g);
        });
      },
      playbookOppsFallbackVisible(){
        return this.rankedOpps.rows.length===0&&this.playbookOppsFallbackRows().length>0;
      },
      gradedLegacyOpps(){return this.playbookOppsFallbackRows();},
      playbookFetchFailed(){
      ...
      formatFallbackScore(row){
        ...
        const isFallback=mode==='fallback_rank'||source.includes('brief')||status==='monitor'||r.strategy==='brief';
        if(!isFallback){
      ...
      topMonitorCandidates(limit=3){
        const tickers=[];
        const seen=new Set();
        const add=(t)=>{const x=String(t||'').toUpperCase().replace(/[^A-Z0-9.^]/g,'');if(!x||x.length>8||seen.has(x))return;seen.add(x);tickers.push(x);};
      ...
      dashboardBestTradeLabel(){
        const td=this.today7.todays_decision;
        if(!td||(!td.can_deploy_today||this.isWaitDay())) return td?.best_trade?.ticker?'Top watch':'Best monitor';
        return 'Best TRADE';
      },
      ...
      instantDegradedBannerLine(){
        ...
        if(this.today7.instant_degraded||this.rankedOpps.instant_degraded||src.includes('instant-degraded')||src==='brief-fallback') return 'Instant degraded — snapshot only · not suitable for sizing or IBKR handoff';
        if(this.healthData&&this.healthData.mode==='loading') return 'Backend warming — brief/monitor queue until health mode is full';
        if(this.cc_status.mode==='LOADING'||this.ccHeader.page_authority_mode==='diagnostic') return 'Backend warming — refresh when health mode is full';
        return '';
      },
      instantDegradedBannerHint(){
        ...
        if(this.healthData&&this.healthData.mode==='loading')
          return 'Wait for /health mode=full before sizing or IBKR handoff · uptime '+Math.round(this.healthData.uptime_seconds||0)+'s';
      ...
      dismissInstantDegradedBanner(){
        ...
        try{sessionStorage.setItem('cc_instant_degraded_dismissed','1');}catch(e){}
      },
      captureInstantDegradedBanner(data){
        ...
        if(data.degraded||data.instant_degraded||String(data.trust?.source||'').includes('instant-degraded')||data.source==='instant-degraded'){
          if(!this.instantDegradedBanner){
            this.instantDegradedBanner=String(data.trust?.reason||data.board_message||data.narrative||'Backend importing — snapshot only').slice(0,220);
          }
        }
      ...
      async fetchWarmupBriefBoard(){
        try{
          ...
          if(r.ok){
            ...
            if((d.opportunities||[]).length){
              ...
              if(!(this.today7.top_ranked||[]).length)
                this.today7.top_ranked=this.playbookBridgeRowsFromTop5((d.opportunities||[]).slice(0,5));
              this.rankedOpps.loading=false;
              return;
            }
          }
          const sr=await fetch('/api/v7/playbook/ranked/snapshot?limit=30',{signal:AbortSignal.timeout(5000)});
          if(sr.ok){
        ...
        }catch(e){console.warn('warmup brief-board',e)}
        finally{
      ...
      async fetchSelfLearnStatus(){
        ...
        }catch(e){console.warn('fetchSelfLearnStatus',e);}
      },
      ...
      todayUsesBriefFallback(){
        ...
        return String(t.source||'').toLowerCase().includes('brief')||String(t.reason||'').toLowerCase().includes('fallback');
      },
      authorityDegradedBanner(){
        ...
        const parts=[dc.system_mode==='degraded'?'System mode: degraded':null,dc.decision_authority_line||null,stale||dc.fallback_board_line||null].filter(Boolean);
        return parts.join(' · ');
      },
      staleSnapshotContext(){
        const t=this.today7.trust||{};
        const da=this.decisionAuthority();
        const stale=!!(t.stale||da.source==='stale_cache'||this.freshness?.worst_tier==='STALE'||this.freshness?.worst_tier==='CRITICAL');
        ...
        const parts=['Fallback board','Historical snapshot only','Not suitable for execution decisions','Refresh required for decision use'];
      ...
      dossierActionsBlockedTitle(){
        ...
        if(this.dossierSizingBlockedReason()==='partial'||this.dossierSizingBlockedReason()==='failed') return 'Research incomplete — actions blocked';
      ...
      dossierSizingBlockedReason(){
        ...
        if(intel?.load_phase==='core'||intel?.partial||this._dossierIntelDegraded(intel)||this.dos.status==='partial_loaded')return 'partial';
      ...
      dossierResearchOnly(){
        if(this.dos.loading||this.dos.status==='loading_core'||this.dos.status==='loading_enrichments') return false;
      ...
      dosLevelColumnLabel(field){
        if(!this.dossierResearchOnly()) return field==='entry'?'Entry zone':field==='stop'?'Stop':field==='target'?'T1 / T2':field==='size'?'Size @1%':'R:R';
      ...
      cardIsFallbackRow(opp){
        if(!opp) return false;
        if(opp.card_display_mode==='reference_only'||opp.levels_indicative_only||opp.confidence_fallback_only||opp.deploy_authority===false) return true;
        if(this.todayUsesBriefFallback()||this.playbookUsesBriefFallback()) return true;
      ...
      cardShowsExecutableSizing(opp){
        ...
        if(this.pageAuthorityIsDegraded()||this.todayUsesBriefFallback()||this.cardIsFallbackRow(opp)) return false;
        const g=String(this.cardExecutionGrade(opp)||'').toUpperCase();
        return ['TRADE','BUY','BUY_ON_DIP','STRONG_TRADE'].includes(g);
      },
      cardSizingPct(opp){
        if(!this.cardShowsExecutableSizing(opp)) return '';
        return Math.min(10,Math.max(0.1,(0.01/(Math.abs(opp.entry_price-opp.stop_price)/opp.entry_price)*100))).toFixed(1)+'%';
      },
      cardSizingNote(opp){
        if(!opp) return '';
        if(this.pageAuthorityIsDegraded()||this.todayUsesBriefFallback()||this.cardIsFallbackRow(opp)) return 'Sizing suspended in fallback mode';
        if(opp.entry_price&&opp.stop_price&&opp.entry_price>opp.stop_price&&!this.cardShowsExecutableSizing(opp)) return 'Indicative sizing only';
        return '';
      },
      cardReferenceBanner(opp){
        if(this.pageAuthorityIsDegraded()||this.isWaitDay()||this.playbookBoardWait()||this.cardIsFallbackRow(opp)||!this.today7.todays_decision?.can_deploy_today) return '只可監察 · monitor only — 未開 deploy authority';
        return '';
      },
      cardExecutionGrade(opp){
        ...
        const gates={
          ...
          data_stale:!!(this.ccFreshnessState().worst_tier&&['STALE','CRITICAL'].includes(this.ccFreshnessState().worst_tier))||!!(this.today7.trust?.stale||da.gates?.data_stale),
          broker_offline:['GATEWAY_DOWN','IBAPI_MISSING','SESSION_INACTIVE'].includes(es)||this.dashboardBrokerOffline(),
          exec_blocked:es==='EXEC_BLOCKED'||!!(this.cc_status?.breaker||da.gates?.exec_blocked),
          handoff_blocked:es==='HANDOFF_BLOCKED',
          fallback_brief:this.todayUsesBriefFallback()||da.source==='fallback_brief'||!!da.gates?.fallback_brief,
        ...
        if((opp.effective_grade||opp.effective_action)&&srv&&srv!==raw) return opp.effective_grade||opp.effective_action;
        if(gates.fallback_brief&&tradeActions.includes(raw)) return 'FALLBACK WATCH';
        if(raw==='PILOT'&&(gatesActive||this.playbookBoardWait()||!da.allows_trade_labels)) return 'PILOT (MONITOR)';
        return opp.effective_action||opp.action||'—';
      },
      effectiveCardAction(opp){return this.cardExecutionGrade(opp);},
      ...
      effectiveCardActionClass(opp){
        ...
        if(['FALLBACK CANDIDATE','FALLBACK WATCH','INCOMPLETE','NOT EXECUTION-GRADE','RESEARCH ONLY','REFERENCE ONLY','WATCH ONLY'].includes(a)) return 'pa';
      ...
      confidenceFinal(opp){
        ...
        const comps=[cb?.thesis??opp?.thesis_conf??0,cb?.timing??opp?.timing_conf??0,cb?.execution??opp?.exec_conf??0,cb?.data??opp?.data_conf??0];
      ...
      confidenceFinalLabel(opp){
        ...
        if(v==null||Number.isNaN(v)) return this.confidenceIsFallback(opp)?'':(opp?.confidence_fallback_only||this.cardIsFallbackRow(opp)?'Fallback only':'Unavailable');
      ...
      confidenceComponents(opp){
        ...
        if(this.confidenceFinal(opp)==null&&(opp?.confidence_unavailable||this.cardIsFallbackRow(opp))){
          return [['Thesis',null],['Timing',null],['Exec',null],['Data',null]];
        }
        return [['Thesis',cb.thesis??opp?.thesis_conf??0],['Timing',cb.timing??opp?.timing_conf??0],['Exec',cb.execution??opp?.exec_conf??0],['Data',cb.data??opp?.data_conf??0]];
      },
      topRankedIsTrade(){
      ...
      topRankedHeroLabel(){
        const td=this.today7.todays_decision;
        if(td&&td.hero_label&&!(this.pageAuthorityIsDegraded()||this.isWaitDay()||this.todayUsesBriefFallback())) return td.hero_label;
        if(this.pageAuthorityIsDegraded()||this.isWaitDay()||this.todayUsesBriefFallback()||this.playbookBoardWait()) return '#1 監察候選 · monitor candidate';
        const r=(this.today7.top_ranked||[])[0];
        if(!r) return '';
        const act=String(this.effectiveCardAction(r)||'').toUpperCase();
      ...
      dashboardOperatorNowLine(){
        const td=this.today7.todays_decision;
        if(this.pageAuthorityIsDegraded()) return 'Now · '+(this.canonicalTradeability()||'WAIT')+' · 只可 Monitor';
      ...
      dashboardOperatorNextActionLine(){
        if(this.pageAuthorityIsDegraded()) return '下一步 · 先修 broker / runtime / board blocker，再回 Dashboard / Playbook 複核。';
        const td=this.today7.todays_decision;
      ...
      playbookOperatorNowLine(){
        if(this.pageAuthorityIsDegraded()||this.playbookBoardWait()) return 'Now · '+(this.canonicalTradeability()||'WAIT')+' · 只可 Monitor';
      ...
      dayStateLabel(){const ds=(this.today7.todays_decision&&this.today7.todays_decision.day_state)||'';if(ds==='A_GRADE_TRADE_DAY')return 'A 級部署日 · A-grade trade day';if(ds==='TRADE_DAY')return '部署日 · Trade day';if(ds==='PILOT_WATCH_DAY')return '觀察日 · Monitor day';if(ds==='NO_TRADE_DAY')return '停手日 · No-trade day';const tb=(this.today7.tradeability||'').replace(/_/g,' ');return tb||'—';},
      ibkrStateFrom(ex,cc){
        const exObj=ex||{};
        const ccObj=cc||this.cc_status||{};
        const diag=exObj.diagnosis||this.ibkr.diagnosis||{};
        if(diag.short&&diag.label) return {short:diag.short,label:diag.label,level:diag.short==='READY'?'ready':(diag.short==='OFFLINE'||diag.short==='NO IBAPI')?'offline':'partial',sub:exObj.sub_status||{},connected:!!(exObj.session_usable||exObj.broker_connected||exObj.ibkr_connected||this.ibkr.session_usable),gw:!!(exObj.gateway_reachable||diag.gateway_reachable||ccObj.ibkr_gateway),handoff:!!exObj.trade_handoff_ready,monitoring:!!(exObj.monitoring_only||this.ibkr.monitoring_only),health:exObj.health||this.ibkr.health||{},hint:diag.hint||''};
        if(exObj.unified_short) return {short:exObj.unified_short,label:exObj.unified_label||exObj.health_label||exObj.unified_short,level:exObj.level||'partial',sub:exObj.sub_status||{},connected:!!(exObj.session_usable||exObj.broker_connected||exObj.ibkr_connected),gw:!!exObj.gateway_reachable,handoff:!!exObj.trade_handoff_ready,monitoring:!!exObj.monitoring_only,health:exObj.health||{}};
        const gw=exObj.gateway_reachable!=null?!!exObj.gateway_reachable:!!(ccObj.ibkr_gateway||this.ibkr.gateway_reachable);
        const connected=!!(exObj.session_usable||exObj.broker_connected||exObj.ibkr_connected||ccObj.ibkr_connected);
        ...
        let short='OFFLINE', label='BROKER OFFLINE', level='offline';
        if(breaker){short='BLOCKED';label='CIRCUIT BREAKER';level='blocked';}
        else if(handoff){short=mode.toUpperCase();label=mode.toUpperCase()+' · HANDOFF READY';level='ready';}
        else if(connected&&monitoring){short='MONITOR';label=healthLabel||mode.toUpperCase()+' · MONITOR / MANUAL';level='partial';}
        else if(connected){short='PARTIAL';label=healthLabel||mode.toUpperCase()+' · PARTIALLY READY';level='partial';}
        else if(gw){short='LOGIN';label='GATEWAY UP · LOGIN REQUIRED';level='partial';}
        return {short,label,level,sub:{broker_transport:gw?'up':'down',session_auth:connected?'active':'inactive',engine:engine?'on':'off',handoff_readiness:handoff?'ready':(connected?'monitoring':'blocked'),bracket_readiness:bracket?'ready':'draft'},connected,gw,handoff,monitoring,health:exObj.health||{}};
      },
      ibkrPillStyle(){
        const st=this.ibkrStateFrom(this.today7.execution_readiness||{});
        ...
        if(st.level==='ready') return mode==='live'?'background:#7f1d1d;color:#fca5a5;border:1px solid #ef4444':'background:#1e3a5f;color:#93c5fd;border:1px solid #3b82f6';
      ...
      ibkrTrustStripLabel(){
        ...
        if(this.ibkr.diagnostics&&this.ibkr.diagnostics.health&&this.ibkr.diagnostics.health.summary_label) return this.ibkr.diagnostics.health.summary_label;
        const st=this.ibkrStateFrom(this.today7.execution_readiness||{});
      ...
      ibkrAccountLoadedLabel(){
        ...
        const loaded=!!(this.ibkr.readiness?.critical_rows||[]).find(r=>r.key==='account'&&r.state==='ready')||this.ibkr.health?.account_status==='ok';
      ...
      ibkrCriticalRows(){
        return this.ibkr.readiness?.critical_rows||(this.ibkr.readiness?.rows||[]).filter(r=>r.category==='critical');
      },
      ibkrWorkflowRows(){
        return this.ibkr.readiness?.workflow_rows||(this.ibkr.readiness?.rows||[]).filter(r=>r.category==='workflow');
      },
      ibkrFarmRows(){
        return this.ibkr.readiness?.farm_rows||(this.ibkr.readiness?.rows||[]).filter(r=>r.category==='farm');
      },
      ibkrReadinessScore(){
      ...
      ibkrRepairChecklist(){
        ...
        const port=Number((this.ibkr.diagnostics&&this.ibkr.diagnostics.port)||(mode==='paper'?7497:4001))||0;
        ...
        const accountReady=!!(this.ibkr.health&&this.ibkr.health.account_status==='ok')||this.ibkrAccountLoadedLabel().startsWith('Account loaded');
        const syncReady=String(this.ibkr.readiness?.portfolio_sync_status||'').toLowerCase()==='ready'&&!this.ibkrBookMismatch();
        ...
        const bracketDetail=(typeof CCHelpers!=='undefined'&&CCHelpers.localizeIbkrBracketReason)
        ...
        return [
          ...
          },
          {
            ...
            detail:'目前 '+host+':'+port+' · '+(code==='wrong_host_docker'?'Docker 請改用 host.docker.internal。':code==='mode_port_mismatch'?'paper/live mode 與 port 唔一致。':'host / port 設定可用。'),
          },
          {
            key:'session',
          ...
          },
          {
            ...
            detail:syncReady?'Broker positions / orders 已對齊。':(this.ibkrBookMismatch()?'Broker truth 與 local book 未對齊。':'仍需刷新 positions / orders。'),
          },
          {
            key:'handoff',
            ...
            detail:(handoffReady&&bracketReady)
      ...
      ibkrRepairNextActionLine(){
        ...
        return next?('下一步 · '+next.label.replace(/^\d+\.\s*/,'')+'。'):'下一步 · 回到 Playbook / Dashboard 確認 authority 同 execution gates。';
      },
      ibkrBookMismatch(){
      ...
      ibkrUnifiedShort(ex){return this.ibkrStateFrom(ex).short;},
      ibkrUnifiedLabel(ex){return this.ibkrStateFrom(ex).label;},
      ...
      discoveryPrimaryBlockerLine(){
        const d=this.scannerHub.data||{};
        if(this.scannerDiscoveryHasFallbackRows()) return '即時掃描失敗 · 目前顯示 fallback samples；Discovery 仍屬 research-only，先到 Playbook 確認。';
        ...
        if(d&&(d.hub_status==='warming'||d.hub_status==='degraded')) return '目前只有 cached / brief rows；Discovery 仍屬 research-only，待 Playbook 確認。';
      ...
      discoveryPrimaryNextActionLine(){
        if(this.discoveryShowCachedLeaders()) return '下一步 · 用 cached leaders 做 research；Playbook 驗證後才可 monitor upgrade。';
        if(this.scannerDiscoveryHasFallbackRows()||this.scannerHub.error) return '下一步 · Retry scanner；若仍失敗，先開 Playbook / Dashboard 驗證 board gate。';
        if(this.scannerHub.data?.diagnostics?.reason_no_hits) return '下一步 · 保持監察；WAIT / NO_TRADE 日出現零命中屬正常。';
        return '下一步 · 用 Playbook 驗證 blocker、upgrade trigger，同埋 execution readiness。';
      },
      scannerHubHeaderNote(){
        if(this.scannerDiscoveryHasFallbackRows()){
          return 'Discovery 只係 scanner funnel。盤面 '+this.canonicalRegimeLine()+' · 下方 rows 係 fallback samples，唔係即時掃描輸出。';
        }
        const d=this.scannerHub.data||{};
        if(d.research_note) return d.research_note;
        return 'Discovery 只係 scanner funnel。盤面 '+this.canonicalRegimeLine()+' · WAIT / NO_TRADE 日出現零命中屬正常。';
      },
      scannerDiscoveryEmptyMessage(){
        if(this.discoveryShowCachedLeaders()) return 'Live scan unavailable — cached leaders below (research only).';
        if(this.scannerDiscoveryHasFallbackRows()) return 'Fallback watchlist samples visible — see cached leaders.';
        if(this.scannerHub.error) return 'Scanner hub could not load: '+this.scannerHub.error+'. Retry or open Dashboard for board posture.';
      ...
      discoveryPromotionPack(h){
        ...
        if(['GATEWAY_DOWN','IBAPI_MISSING','SESSION_INACTIVE'].includes(es)) missing.push('broker '+es.replace(/_/g,' ').toLowerCase());
      ...
      kpiScannedCount(){
        ...
        if(f&&(f.universe_scanned!=null||f.universe!=null)) return Number(f.universe_scanned??f.universe)||0;
      ...
      marketStripStaleDowngrade(){
        ...
        return src.includes('snapshot')||src.includes('disk')||src.includes('cache')||src.includes('pulse')||src.includes('brief')||this._marketDataSnapshotOld(asOf);
      },
      marketStripStaleDowngradeLines(){
        const lines=this.decisionAuthority().degraded_copy?.stale_snapshot_lines;
        const base=Array.isArray(lines)&&lines.length
          ?lines.slice()
          :['Historical snapshot only','Not suitable for execution decisions','Refresh required for decision use'];
      ...
      marketPulseIndices(){
        ...
        const rows=Array.isArray(pulse.indices)&&pulse.indices.length
          ?pulse.indices
          :(Array.isArray(this.indices)?this.indices:[]);
        return rows
          .map(ix=>this.normalizeMarketItem(ix))
          .filter(ix=>ix&&(ix.symbol||ix.name));
      },
      ...
      playbookVisibleRows(){
        ...
        const focus=String(this.playbookFocusTicker||'').toUpperCase().trim();
      ...
      playbookCardGateLine(r){
        ...
        detail=detail.replace(/^(AVOID|BLOCKED|WATCH|NO_TRADE|PILOT|FALLBACK WATCH)\s*[·\-\—:\u2014]\s*/i,'').trim();
      ...
      playbookCardMonitorLine(r){ return this.playbookCardGateLine(r); },
      ...
      playbookRejectedDisplayRows(){
        ...
        return (this.rankedOpps.rows||[]).filter(r=>['AVOID','NO_TRADE'].includes(String(r.action||'').toUpperCase()));
      },
      playbookNoValidMonitors(){
        return this.playbookBoardHasContent()
      ...
      derivePlaybookNearMiss(opps){
        const rows=opps||[];
        const reject=new Set(['AVOID','NO_TRADE','BLOCKED']);
        const watch=rows.filter(x=>!reject.has(String(x.action||'').toUpperCase())&&['WATCH','PILOT'].includes(String(x.action||'').toUpperCase())&&(x.score||0)>=6).slice(0,8);
        if(watch.length) return watch;
        return [...rows].filter(r=>!reject.has(String(r.action||'').toUpperCase())).sort((a,b)=>(b.score||0)-(a.score||0)).filter(r=>(r.score||0)>=4).slice(0,3).map(r=>({
          ...r,
          action:'WATCH',
          whats_missing:r.whats_missing||'Ranked by scan but blocked by deploy gates — monitor for upgrade',
        }));
      },
      playbookBoardHasContent(){
        return this.playbookVisibleRows().length>0
      ...
      playbookEmptyComment(){
        if(this.playbookFetchFailed()){
          return (this.rankedOpps.emergency?.detail||this.rankedOpps.board_explanation||'Live ranked fetch failed — not an empty market day.');
        }
      ...
      dashboardWaitTopComment(compact){
        let s='WAIT · 今日唔做 full deploy。盤面只產生 watch-quality names，暫時冇名稱同時通過 thesis、timing、R:R 同 execution。今日應以監察為主，唔好強行出手。';
        if(this.dashboardBrokerOffline()){
          s+=' Broker offline，execution 暫不可用；目前未有 setup 可升級到 deploy。';
        }else{
          s+=' 目前未有 setup 可升級到 deploy。';
        }
        if(compact) return 'WAIT · 先 Monitor，唔好強行做。今日唔做 full deploy'+(this.dashboardBrokerOffline()?'；broker offline。':'。');
        return s;
      },
      dashboardWhyNotDeployComment(){
        ...
        return 'The issue is not idea scarcity — it is quality scarcity. '+scannedN+' names were scanned, '+watchN+' reached watch-qualified status, but '+deployN+' passed the deploy gate because timing, R:R, and execution readiness remain incomplete. In this state, the system should prefer patience over participation.';
      },
      ...
      playbookUsesBriefFallback(){
        ...
        if(src==='brief'||src.includes('brief')||src.includes('fallback')||src==='compressed_fallback') return true;
      ...
      playbookWatchQueueKeys(){
        return ['near_trigger','early_setup','needs_volume','needs_thesis','needs_timing','needs_broker_only','needs_data_refresh','dead_remove'];
      },
      playbookWatchQueueLabel(key){
        const m={near_trigger:'Near trigger',early_setup:'Early setup',needs_volume:'Needs volume',needs_thesis:'Needs thesis',needs_timing:'Needs timing',needs_broker_only:'Needs broker',needs_data_refresh:'Needs data refresh',dead_remove:'Dead / remove'};
        return m[key]||key;
      },
      ...
      playbookHasTradeCards(){
        return (this.rankedOpps.rows||[]).some(r=>['TRADE','BUY','BUY_ON_DIP','TRADE_NOW','STRONG_TRADE'].includes(String(r.action||'').toUpperCase()));
      },
      playbookExecutionGateOverrideActive(){
        if(!this.playbookBoardWait()||!this.playbookHasTradeCards()) return false;
        return this.playbookUsesBriefFallback()||this.playbookBrokerOffline()||this.playbookDeployableCount()<1;
      },
      ...
      playbookWhatToMonitorLine(){
        ...
        if(opts.nearMissCount) parts.push(opts.nearMissCount+' near-miss row'+(opts.nearMissCount===1?'':'s'));
        parts.push('deploy unlock checklist below');
      ...
      playbookBestActionDisplay(){
        if(this.playbookBoardWait()&&(this.playbookBrokerOffline()||this.playbookDeployableCount()<1||this.playbookUsesBriefFallback())){
      ...
      playbookDecisionConfidenceDisplay(){
        const ba=this.rankedOpps.bestAction;
        if(this.playbookBoardWait()&&(this.playbookUsesBriefFallback()||ba?.data_freshness==='STALE'||String(ba?.evidence_quality||'').toLowerCase()==='low')){
      ...
      playbookNearMissMissing(nm){
        ...
        if(raw&&raw!=='At gate — confirm volume and R:R'&&raw!=='At gate — review sizing') return 'Missing: '+raw;
      ...
      playbookNearMissUpgradeLine(){
        ...
        return rows.map(nm=>{
          const tk=nm.ticker||'—';
          const trig=nm.upgrade_trigger||nm.whats_missing||this.playbookNearMissMissing(nm).replace(/^Missing:\s*/,'');
      ...
      eventRiskKey(r){return this.eventRiskTicker(r)||this.eventRiskLabel(r)||String(r);},
      eventRiskTicker(r){
        if(!r) return '';
      ...
      openEventRiskDossier(r){
        const tk=this.eventRiskTicker(r);
        if(tk){this.openDossier(tk);return;}
      ...
      playbookScoreReconciliationDetail(){
        ...
        return rows.length?('Divergent: '+rows.map(r=>r.ticker).filter(Boolean).join(', ')):'';
      },
      rowScoreFamiliesDiverge(r){
        ...
        if(scanner==null&&r.score_card?.families?.board_investability) scanner=r.score_card.families.board_investability.raw_scanner;
      ...
      flowPlaybookCrossBadge(ticker){
        ...
        const pools=[d.actionable_top3,d.watch_for_confirm,d.live_flow,d.best_bullish_flow,d.best_bearish_flow].filter(Boolean);
      ...
      trackRecordGateLine(){
        ...
        if(this.today7.ai_narrative||this.today7.ai_loading) return 'NO TRACK RECORD — model output not live-track-recorded';
        if((this.today7.top_ranked||[]).some(r=>r.ml_confidence!=null&&!r.live_track_record)) return 'NO TRACK RECORD — model output not live-track-recorded';
        return '';
      },
      ...
      playbookSymbolComment(r,idx){
        ...
        if(topWatch&&tk===topWatch){
          return sym+' is the best watch on the board, not the best trade on the board. It ranks near the top on relative priority, but still lacks the clean R:R, thesis strength, and leadership profile needed for deployment.';
        }
        return 'Top watch, not top trade. '+sym+' leads a weak board, but still lacks the clean R:R, thesis strength, and leadership profile needed for deployment.';
      },
      ...
      btLabPlainSummary(){
        ...
        if(d.degraded||d.research_only) return 'Backend still warming — no usable walk-forward evidence yet. This page is diagnostic only.';
        const v=String(d.walk_forward?.verdict||'').toLowerCase();
        if(v==='insufficient_data'||v==='skipped') return 'No usable walk-forward evidence yet. This page is diagnostic only.';
        if(v==='unstable') return 'Walk-forward is unstable — treat results as diagnostic, not deploy evidence.';
        if(v==='stable_across_windows') return 'Walk-forward looks consistent across windows — still backtest-only, not live validation.';
      ...
      btLabEvidenceLine(){
        ...
        const v=String(d.walk_forward?.verdict||'').toLowerCase();
        if(v==='insufficient_data') return 'Insufficient sample depth — too few trades or windows for reliable attribution.';
        if(v==='unstable') return 'Evidence quality is weak — performance inconsistent across windows with low trade count.';
        if(v==='skipped') return 'Walk-forward skipped — core backtest only, no cross-window stability check.';
        if(this.btLabAttributionLowValue()) return 'Attribution has low informational value — insufficient trade count or incomplete replay coverage.';
        if(v==='stable_across_windows') return 'Moderate evidence — windows show positive returns, but backtest ≠ live track record.';
      ...
      btLabActionLine(){
        ...
        const v=String(d.walk_forward?.verdict||'').toLowerCase();
        if(v==='insufficient_data') return 'Reduce trust in displayed metrics; try a longer period or wait for more trade history.';
        if(v==='unstable') return 'Do not use attractive segments to justify live capital — diagnose robustness only.';
        if(v==='skipped') return 'Enable walk-forward to compare stability across windows before trusting the backtest.';
        if(v==='stable_across_windows') return 'Use for hypothesis validation only — live validation required before sizing.';
      ...
      btLabAttributionLowValue(){
        ...
        const allZero=ranked.every(s=>!(Number(s.return_pct)||Number(s.sharpe)||Number(s.total_trades)||Number(s.trades)));
        const reviewEmpty=!(Number(review.trade_count)||(review.best_trades||[]).length);
        return allZero||reviewEmpty;
      },
      aiCommentaryWarranted(){const td=this.today7.todays_decision;const dm=this.today7.decision_model;if((td?.risk_blockers||[]).length)return true;if((td?.why_not_aggressive||[]).length>=2)return true;if(dm&&dm.macro_regime==='Hostile'&&dm.opportunity_quality!=='Weak')return true;if((this.today7.event_risks||[]).length>=2)return true;if(this.today7.overlap_warning&&this.today7.overlap_warning.level==='high')return true;return false;},
      ...
      async fetchAIStatus(){
        ...
        }catch(e){console.warn('fetchAIStatus',e)}
      },
      buildDecisionHubFromToday(d){
        ...
        const best_idea=top5[0]?{ticker:top5[0].ticker,action:top5[0].action,score:top5[0].score}:(
          best_action.best_trade_now?{ticker:best_action.best_trade_now.ticker,action:best_action.best_trade_now.action}:null
        );
        const avoid_now=(d.avoid||[]).slice(0,5).map(a=>typeof a==='string'?{ticker:'—',reason:a,category:'regime'}:{ticker:a.ticker||'—',reason:a.reason||'',category:a.category||'filter'});
        let best_rr=null;
        (top5||[]).forEach(t=>{const rr=this.parseRR(t.risk_reward||0);if(rr>0&&(!best_rr||rr>best_rr.risk_reward))best_rr={ticker:t.ticker,risk_reward:rr};});
        const stock_mon=(d.near_miss||[]).slice(0,3).map(nm=>({class:'stock',rule:'Upgrade watch '+(nm.ticker||''),detail:nm.upgrade_trigger||''}));
        const market_mon=[];
        if(regime.vix!=null) market_mon.push({class:'market',rule:'VIX '+regime.vix,detail:'Reduce size if VIX >25'});
        if(regime.breadth!=null) market_mon.push({class:'market',rule:'Breadth '+regime.breadth+'%',detail:'Broad deploy needs breadth >50%'});
        const ex=d.execution_readiness||best_action.execution_readiness||{};
        const execution_mon=[
          {class:'execution',rule:'Gateway',detail:ex.gateway_reachable?'Up — connect session':'Down — paper signals only'},
          {class:'execution',rule:'Broker login',detail:ex.broker_connected?((ex.paper_or_live||'paper').toUpperCase()+' connected'):'Not logged in'},
          {class:'execution',rule:'Position sync',detail:ex.portfolio_source||(ex.portfolio_synced?'IBKR':'Manual')},
          {class:'execution',rule:'Bracket readiness',detail:ex.bracket_order_ready?'Ready':'Draft in IBKR tab'},
          {class:'execution',rule:'Order handoff',detail:ex.readiness_label||'Broker offline'},
        ];
        if(ex.last_heartbeat) execution_mon.push({class:'execution',rule:'Heartbeat',detail:ex.last_heartbeat});
        this.decisionHub={
          warming:false,
          _from_today:true,
          decision_strip:{
          ...
          monitoring:{
            stock:stock_mon,
            portfolio:[{class:'portfolio',rule:'Position size vs 1R stop',detail:'Resize if stop widens >15%'}],
            market:market_mon,
            smart_money:[],
      ...
      async fetchCcHeader(){return this.fetchCcStatus();},
      async fetchCcStatus(){
        try{
          const tabQ=this.tab?('?tab='+encodeURIComponent(this.tab)):'';
          const r=await fetch('/api/ops/cc-header'+tabQ,{headers:{'X-API-Key':window._apiKey||'dev-secret-local'}});
          if(r.ok){
            const h=await r.json();
            const eng=h.engine||{};
            this.ccHeader={
            ...
            if(h.display_mode==='LOADING') this.healthData={...(this.healthData||{}),mode:'loading',uptime_seconds:this.healthData?.uptime_seconds||0};
            ...
            this.cc_status.ibkr_monitoring=!!(ib.monitoring_only||ib.health&&ib.health.handoff_status==='monitoring_only');
            this.cc_status.ibkr_health_label=ib.health_label||(ib.diagnosis&&ib.diagnosis.label)||(ib.health&&ib.health.summary_label)||'';
        ...
        }catch(e){/* fall through to legacy polls */}
        try{
        ...
        }catch(e){this.cc_status.mode='PAPER';this.live=false}
        try{
      ...
      contextDecisionBar(){
        ...
        if(this.tab==='portfolio'&&this.pfDecision&&this.pfDecision.decision_bar)return this.pfDecision.decision_bar;
        if(this.tab==='funds'&&this.fundMonitor.console&&this.fundMonitor.console.decision_bar)return this.fundMonitor.console.decision_bar;
        if(this.tab==='dossier'&&this.dos.intel&&this.dos.intel.decision_bar)return this.dos.intel.decision_bar;
        if((this.tab==='today'||this.tab==='signals')&&this.decisionHub&&this.decisionHub.decision_bar)return this.decisionHub.decision_bar;
        return null;
      },
      normalizedContextDecisionBar(){
        ...
        if(!raw) return null;
        const mode=this.pageSurface();
        const chip=this.normalizedAuthorityChipForTab(this.tab,{badge:raw.verdict,authority:'monitor_only',short:raw.next_action,authority_label:''});
        ...
        const blockedChip=['BLOCKED','MONITOR ONLY','RESEARCH ONLY','CONFIRM ONLY','ANALYSIS ONLY'].includes(String(chip.badge||'').toUpperCase());
        const researchSurface=!['dashboard_core','playbook_core','ibkr_execution'].includes(mode);
        if(blockedChip||researchSurface||this.pageAuthorityIsDegraded()||this.isWaitDay()||this.playbookBoardWait()){
      ...
      fundRegimeNote(){
        ...
        if(label==='UNKNOWN'&&this.today7.regime?.trend) return 'Local sleeve regime unresolved — fallback to Today regime';
        return '';
      },
      fundPayloadDegraded(){
        const d=this.fundMonitor.data||{};
      ...
      fundBenchmarkLine(){
        ...
        if(!d) return 'Loading…';
        if(this.fundPayloadDegraded()||d.benchmark_return_pct==null) return 'Benchmark '+bm+' — unavailable or pending';
      ...
      fundPrimarySummary(){
        ...
        const canAllocate=Number(strip.live_eligible_count||0)>0&&this.fundExecutionReadyBool()&&!this.fundPayloadDegraded();
        const strongestEligible=inv.strongest_eligible?.label||'';
        const strongestResearch=inv.strongest_research?.label||'';
        const strongestSleeve=strongestEligible||(strongestResearch?(strongestResearch+' · research-only'):'—');
        const whyBlocked=canAllocate
          ? 'Board / execution 已對齊，可按 band 分配'
          : (fetchFailed
            ? (this.pageOperatorSentence().blocker||'fund API fetch failed')
            : (this.fundWhyNotMoreLine(inv.why_not_more||strip.why_not_more)||inv.truth_headline||this.fundAllocationStatusNote()||'仍未有 live-validated sleeve'));
        const executionMode=canAllocate
          ? '可執行 · Executable'
          : (this.fundExecutionReadyBool()?'紙上觀察 · Paper only':'研究模式 · Research only');
        const nextAction=canAllocate
          ? ('只複核 '+(strongestEligible||strongestResearch||'當前 sleeve')+'，再按 '+(this.fundAllocationMaxCapital()||'band')+' 控制資金。')
          : (fetchFailed
      ...
      discoveryCachedLeadersRows(){
        ...
        if(!out.length&&(this.scannerHub.error||this.scannerDiscoveryHasFallbackRows())){
          (this.rankedOpps.rows||[]).filter(r=>!['AVOID','NO_TRADE','BLOCKED'].includes(String(r.action||'').toUpperCase())).slice(0,6).forEach(r=>add(r,'playbook-bridge'));
      ...
      discoveryCachedIntentRows(intent){
        ...
        const add=(row,source)=>{
          ...
          seen.add(t);
          out.push(typeof row==='object'&&row!==null?{...row,ticker:t,source:source||intent}:{ticker:t,source:source||intent});
        };
        pool.slice(0,8).forEach(r=>add(r,intent.toLowerCase()));
        if(!out.length&&(this.scannerHub.error||this.scannerDiscoveryHasFallbackRows())){
          (this.rankedOpps.rows||[]).filter(r=>{
            const a=String(r.action||'').toUpperCase();
            return !reject.has(a)&&(intent!=='BREAKOUTS'||a==='TRADE'||r.ladder_bucket==='watch_upgrade');
          }).slice(0,6).forEach(r=>add(r,'playbook-bridge'));
        }
        return out.slice(0,8);
      },
      discoveryShowCachedBreakouts(){
        return (this.scannerHub.error||this.scannerDiscoveryHasFallbackRows())&&this.discoveryCachedIntentRows('BREAKOUTS').length>0;
      },
      ...
      fundMetricPct(val,card,pendingLabel){
        if(this.fundCardMetricsPending(card)&&(val==null||val===0)) return pendingLabel||'Metrics pending';
        if(val==null) return '—';
        return val+'%';
      },
      ...
      fundReturnDisplay(card){
        if(this.fundCardMetricsPending(card)&&(card.fund_return_pct==null||card.fund_return_pct===0)) return 'Backtest evidence unavailable';
        return this.fundMetricPct(card.fund_return_pct,card,'Metrics pending');
      },
      fundAlphaDisplay(card){
        if(this.fundCardMetricsPending(card)&&(card.excess_return_pct==null||card.excess_return_pct===0)) return 'Metrics pending';
        if(card.excess_return_pct==null) return '—';
        const v=card.excess_return_pct;
        return (v>=0?'+':'')+v+'%';
      },
      fundDrawdownDisplay(card){
        if(this.fundCardMetricsPending(card)&&(card.max_drawdown_pct==null||card.max_drawdown_pct===0)) return 'Metrics pending';
      ...
      fundEvidenceWindowLine(card){
        ...
        let bm='BM pending';
        if(!this.fundCardMetricsPending(card)&&card.benchmark_return_pct!=null) bm='BM '+card.benchmark_return_pct+'%';
        else if(this.fundPayloadDegraded()) bm='BM unavailable or pending';
        return 'Window: '+win+' · '+bm+' · live trades: '+live;
      },
      verdictPillClass(v){
        const m={BUY:'pg',DEPLOY:'pg',ALLOCATE:'pg',TRADE:'pg',SCALE:'pg',PILOT:'pb',HOLD:'pw',WAIT:'pa',WATCH:'pa',REDUCE:'pr',AVOID:'pr',REBALANCE:'pa',PAUSE:'pr','RESEARCH ONLY':'pa','ANALYSIS ONLY':'pa','HOLD CASH':'pw','MONITOR ONLY':'pa','CONFIRM ONLY':'pa',BLOCKED:'pr'};
      ...
      async fetchAosMonitors(){
        ...
        try{
          const[r1,r2]=await Promise.all([
            ...
            fetch('/api/v7/monitors/evaluate',{headers:{'X-API-Key':window._apiKey||'dev-secret-local'}})
          ]);
          if(r1.ok){const d=await r1.json();this.aosMonitors.rules=d.monitors||[];}
          if(r2.ok){const d=await r2.json();this.aosMonitors.alerts=d.alerts||[];}
        }catch(e){}
        finally{this.aosMonitors.loading=false}
      },
      switchTab(t){
        ...
        if(tSafe==='strategy-lab'){if(!this.today7?.regime)this.fetchToday7();this.fetchStrategyLabShell();}
        if(tSafe==='shadow'){this.fetchShadowShell();}
        if(tSafe==='reports'){this.fetchReportsLib();}
        if(tSafe==='rs'){this.fetchRs();}
        if(tSafe==='notrade'){this.fetchRejections();}
        if(tSafe==='command'){this.fetchCommandBoard();}
        if(tSafe==='ops'){this.fetchCcStatus();this.fetchOpsRuntime();this.fetchOpsConsole();this.fetchChangelog();this.fetchErrorLog();this.fetchNotifyLog();}
        if(tSafe==='ibkr'){this.ibkrRefreshAll();}
        if(tSafe==='btlab'&&!this.btLab.data&&!this.btLab.loading){/* user clicks Run lab */}
        if(this.uiExpandAll){
      ...
      pfRisk(){
        ...
        let heatDollars=0,topVal=0,topTicker='',withStop=0,withoutStop=0;
        ...
        for(const p of positions){
          ...
          if(breached){localBreached++;continue;}
          if(px&&stopOk&&stop>0&&sh){heatDollars+=Math.max(0,(px-stop)*sh);withStop++;}
          else if(sh>0){withoutStop++;}
          const val=(px*sh)||p.market_value||0;
        ...
        if(!positions.length)heatDisplay='—';
          ...
          heatDisplay='POST-BREACH'+(postBreachR!=null?(' · '+Number(postBreachR).toFixed(2)+'R open'):'');
        }
        ...
        const heatColor=stopBreached?'red':heatAvailable&&(heatPct>=6)?'red':heatAvailable&&(heatPct>=4)?'amber':(heatDisplay.startsWith('PARTIAL')||heatDisplay==='UNAVAILABLE')?'amber':'green';
        const heatPctClass=stopBreached?'text-red-400':(heatDisplay.startsWith('PARTIAL')||heatDisplay==='UNAVAILABLE'||!heatAvailable)?'text-amber-300':heatPct>=6?'text-red-400':heatPct>=4?'text-amber-300':'text-green-400';
        let partialRiskMsg='';
        ...
        else if(withoutStop>0&&withStop>0)partialRiskMsg='Risk model partial until stop is added on '+withoutStop+' position(s). Heat excludes undefined risk.';
        else if(withoutStop>0&&withStop===0)partialRiskMsg='Heat unavailable — stop not set. Risk model partial until stop is added.';
        return{equity,heatDollars,heatPct,heatR,count:positions.length,withStop,withoutStop,stopCoverage,heatAvailable,stopBreached,breachCount,heatQualityLabel,heatDisplay,stopAnchorLabel,topPct,topTicker,var95,var95Pct,var95Quality,var95Tier,var95Sample,var95Warning,heatColor,heatPctClass,partialRiskMsg};
      },
      pfBenchmarkStatsOk(){
      ...
      pfPosRisk(pos){
        ...
        const unrealR=stopDefined&&pos.unrealized_r!=null?pos.unrealized_r:(stopDefined&&px&&entry?((px-entry)/risk):null);
        const distPct=pos.distance_to_stop_pct!=null?pos.distance_to_stop_pct:(stopDefined&&px?((px-stop)/px*100):null);
        const distUsd=pos.distance_to_stop_usd;
        const alloc=(this.pfDecision&&this.pfDecision.allocation_monitor||[]).find(r=>(r.asset||'').toUpperCase()===(pos.ticker||'').toUpperCase());
        ...
        return{
          ...
          distStopLabel:distPct!=null?(distPct.toFixed(1)+'% / $'+(distUsd!=null?Math.abs(distUsd).toFixed(0):'?')):'—',
          riskStatus,
          riskStatusClass:stopDefined?(riskStatus.includes('BREACH')?'text-red-400':riskStatus.includes('UNDEF')||riskStatus.includes('MISSING')?'text-amber-300':'text-green-400'):'text-red-400',
          nextAction,
          driftPct,
          nextActionClass:nextAction.includes('EXIT')||nextAction.includes('SET')||nextAction.includes('NO STOP')?'pr':nextAction.includes('TRIM')||nextAction.includes('TRAIL')?'pa':nextAction.includes('ADD')?'pg':'pw',
        };
      },
      pfSortedPositions(){
        const positions=(this.pf&&this.pf.positions)||[];
      ...
      pfBrokerSource(){
        ...
        let pill=link?link.source_pill:(this.pf.source==='ibkr'?(this.cc_status.ibkr_mode==='live'?'IBKR LIVE':'IBKR PAPER'):'MANUAL');
        if((!link||pill==='MANUAL')&&this.cc_status&&this.cc_status.ibkr_connected){
          pill=this.cc_status.ibkr_mode==='live'?'IBKR LIVE':'IBKR PAPER';
        }
        const pillClass=(pill.includes('LIVE')||pill.includes('PAPER')||pill==='MIXED')?'color:var(--green)':pill==='MANUAL'?'color:var(--blue)':'color:var(--amber)';
      ...
      todayFocusAuthorityChip(){
        return this.normalizedAuthorityChipForTab('today',{badge:String(this.today7.todays_decision?.deploy_posture||'').toUpperCase(),authority:'monitor_only'});
      },
      dosVerdict(){
        const d=this.dos&&this.dos.data; if(!d)return{label:'—',pill:'pw',color:'border',conf:null,reason:''};
        ...
        if(conf!=null&&conf>=0.7&&conflict==='LOW'&&leader)return{label:'TRADE',pill:'pg',color:'green',conf,reason:'High conviction · low conflict · sector leader. Size at 1R.'};
        if(conf!=null&&conf>=0.55&&conflict!=='HIGH')return{label:'WATCH',pill:'pa',color:'amber',conf,reason:'Setup forming — monitor for trigger; do not chase.'};
      ...
      dosUnified(){
        ...
        if(u){
          ...
          const conf=u.confidence_available===false?null:(u.confidence!=null?u.confidence:(this.dosVerdict().conf||null));
        ...
        return{label:v.label,pill:v.pill,color:v.color,confidence:conf,confidence_available:conf!=null&&conf>0,confidence_label:conf!=null&&conf>0?null:'Pending calibration',reason:v.reason,entry_zone:tp.entry_zone,stop:tp.stop,target_1r:tp.target_1r,target_2r:tp.target_2r,rr_ratio:tp.rr_ratio,rr_ratio_display:rr,invalidation:tp.invalidation};
      },
      ...
      dosConfidenceMetrics(){
        ...
        return intel?.confidence_metrics||{
      ...
      dosFormatDecisionConfidence(u){
        ...
        if(m.decision_confidence_available===false||pct==null||pct<=0){
          return 'Decision confidence · '+(m.decision_confidence_label||src.confidence_label||'Pending calibration');
        }
        const proxy=m.decision_confidence_source==='confluence'||src.confidence_source==='confluence'?' (proxy)':'';
      ...
      dosSizeExplanation(){
        if(this.dossierSizingBlocked()){
          const r=this.dossierSizingBlockedReason();
          if(window.CCHelpers&&CCHelpers.dossierSizingExplanation)return CCHelpers.dossierSizingExplanation(true,r);
      ...
      dosTradePlanNoteValue(){
        const tp=this.dos?.data?.trade_plan||{};
        const opts={note:tp.note,setup_type:tp.setup_type,research_only:this.dossierResearchOnly(),levels_blank:this.dosTradePlanLevelsBlank()};
      ...
      dosTradePlanRows(){
        ...
        const rows=[
          ...
          {label:'T1 / T2',value:(tp.target_1r||u.target_1r)?('$'+(tp.target_1r||u.target_1r)+' / $'+(tp.target_2r||u.target_2r)):'—'},
      ...
      dosSizeShares(){
        ...
        const d=this.dos?.data; if(!d?.price)return 0;
        const u=this.dosUnified();
        const stop=u.stop;
      ...
      dosSetTab(t){
        this.dos.subTab=t;
        if(t==='technicals')this.$nextTick(()=>{this.renderDossierChart();this.renderBenchChart()});
      ...
      dosHasLoadedData(){
        ...
        const intel=this.dos?.intel;
        return !!(intel&&(intel.dossier&&(intel.dossier.symbol||intel.dossier.price)||intel.unified_decision||intel.narrative));
      },
      dosPeersRows(){
      ...
      async fetchDosPeers(){
        ...
        try{
          const r=await this.ccFetch('/api/dossier/'+encodeURIComponent(tk)+'/peers',{retries:2,backoff:600,timeoutMs:20000});
          if(r&&r.ok)this.dos.peers=await r.json();
        }catch(e){console.warn('peers',e)}
        finally{this.dos.peersLoading=false}
      },
      async fetchDosOptions(){
        ...
        }catch(e){console.warn('options',e);this.dos.optionsData=null}
        finally{this.dos.optionsLoading=false}
      },
      flowIbkrDraft(c){
        ...
        this.dos.ticker=tk;
        this.ibkr.orderForm={...this.ibkr.orderForm,symbol:tk,secType:'STK',action:isPut?'SELL':'BUY',qty:this.dosSizeShares()||1,orderType:'MKT',limitPrice:'',useBracket:false};
      ...
      opsAdvancedCollapsedCopy(){
        ...
        const note=(d&&d.collapsed_diagnostics_note)||'Engine off or insufficient trade sample — Ops experimental panels below (self-learning, Thompson sizing, execution metrics) need runtime evidence before they affect capital. Dossier research loads from market data independently.';
      ...
      opsAdvancedDiagnosticsCollapsedTitle(){
        const H=window.CCHelpers||{};
        return H.opsAdvancedDiagnosticsCollapsedTitle?H.opsAdvancedDiagnosticsCollapsedTitle():'進階診斷（收合） · Advanced diagnostics (collapsed)';
      },
      opsAdvancedSectionKey(key){
        const H=window.CCHelpers||{};
        return H.localizeOpsAdvancedSectionKey?H.localizeOpsAdvancedSectionKey(key):String(key||'').replace(/_/g,' ');
      },
      ...
      opsSectionInactiveMsg(key){
        ...
        const s=this.opsConsole&&this.opsConsole.data&&this.opsConsole.data.section_states;
        if(s&&s[key]){
          const label=H.localizeOpsAdvancedSectionLabel?H.localizeOpsAdvancedSectionLabel(s[key].label||'inactive'):(s[key].label||'inactive');
          const detail=H.localizeOpsAdvancedSectionDetail?H.localizeOpsAdvancedSectionDetail(s[key].detail||''):(s[key].detail||'');
          return label+': '+detail;
        }
        ...
        try{
          ...
          if(r.ok){
            ...
            if(this.opsRuntime.data.engine){
              this.ops={...this.ops,...this.opsRuntime.data.engine,components:this.ops.components||this.opsRuntime.data.components||{}};
            }
          }else{
      ...
      async fetchChangelog(){
        ...
        try{
          ...
          const state=this.opsInferDegradedState({error:this.changelogPanel.error,fallback:!!this.changelogPanel.entries.length,timed_out:timedOut});
          this.surfaceFetchHints.ops_updates={loading:false,error:this.opsDegradedLine(state,this.changelogPanel.error),fallback:state==='fallback',timed_out:timedOut};
        }finally{this.changelogPanel.loading=false}
      },
      async sendDiscordTestPing(){
        try{
          const r=await fetch('/api/v7/notify/test?message='+encodeURIComponent('CC real-life notice test'),{method:'POST'});
          const d=await r.json();
          const mode=d.config?.mode||'—';
          alert(d.pushed_to_discord?`✓ Sent to Discord (${mode})`:`⚠ Not sent (${mode}) — check bot token, webhook, or channel name`);
          this.fetchNotifyLog();
      ...
      async fetchNotifyLog(){
        ...
        }catch(e){console.warn('notify log fetch failed',e)}
        finally{this.notifyLog.loading=false}
      },
      async opsStartEngine(){
        if(this.ops.engineStarting||this.ops.running) return;
        this.ops.engineStarting=true;
        try{
          const r=await fetch('/api/ops/engine/start',{method:'POST',headers:{'X-API-Key':window._apiKey||'dev-secret-local'}});
          ...
          this.fetchErrorLog();
        }finally{this.ops.engineStarting=false}
      },
      async fetchErrorLog(){
        this.errorLog.loading=true;this.errorLog.error='';
      ...
      async fetchLeaders(){
        ...
        }catch(e){this.leadersPanel.error=e.message||'leaders failed';this.leadersPanel.data=null}
        finally{this.leadersPanel.loading=false}
      },
      dosIbkrPaperDraft(){
        if(this.dossierSizingBlocked()){alert('Sizing blocked — wait for decision-grade dossier before IBKR handoff.');return;}
        const tk=(this.dos.ticker||this.dos.data?.symbol||'').toUpperCase();
        const u=this.dosUnified();
        const qty=this.dosSizeShares()||1;
        this.ibkr.orderForm={...this.ibkr.orderForm,symbol:tk,secType:'STK',action:'BUY',qty,orderType:'LMT',limitPrice:String(this.dos.data?.price||''),useBracket:true,stopPrice:String(u.stop||''),targetPrice:String(u.target_1r||'')};
        this.ibkr.workingBracket={symbol:tk,quantity:qty,entry_price:this.dos.data?.price,stop_price:u.stop,take_profit:u.target_1r,draft:true};
      ...
      dosIbkrSetAlert(){
        ...
        const u=this.dosUnified();
        const stop=u.stop;
        if(!stop){alert('No stop level in trade plan — set structure first.');return;}
        this.ibkr.orderForm={...this.ibkr.orderForm,symbol:tk,secType:'STK',action:'SELL',qty:this.dosSizeShares()||1,orderType:'STP',limitPrice:'',stopPrice:String(stop),useBracket:false};
      ...
      async fetchPortfolio(){
        ...
        try{
            ...
            if(this.cc_status&&this.cc_status.ibkr_connected){
              ...
              if(r2.ok){
                ...
                const brokerPositions=(d2.positions||[]).map(p=>{
                  ...
                  const manual=(this.pf.positions||[]).find(m=>(m.ticker||m.symbol||'').toUpperCase()===sym)||{};
                  const stop=Number(manual.stop_price||manual.initial_stop||manual.current_stop||0);
                  return {
                    ticker:sym,
          ...
          if(this.pf.positions.length>0){
            ...
            setTimeout(async()=>{
              const enrichTasks=positions.map(p=>
                this.ccFetch('/api/dossier/'+p.ticker+'/trade-advice?buy_price='+p.entry_price,{retries:2,backoff:400}).then(r=>r&&r.ok?r.json():null).catch(()=>null)
              );
          ...
          if(local&&local.holdings&&local.holdings.length){
            ...
            this.pf.source='manual';
            const totalVal=local.holdings.reduce((s,p)=>s+(p.market_value||p.entry_price*p.shares||0),0);
            const totalCost=local.holdings.reduce((s,p)=>s+(p.cost_basis||p.entry_price*p.shares||0),0);
            this.pf.summary={
      ...
      async fetchPortfolioDecision(){
        ...
        try{
          ...
          try{
            const ddPct=(this.pfDecision&&this.pfDecision.curve_diagnostics&&this.pfDecision.curve_diagnostics.max_drawdown_pct)||8.5;
            const qs=await this.ccFetchJson('/api/v7/quant/drawdown-sizing?current_dd_pct='+encodeURIComponent(ddPct)+'&research_only='+((this.authorityDegradedBanner()||this.todayUsesBriefFallback())?'true':'false'),{tab:'portfolio',retries:1,backoff:400});
            if(qs.ok)this.pfQuantSizing=qs.data;
          }catch(e){console.warn('quant drawdown-sizing',e)}
        }catch(e){console.warn('portfolio-decision',e)}
        finally{this.pfDecisionLoading=false}
      },
      async fetchPortfolioEquity(){
        this.pfEquityLoading=true;
        try{
          const r=await this.ccFetch('/api/v7/portfolio-equity?period=6mo&benchmark=SPY',{retries:2,backoff:500});
          if(r&&r.ok)this.pfEquity=await r.json();
        }catch(e){console.warn('portfolio-equity',e)}
        finally{this.pfEquityLoading=false}
      },
      async runRebalanceSim(policy,targetsStr){
        ...
        }catch(e){console.warn('rebalance-sim',e)}
      },
      async fetchScenarioList(){
        try{
          const r=await fetch('/api/scenarios');
          if(r.ok){const d=await r.json();this.pfScenario.list=d.scenarios||[];}
        }catch(e){console.warn('scenarios list',e)}
      },
      async runScenarioShock(key){
        if(!key)return;
        this.pfScenario.loading=true;this.pfScenario.selected=key;this.pfScenario.result=null;
        try{
          const r=await this.ccFetch('/api/v7/scenario-shock?scenario_key='+encodeURIComponent(key),{method:'POST',retries:2,backoff:600});
          if(r&&r.ok)this.pfScenario.result=await r.json();
        }catch(e){console.warn('scenario-shock',e)}
        finally{this.pfScenario.loading=false}
      },
      async fetchBacktestLab(){
        if(!this.btLab.ticker)return;
        this.btLab.loading=true;this.btLab.error='';this.btLab.data=null;
        try{
          const u='/api/v7/backtest-lab?ticker='+encodeURIComponent(this.btLab.ticker)+'&strategy='+encodeURIComponent(this.btLab.strategy)+'&period='+encodeURIComponent(this.btLab.period)+'&walk_forward='+(this.btLab.walkForward?'true':'false');
          ...
          try{
            const qc=await this.ccFetchJson('/api/v7/quant/strategy-health?ticker='+encodeURIComponent(this.btLab.ticker),{tab:'btlab',retries:1,backoff:400});
            if(qc.ok)this.btLab.quantCurve=qc.data;
          }catch(e){console.warn('quant strategy-health',e)}
          this.$nextTick(()=>this.renderBtlabChart());
        }catch(e){this.btLab.error=this.surfaceFetchErrorLine(e.message||String(e),'btlab');console.warn('backtest-lab',e)}
        finally{this.btLab.loading=false}
      },
      renderBtlabChart(){
        ...
        }catch(e){console.warn('btlab chart',e)}
      },
      async fetchQuoteWorkstation(){
        ...
        try{
          const r=await this.ccFetch('/api/v7/quote-workstation/'+encodeURIComponent(tk),{retries:2,backoff:600});
          if(r&&r.ok)this.dosWorkstation.data=await r.json();
        }catch(e){console.warn('quote-workstation',e)}
        finally{this.dosWorkstation.loading=false}
      },
      async fetchCatalystCalendar(){
        ...
        }catch(e){console.warn('catalyst-calendar',e)}
        finally{this.platformExtras.loading=false}
      },
      async runPmMemo(scope){
        const u='/api/v7/pm-memo?scope='+encodeURIComponent(scope)+(scope==='ticker'&&this.dos.ticker?'&ticker='+encodeURIComponent(this.dos.ticker):'');
        try{
        ...
        }catch(e){console.warn('pm-memo',e)}
      },
      async fetchLedgerView(force=false){
        ...
        try{
          ...
          if(r1.ok){const d=await r1.json();this.ledgerView.rows=d.rows||[];}
          if(r2.ok){
            const raw=await r2.json();
            const s=raw.summary||{};
            const aggOk=raw.aggregate_metrics_available!==false
        ...
        }catch(_){this.ledgerView.rows=[];this.ledgerView.stats=null;}
        finally{this.ledgerView.loading=false;}
      },
      ...
      async addPosition(){
        ...
        this.pf.addError='';
        this.pf.addSuccess='';
        if(!ticker){this.pf.addError='Ticker is required';return;}
        if(!shares||shares<=0){this.pf.addError='Shares must be greater than 0';return;}
        if(!entry||entry<=0){this.pf.addError='Entry price must be greater than 0';return;}
        ...
        const finishAdd=(d,pos)=>{
          const msg=(d&&d.message)||((d&&d.broker_sync==='unavailable')?'Saved locally · Broker sync unavailable':'Position saved');
          this.pf.addSuccess=msg;
          this.resetAddPositionForm();
          if(pos&&pos.quote_pending){
            this.pf.alerts=[{ticker:pos.ticker,type:'QUOTE_PENDING',severity:'warning',msg:'⏳ '+pos.ticker+' 已保存 · 報價待回補，暫以 entry price 顯示'},...(this.pf.alerts||[])];
          }
        };
        try{
          const r=await this.ccFetch('/api/portfolio/position',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body),retries:2,backoff:400});
          if(r&&r.ok){
            const d=await r.json();
            await this.fetchPortfolio();
      ...
      _dosCacheKey(t){return 'cc_dos_cache_v2_'+(t||'').toUpperCase()},
      _normalizeStockIntel(intel){
        ...
        const isEnvelope=!!(intel.dossier||intel.unified_decision||intel.narrative||intel.load_phase||intel.decision_stack);
        let dossier=null;
        ...
        const envelope=isEnvelope?{...intel,dossier}:{...intel,dossier,load_phase:intel.load_phase||'full'};
      ...
      dosSaveCache(t,intel){
        ...
        try{localStorage.setItem(this._dosCacheKey(t),JSON.stringify({ts:Date.now(),intel:norm}))}catch(_){}
      },
      dosLoadCache(t){
        if(!t)return null;
      ...
      dosTrustAsOf(){
        ...
        if(this.dos.status==='partial_loaded'&&(this.dos.partialNotice||this.dos.error))return this.dos.partialNotice||'Partial load — see notice below';
        if(this.dos.status==='stale_fallback'&&this.dos.staleAt)return 'Cached: '+new Date(this.dos.staleAt).toISOString().slice(0,19);
        if(this.dos.data&&this.dos.data.trust&&this.dos.data.trust.as_of)return 'As-of: '+this.dos.data.trust.as_of.slice(0,19);
      ...
      _applyDossierIntel(intel,meta={}){
        ...
        if(meta.stale){
          ...
          this.dos.status=envelope.load_phase==='core'?'partial_loaded':(this.dos.loadingEnrich?'loading_enrichments':'partial_loaded');
        }else{
          this.dos.status='loaded';
          this.dos.staleAt=null;
        }
        this.dos.chartPeriod=this.dos.chartPeriod||'6mo';
        if(this.dos.subTab==='technicals')this.$nextTick(()=>{this.renderDossierChart();this.renderBenchChart()});
      ...
      async _refreshDossierFullSilent(tk, reqId){
        try{
          const r=await this.ccFetch('/api/v7/stock-intel/'+encodeURIComponent(tk),{retries:1,backoff:800,timeoutMs:60000});
      ...
      async fetchDossierEnrichments(){
        ...
        try{
          const r=await this.ccFetch('/api/v7/stock-intel/'+encodeURIComponent(tk)+'?enrichments=true',{retries:2,backoff:800,timeoutMs:45000});
          if(reqId!==this.dos._reqId)return;
          if(!r||!r.ok){const e=r?await r.json().catch(()=>({})):{};throw new Error(this.formatReplayError(e.detail||e.error||'Enrichments unavailable'))}
      ...
      async fetchOpportunityIntel(){
        const tk=(this.dos.ticker||'').trim().toUpperCase();
        if(!tk){this.dos.oppIntel=null;return;}
        const reqId=this.dos._reqId;
      ...
      async fetchDossier(opts={}){
        ...
        if(this.dos._abort){try{this.dos._abort.abort()}catch(_){}}
        ...
        try{
          ...
          const r=await this.ccFetch(url,{retries:2,backoff:800,timeoutMs:15000,init:{signal:abort.signal}});
          ...
          if(coreOnly){
            this.dos.status=degraded||intel.load_phase==='core'||intel.partial?'partial_loaded':'loaded';
            this._syncDossierFetchHints();
            return;
          }
      ...
      openDossier(ticker,meta){
        ...
        if(meta&&typeof meta==='object'){
          Object.entries(meta).forEach(([k,v])=>{if(v!=null&&v!=='')ctx[k]=v;});
        }
        if(from==='signals'){
      ...
      async runBT(){
        ...
        try{const u='/api/live/backtest?ticker='+encodeURIComponent(this.bt.ticker)+'&strategy='+this.bt.strategy+'&period='+this.bt.period;const r=await this.ccFetch(u,{method:'POST',retries:2,backoff:600});if(!r||!r.ok){let msg='Failed';try{const e=await r.json();msg=e.detail||e.error||msg;}catch(_x){}throw new Error(msg)}const data=await r.json();if(data.degraded){alert('API warming up — backtest shell only. Retry when health mode is full.');}this.bt.result=data;this.$nextTick(()=>this.renderBTEquityChart())}catch(e){alert('Error: '+this.surfaceFetchErrorLine(e.message,'backtest'))}finally{this.bt.loading=false}
      },
      ...
      async ibkrDisconnect(){
        ...
        this.ibkr.connected=false;this.ibkr.account=null;this.ibkr.positions=[];this.ibkr.openOrders=[];this.ibkr.recentFills=[];this.ibkr.readiness=null;this.ibkr.diagnostics=null;
      },
      ...
      async ibkrFetchOpenOrders(){
        ...
        }catch(e){console.warn('ibkr open orders failed',e)}
      },
      async ibkrFetchRecentFills(){
        try{
          const r=await fetch('/api/ibkr/recent-fills',{headers:{'X-API-Key':window._apiKey||'dev-secret-local'}});
          if(!r.ok)return;
          const d=await r.json();
          this.ibkr.recentFills=d.fills||[];
        }catch(e){console.warn('ibkr fills failed',e)}
      },
      async ibkrFetchPortfolioCompare(){
        try{
          ...
          if(manual>0&&broker===0&&this.ibkr.connected){
            ...
            note='When connected, IBKR positions are broker truth; Portfolio tab overlays stop/target research fields.';
          }
          this.ibkr.portfolioCompare={manual,broker,note};
        }catch(e){console.warn('portfolio compare failed',e)}
      },
      ...
      ibkrOrderNotional(){
        ...
        const notional=px>0&&qty>0?('$'+(px*qty).toLocaleString(undefined,{maximumFractionDigits:0})):'MKT — price at fill';
        let risk='—';
      ...
      ibkrBracketPreview(){
        const f=this.ibkr.orderForm;
        const stop=Number(f.stopPrice)||0,target=Number(f.targetPrice)||0,entry=Number(f.limitPrice)||0;
        ...
        if(stop&&target){
          ...
          if(risk>0&&reward>0)riskReward=`$${(risk*Number(f.qty||0)).toFixed(0)} risk · $${(reward*Number(f.qty||0)).toFixed(0)} reward · R:R ${(reward/risk).toFixed(2)}:1`;
        }
        return{
          ocaNote: stop&&target ? 'OCA group assigned on transmit (one child cancels the other)' : 'Pending stop/target',
          riskReward,
      ...
      ibkrPreviewOrder(){
        ...
        if(f.useBracket){
          ...
          const px=f.orderType==='LMT'?('LMT $'+(f.limitPrice||'?')):f.orderType==='STP'?('STP @ $'+(f.stopPrice||'?')):'MKT';
      ...
      async ibkrPlaceOrder(){
        ...
        const sym=(f.symbol||'').toUpperCase();
        if(!sym){this.ibkr.orderError='Symbol required';return;}
        if(f.orderType==='STP'&&!f.stopPrice){this.ibkr.orderError='Stop trigger price required for STP orders';return;}
        if(f.orderType==='LMT'&&!f.limitPrice){this.ibkr.orderError='Limit price required for LMT orders';return;}
        const px=f.orderType==='MKT'?'MKT':f.orderType==='LMT'?('LMT @ $'+(f.limitPrice||'?')):('STP @ $'+(f.stopPrice||'?'));
        ...
        this.ibkr.loading=true;this.ibkr.orderResult=null;this.ibkr.orderError='';this.ibkr.orderPreview=null;
        try{
          const body={symbol:sym,sec_type:f.secType,action:f.action,quantity:Number(f.qty),order_type:f.orderType,tif:f.tif||'DAY'};
      ...
      async ibkrPlaceBracket(){
        ...
        try{
          ...
          if(this.ibkr.mode==='live'){
            this.ibkr.orderError='🛑 SLIPPAGE GATE UNAVAILABLE — live orders blocked until /api/slippage/check responds.';
      ...
      sleeveSparkPoints(curve){
        if(!curve||curve.length<2)return '';
        const mn=Math.min(...curve),mx=Math.max(...curve),rg=mx-mn||1;
        return curve.map((v,i)=>`${i*4},${18-Math.round(((v-mn)/rg)*16)}`).join(' ');
      },
      marketRoleFor(sym){
        const raw=(sym||'').toUpperCase();
        const s=raw.replace(/^\^/,'');
        if(['SPY','QQQ','IWM','DIA','BTC-USD','ETH-USD'].includes(raw)||['SPY','QQQ','IWM','DIA'].includes(s)) return 'risk-on';
      ...
      _marketStripFromLive(d){
        ...
        want.forEach(w=>{
          ...
          const hit=bySym[k];
          if(hit&&!ordered.some(x=>(x.symbol||'').toUpperCase()===(hit.symbol||'').toUpperCase())) ordered.push(hit);
        });
        pool.forEach(item=>{
          const n=this.normalizeMarketItem(item);
          const key=(n.symbol||'').toUpperCase();
      ...
      async fetchMarketStrip(){
        ...
        try{
          ...
          if(!r||!r.ok){
            const msg=r&&r.status===503?'API warming up — retry in a few seconds':'Market data unavailable (HTTP '+(r?r.status:'?')+')';
      ...
      async fetchToday7(){
        try{const r=await this.ccFetch('/api/v7/today',{retries:2,backoff:500,timeoutMs:15000});if(!r||!r.ok)throw new Error('HTTP '+(r?r.status:'fail'));const d=await r.json();this.captureInstantDegradedBanner(d);if(d&&d.error)throw new Error(String(d.error));if(!d||!d.market_regime)throw new Error('incomplete today payload');this.today7.regime=d.market_regime||null;this.today7.decision_authority=d.decision_authority||null;this.today7.top_ranked=d.top_5||[];this.today7.todays_decision=d.todays_decision||null;this.today7.bdr_summary=d.bdr_summary||null;if(this.bdrShouldAutoOpen()) this.bdrPanelOpen=true;this.today7.feature_ic_status=d.feature_ic_status||null;this.today7.ml_advisory=d.ml_advisory||null;this.today7.best_action=d.best_action||null;this.today7.overlap_warning=d.overlap_warning||null;this.today7.near_miss=d.near_miss||[];this.today7.score_reconciliation=d.score_reconciliation||null;this.today7.no_setup_diagnosis=d.no_setup_diagnosis||null;this.today7.quant_cluster_hints=d.quant_cluster_hints||[];this.today7.unlock_deploy=d.unlock_deploy||null;this.today7.regime_wait_explanation=d.regime_wait_explanation||[];this.today7.monitor_triggers=d.monitor_triggers||[];this.today7.sleeve_summary=d.sleeve_summary||null;this.today7.execution_readiness=d.execution_readiness||d.best_action?.execution_readiness||null;this.today7.evidence_badges=d.evidence_badges||null;this.today7.cross_asset_confirmation=d.cross_asset_confirmation||null;this.today7.index_regime_summary=d.index_regime_summary||null;this.today7.regime_strip=d.regime_strip||null;this.today7.regime_stack_summary=d.regime_stack_summary||null;this.today7.allocator_stance=d.allocator_stance||null;this.today7.ai_reason_codes=d.ai_reason_codes||[];this.today7.filter_funnel=d.filter_funnel||null;this.today7.decision_model=d.decision_model||null;this.today7.decision_hierarchy=d.decision_hierarchy||null;this.today7.passive_baseline=d.passive_baseline||null;this.today7.complexity_challenge=d.complexity_challenge||null;this.today7.restraint=d.restraint||null;this.today7.surface_authority=d.surface_authority||null;this.today7.crisis_regime=d.crisis_regime||null;this.today7.naval_clarity=d.naval_clarity||null;this.today7.buffett_clarity=d.buffett_clarity||null;this.today7.index_fund_posture=d.index_fund_posture||null;this.today7.principles_posture=d.principles_posture||null;this.today7.avoid_grouped=d.avoid_grouped||null;this.today7.bucket_quality=d.bucket_quality||[];this.today7.avoid_list=(d.avoid_now||[]).length?(d.avoid_now||[]):(d.avoid||[]).map(a=>typeof a==='string'?{ticker:'—',reason:a,category:'regime'}:a);this.today7.tradeability=(d.cc_state&&d.cc_state.tradeability_state&&d.cc_state.tradeability_state.tradeability)||(d.decision_model&&d.decision_model.honest_tradeability)||(d.market_regime||{}).tradeability||'';this.today7.what_changed=d.what_changed||[];this.today7.event_risks=d.event_risks||[];this.today7.best_family=d.best_setup_family||null;this.today7.pulse=d.market_pulse||null;this.today7.narrative=d.narrative||'';this.today7.ai_narrative=d.ai_narrative||null;this.today7.date=d.date||'';this.today7.trust=d.trust||{};this.today7.cc_state=d.cc_state||null;this.today7.system_state=d.system_state||null;this.today7.dashboard_monitors=d.dashboard_monitors||[];this.today7.page_capability=d.page_capability||null;
          ...
          if(this.isWaitDay()&&!this.uiExpandAll){this.today7.avoid_collapsed=true;this.today7.ai_commentary_open=false;}
        ...
        }catch(e){console.warn('v7/today fetch failed',e);this.today7.trust={...(this.today7.trust||{}),stale:true,reason:String(e.message||e).slice(0,140)};this.hydrateToday7FromCache();if(!(this.today7.top_ranked||[]).length){const bridged=this.playbookBridgeFromCaches();if(bridged.length)this.today7.top_ranked=bridged.slice(0,5);}if(!this.playbookBoardHasContent())this.fetchWarmupBriefBoard();}
      },
      ...
      saveToday7ToCache(d){
        try{
          ...
          localStorage.setItem('cc_today7_snapshot',JSON.stringify({
        ...
        }catch(e){console.warn('today7 cache save failed',e)}
      },
      async fetchDecisionHub(){
        try{
          ...
          if(hub.warming&&local?.decision_strip?.best_idea_now){
            this.decisionHub={...hub,decision_strip:local.decision_strip,monitoring:hub.monitoring||local.monitoring};
          }else{
            this.decisionHub=hub;
          }
        }catch(e){console.warn('decision-hub',e)}
      },
        ...
        try{
          if(needsBoard&&!this.rankedOpps.actionFilter&&!this.rankedOpps.sectorFilter){
            ...
            if(this.rankedOpps.rows.length){
              ...
              try{
                const sr=await fetch('/api/v7/playbook/ranked/snapshot?limit=50',{signal:AbortSignal.timeout(5000)});
                if(sr.ok) this.applyRankedPayload(await sr.json(),{fromSnapshot:true});
              }catch(e){console.warn('ranked snapshot hydrate failed',e)}
          ...
          if(!r.ok){
            if(r.status===503){
              ...
              }catch(e2){console.warn('ranked snapshot fallback failed',e2)}
            }
            throw new Error('HTTP '+r.status);
          }
          this.applyRankedPayload(await r.json());
          this.rankedOpps.fetch_failed=false;
          this.surfaceFetchHints.signals={...(this.surfaceFetchHints.signals||{}),failed_fetch:false,error:''};
          if(!this.rankedOpps.rows.length&&!this.rankedOpps.actionFilter&&!this.rankedOpps.sectorFilter&&!this.playbookIsEmergencyBoard()){
            if(this.healthMode==='loading') await this.fetchWarmupBriefBoard();
            if(!this.rankedOpps.rows.length){
            try{
              const sr=await fetch('/api/v7/opportunity-scanner?regime='+(this.regime.label||'BULL')+'&top_n=50');
              if(sr.ok){
                const sd=await sr.json();
                const cands=(sd.candidates||[]).slice(0,50);
                if(cands.length){
                  this.rankedOpps.rows=cands.map((c)=>({ticker:c.ticker||c.symbol,score:c.score||c.composite_score||0,action:c.tag||c.action||'WATCH',sector_type:c.sector||'',entry_price:c.close||c.price,stop_price:c.stop_loss,target_price:c.activation||c.target,risk_reward:c.rr||0,setup:c.strategy||c.engine||'scanner',grade:c.score>=8?'A':c.score>=6?'B':'C',thesis_conf:0.6,timing_conf:0.5,exec_conf:0.5,data_conf:0.7,why_now:c.tags?c.tags.join(' · '):c.reason||''}));
                  this.rankedOpps.source='scanner';
                  this.rankedOpps.board_mode='compressed_fallback';
                }
              }
            }catch(e){console.warn('scanner fallback failed',e)}
            }
          }
          if(this.rankedOpps.rows.length) this.loadSparklines(this.rankedOpps.rows);
          if((this.rankedOpps.rows||[]).length||(this.rankedOpps.near_miss||[]).length) this.saveRankedToCache();
        }catch(e){
          ...
          if(!this.playbookBoardHasContent()){
            ...
            if(this.rankedOpps.fetch_failed)
              this.surfaceFetchHints.signals={...(this.surfaceFetchHints.signals||{}),failed_fetch:true,error:this.surfaceFetchStateMessage('failed_fetch')};
          }
        }
        finally{this.rankedOpps.loading=false;this.rankedOpps.refreshing=false}
      },
      applyRankedPayload(d,meta={}){
        ...
        const incomingAllAvoid=!!(d.opportunities||[]).length&&!(d.opportunities||[]).some(row=>!['AVOID','NO_TRADE'].includes(String(row.action||'').toUpperCase()));
        ...
        if(incomingEmpty&&(d.degraded||d.instant_degraded)&&!hasLocal&&!meta.fromCache){
          const bridged=this.playbookBridgeFromCaches();
          if(bridged.length){
            d={...d,opportunities:bridged,count:bridged.length,source:d.source||'brief-bridge',
              board_message:d.board_message||'API returned empty — showing brief/cache bridge (monitor only)'};
          }
        }
        ...
        if((stillEmpty||incomingAllAvoid)&&!(d.near_miss||[]).length&&hasLocal&&!meta.force){
          ...
          this.rankedOpps.warning=d.warning||this.rankedOpps.warning||'Live refresh returned empty — showing last-good board';
        ...
        this.rankedOpps.near_miss=(d.near_miss&&d.near_miss.length)?d.near_miss:this.derivePlaybookNearMiss(d.opportunities||[]);
      ...
      hydrateRankedFromCache(){
        ...
        }catch(e){console.warn('playbook cache hydrate failed',e)}
      },
      saveRankedToCache(){
        try{
          const payload={
        ...
        }catch(e){console.warn('playbook cache save failed',e)}
      },
      _SCANNER_META_KEYS:['count','top_hits','display_count','urgent_count','warning_count','urgent','warnings'],
      ...
      scannerCategoryLabel(cat){
        const m={VALIDATION:'Confirmation',NO_TRADE:'Avoid Now',PATTERN:'Price / Pattern',FLOW:'Options Flow',RISK:'Risk Flags',SECTOR:'Sector',LEADERS:'Leaders',PULLBACKS:'Pullbacks',BREAKOUTS:'Breakouts'};
      ...
      scannerCategoryEmptyWhy(cat){
        ...
        const w={FLOW:'No unusual volume or options-flow triggers in the scanned universe.',VALIDATION:'No confirmation / calibration patterns passed strict gates.',PATTERN:'No VCP, breakout, or pullback patterns met score thresholds.',SECTOR:'No sector rotation or leader/laggard signals fired.',RISK:'No risk warnings — or regime is permissive.',LEADERS:'No RS leaders mapped for this intent filter.',PULLBACKS:'No pullback setups in current tape.',BREAKOUTS:'No breakout pivots with confirming volume.',NO_TRADE:'No avoid-now composite triggers — see Rejections for pipeline blocks.'};
        return w[cat]||'No symbols passed this category threshold in the current run.';
      },
      scannerIntentRow(intent){
        const d=this.scannerHub.data;
        return (d&&d.decision_intent&&d.decision_intent[intent])||(d&&d.category===intent?{probe_status:d.probe_status,count:d.count,regime_note:d.regime_note,empty_why:d.empty_why}:null);
      },
      ...
      scannerPageGateLabel(){
        const tb=(this.today7&&this.today7.tradeability)||(this.scannerHub.data&&this.scannerHub.data.diagnostics&&this.scannerHub.data.diagnostics.tradeability)||'';
        if(!tb) return '';
        return 'Dashboard gate: '+String(tb).replace(/_/g,' ');
      },
      scannerPageGateClass(){
        const tb=String((this.today7&&this.today7.tradeability)||(this.scannerHub.data&&this.scannerHub.data.diagnostics&&this.scannerHub.data.diagnostics.tradeability)||'').toUpperCase();
      ...
      scannerCategoryScannerEntries(catName){
        ...
        return Object.entries(grp).filter(([n,p])=>!this._SCANNER_META_KEYS.includes(n)&&this.scannerHitCount(p)>0);
      },
      ...
      discoveryConfidenceLabel(row){
        const r=row||{};
        if(String(r.score_display_mode||'')==='fallback_rank'||this.scannerUsesBriefFallback()||this.scannerDiscoveryHasFallbackRows()){
      ...
      portfolioHasTicker(tk){
        if(!tk) return false;
        return (this.pf?.positions||[]).some(p=>(p.ticker||p.symbol||'').toUpperCase()===String(tk).toUpperCase());
      },
      ...
      hydrateScannersFromCache(){
        ...
        }catch(e){console.warn('scanner hub cache hydrate failed',e)}
      },
      saveScannersToCache(d){
        try{
          if(!this.scannerHubIsRenderable(d)) return;
          localStorage.setItem('cc_scanner_hub_snapshot',JSON.stringify({...d,saved_at:Date.now()}));
        }catch(e){console.warn('scanner hub cache save failed',e)}
      },
            ...
            if(r&&r.status===503){
              ...
              if(sr.ok){
                ...
                if(this.scannerHubIsRenderable(d)){
                  this.scannerHub.data=d;this.scannerHub.error='';this.saveScannersToCache(d);
                  this.surfaceFetchHints[tab]={loading:false,error:'',fallback:this.scannerUsesBriefFallback(),research_only:true};
                  return;
                }
              }
            }
      ...
      guideAuthorityStrip(){ return 'GUIDE MODE · Reference only · Decision surfaces suspended'; },
      guideStatusNote(){
        const mode=(this.cc_status&&this.cc_status.mode)||'PAPER';
        return 'Mode · '+mode+' · Runtime not evaluated here';
      },
      tabAuthorityChip(){
        if(this.tab==='guide'){
          return {
        ...
        if(!strip||!(strip.surfaces||[]).length){
          return this.normalizedAuthorityChipForTab(this.tab,{badge:'—',short:'',authority:'research_only',authority_label:''});
        }
      ...
      aiCommentaryEmptyLine(){
        if(this.today7.ai_provider==='stub') return 'Rule-based summary only — configure an LLM for richer narrative; board authority unchanged.';
      ...
      flowRegimeContextHint(){
        if(this.flowNotCalibrated()) return 'Calibration pending — treat grades as heuristic until forward outcomes are logged.';
        return 'Regime context only — deploy authority stays with Playbook and Dossier.';
      },
      flowShowMockPreviewSubtitle(){
        const d=this.flowPanel?.decision;
        return !!(d&&(d.freshness?.synthetic||d.mock_only)&&(d.mock_flow||[]).length&&!(d.count_live||0));
      },
      flowPmActionLabel(action){
        const a=String(action||'').toUpperCase();
        if(a==='WATCH_FOR_STOCK_CONFIRM') return '待價格確認 · confirm';
        if(a==='NOT_ACTIONABLE') return '研究模式 · research';
        if(a==='BUYABLE_NOW'||a==='ACTIONABLE'||a==='PROMOTION_CANDIDATE') return '研究候選 · research candidate';
      ...
      rsVisibleBuyabilityLabel(label){
        ...
        if(['BUYABLE_NOW','ACTIONABLE','BUY','TRADE','PROMOTION_CANDIDATE'].includes(a)) return '不可執行 · not executable';
      ...
      rsResearchActionLabel(row){
        ...
        if(['BUYABLE_NOW','ACTIONABLE','BUY','TRADE','PROMOTION_CANDIDATE'].includes(a)) return '研究候選 · research candidate';
      ...
      flowCalibrationDetail(){
        ...
        return c?.detail||'The flow model has not yet logged enough closed trade outcomes to estimate hit-rate, follow-through quality, or sizing usefulness. Until this sample exists, all flow grades should be treated as heuristic research labels only.';
      },
      flowCardDisclaimerShort(c){
        const tk=String(c?.underlying||'').toUpperCase();
        if(['QCOM','NET','AMAT'].includes(tk)) return 'Mock flow — confirm structure in Playbook / Dossier first.';
      ...
      flowCardDisclaimerDetail(c){
        const tk=String(c?.underlying||'').toUpperCase();
        if(tk==='QCOM') return 'QCOM 可能見到偏高 options 活動，但未經 live provider 同 stock follow-through 確認前，只屬 research-only。先用 Playbook / Dossier 確認 sector leadership 同 structure。';
        if(tk==='NET') return 'NET mock flow can look active on vol/OI, yet remains research preview only. Confirm trend and risk in Playbook / Dossier; do not size from synthetic flow alone.';
        if(tk==='AMAT') return 'AMAT mock prints are illustrative equipment-sector colour, not deployable edge. Wait for live flow and stock confirmation elsewhere before any sizing discussion.';
        if(tk==='SOUN') return 'SOUN 嘅 synthetic flow 雖然吸睛，但目前只屬 research-only。呢種 options profile 代表注意力，唔代表 deployable edge；未有 live flow 同 stock 結構確認前，只可當 watchlist colour。';
        if(tk==='AAPL') return 'AAPL flow is interesting but not investable in its current form. The flow is mock / synthetic, and the stock itself is not fully confirmed by the broader decision stack. Use this as supporting context only, not as a reason to initiate or size a trade.';
        return '';
      },

      async fetchFlow(){
        const tab='flow';
        this.flowPanel.loading=true;this.flowPanel.error='';
        try{
        ...
        }catch(e){this.flowPanel.error=this.surfaceFetchErrorLine(e.message||'flow fetch failed',tab)}
        finally{this.flowPanel.loading=false}
      },
      ...
      async fetchVibeAgent(){
        const tab='agent';
        this.vibeAgent.loading=true;this.vibeAgent.error='';
        try{
          const paused=!!this.vibeAgent.paused;
          const [st,bf,ru,al,jo,ev,ct]=await Promise.all([
            this.ccFetchJson('/api/v7/vibe-agent/status?paused='+(paused?'true':'false'),{tab,retries:2,backoff:500}),
          ...
          if(ev.ok&&Array.isArray(ev.data?.triggeredAlerts)){
            ...
            if(merged.length){
              const seen=new Set((this.vibeAgent.alerts||[]).map(a=>a.id||a.ruleId+'-'+a.triggeredAt));
              merged.forEach(a=>{
                const k=a.id||a.ruleId+'-'+a.triggeredAt;
                if(!seen.has(k)){this.vibeAgent.alerts.unshift(a);seen.add(k);}
              });
            }
          }
          if(!st.ok&&!bf.ok) this.vibeAgent.error=this.surfaceFetchErrorLine(st.error||bf.error||'agent fetch failed',tab);
        }catch(e){this.vibeAgent.error=this.surfaceFetchErrorLine(e.message||'agent fetch failed',tab)}
        finally{this.vibeAgent.loading=false}
      },
      ...
      async parseVibeIntentPreview(){
        ...
        this.vibeAgent.loading=true;this.vibeAgent.error='';
        try{
          const res=await this.ccFetchJson('/api/v7/vibe-agent/intent/parse',{tab:'agent',retries:1,backoff:300,init:{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text})}});
          if(!res.ok){this.vibeAgent.error=res.error||'parse failed';return;}
          this.vibeAgent.intentPlan=res.data?.plan||null;
        }catch(e){this.vibeAgent.error=e.message||'parse failed'}
        finally{this.vibeAgent.loading=false}
      },
      ...
      async saveVibeIntent(){
        ...
        this.vibeAgent.loading=true;this.vibeAgent.error='';
        try{
          const res=await this.ccFetchJson('/api/v7/vibe-agent/intent',{tab:'agent',retries:1,backoff:300,init:{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text})}});
          if(!res.ok){this.vibeAgent.error=res.error||'save failed';return;}
          this.vibeAgent.intentPlan=res.data?.plan||null;
          this.vibeAgent.intentText='';
          await this.fetchVibeAgent();
        }catch(e){this.vibeAgent.error=e.message||'save failed'}
        finally{this.vibeAgent.loading=false}
      },
      ...
      async saveAgentWatchRule(){
        ...
        const body={
          ...
          asset:String(d.asset||'').trim().toUpperCase()||'WATCHLIST',
        ...
        try{
          const res=await this.ccFetchJson('/api/v7/vibe-agent/rules',{tab:'agent',retries:1,backoff:300,init:{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}});
          if(!res.ok){this.vibeAgent.error=res.error||'rule save failed';return;}
          this.vibeAgent.ruleDraft={name:'',asset:'',ruleType:'price_zone_touch',condition:'',expiryDays:14};
          await this.fetchVibeAgent();
        }catch(e){this.vibeAgent.error=e.message||'rule save failed'}
        finally{this.vibeAgent.loading=false}
      },
      ...
      async muteAgentRule(ruleId){
        if(!ruleId) return;
        try{
          await this.ccFetchJson('/api/v7/vibe-agent/rules/'+encodeURIComponent(ruleId),{tab:'agent',retries:1,init:{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({status:'muted'})}});
          await this.fetchVibeAgent();
        }catch(e){}
      },
      ...
      async checkAgentGuardrail(actionType,context){
        try{
          const res=await this.ccFetchJson('/api/v7/vibe-agent/guardrail',{tab:'agent',retries:1,init:{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action_type:actionType,context:context||{}})}});
      ...
      async fetchStrategyLabShell(){
        ...
        this.strategyLab.loading=true;this.strategyLab.error='';
        try{
          const res=await this.ccFetchJson('/api/v7/research/contract?surface=strategy-lab',{tab,retries:1});
          if(res.ok) this.strategyLab.safety=res.data;
        }catch(e){this.strategyLab.error=e.message||'load failed'}
        finally{this.strategyLab.loading=false}
      },
      ...
      async parseStrategyDraft(){
        ...
        this.strategyLab.loading=true;this.strategyLab.error='';
        try{
          const res=await this.ccFetchJson('/api/v7/research/strategy/parse',{tab:'strategy-lab',retries:1,init:{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({prompt})}});
          if(!res.ok){this.strategyLab.error=res.error||'parse failed';return;}
          this.strategyLab.draft=res.data?.draft||null;
        }catch(e){this.strategyLab.error=e.message||'parse failed'}
        finally{this.strategyLab.loading=false}
      },
      ...
      async saveStrategyDraft(){
        ...
        try{
          const res=await this.ccFetchJson('/api/v7/research/strategy',{tab:'strategy-lab',retries:1,init:{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({prompt})}});
          if(!res.ok){this.strategyLab.error=res.error;return;}
          this.strategyLab.draft=res.data?.draft||null;
        }catch(e){this.strategyLab.error=e.message}
        finally{this.strategyLab.loading=false}
      },

      async runStrategyValidation(){
        if(!this.strategyLab.draft?.id){this.strategyLab.error='Save draft first';return;}
        this.strategyLab.loading=true;
        try{
          const res=await this.ccFetchJson('/api/v7/research/validate',{tab:'strategy-lab',retries:1,init:{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({draft_id:this.strategyLab.draft.id})}});
          if(!res.ok){this.strategyLab.error=res.error;return;}
      ...
      async runResearchPipeline(){
        ...
        this.strategyLab.loading=true;
        try{
          const res=await this.ccFetchJson('/api/v7/research/pipeline',{tab:'strategy-lab',retries:1,init:{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({prompt})}});
          if(!res.ok){this.strategyLab.error=res.error;return;}
      ...
      async runCommitteeReview(){
        ...
        this.strategyLab.loading=true;
        try{
          const res=await this.ccFetchJson('/api/v7/research/committee/review',{tab:'strategy-lab',retries:1,init:{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({subject:this.strategyLab.draft})}});
      ...
      async exportStrategyPine(){
        if(!this.strategyLab.draft) return;
        try{
          const res=await this.ccFetchJson('/api/v7/research/export/pine',{tab:'strategy-lab',retries:1,init:{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({draft:this.strategyLab.draft})}});
      ...
      async fetchShadowShell(){
        ...
        finally{this.shadowAccount.loading=false}
      },

      async analyzeShadowAccount(){
        let trades=[];
        try{trades=JSON.parse(this.shadowAccount.tradesJson||'[]');}catch(e){this.shadowAccount.error='Invalid JSON';return;}
        if(!Array.isArray(trades)||!trades.length){this.shadowAccount.error='trades array required';return;}
        this.shadowAccount.loading=true;this.shadowAccount.error='';
        try{
          const res=await this.ccFetchJson('/api/v7/research/shadow/analyze',{tab:'shadow',retries:1,init:{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({trades,source:'manual_journal'})}});
          if(!res.ok){this.shadowAccount.error=res.error;return;}
          this.shadowAccount.result=res.data?.shadow||null;
          this.shadowAccount.page_capability=res.data?.page_capability||null;
        }catch(e){this.shadowAccount.error=e.message}
        finally{this.shadowAccount.loading=false}
      },

      async fetchReportsLib(){
        this.reportsLib.loading=true;this.reportsLib.error='';
        try{
          const res=await this.ccFetchJson('/api/v7/research/reports?limit=40',{tab:'reports',retries:2});
          if(!res.ok){this.reportsLib.error=res.error;return;}
          this.reportsLib.reports=res.data?.reports||[];
          this.reportsLib.page_capability=res.data?.page_capability||null;
        }catch(e){this.reportsLib.error=e.message}
        finally{this.reportsLib.loading=false}
      },

      async exportReport(reportId,fmt){
        if(!reportId) return;
        try{
          const r=await this.ccFetch('/api/v7/research/reports/'+encodeURIComponent(reportId)+'/export?fmt='+(fmt||'markdown'),{tab:'reports',retries:1});
          if(r&&r.ok) this.reportsLib.exportText=await r.text();
        }catch(e){}
      },

      async fetchOpsConsole(){
        this.opsConsole.loading=true;this.opsConsole.error='';
        try{
          ...
          const fromEvidence={};
          (d.component_evidence||[]).forEach(c=>{if(c&&c.name)fromEvidence[c.name]=!!c.ok;});
          const mergedComponents=Object.keys(fromEvidence).length?fromEvidence:(d.components||{});
          if(d.engine){
            this.ops={...this.ops,...d.engine,components:Object.keys(mergedComponents).length?mergedComponents:(this.ops.components||{})};
          }
          this.opsDetail={
            uptime:d.uptime,
            latency:d.latency,
        ...
        }catch(e){this.opsConsole.error=this.surfaceFetchErrorLine(e.message||'ops console failed','ops')}
        finally{this.opsConsole.loading=false}
      },

      async fetchFunds(){
        this.fundMonitor.loading=true;this.fundMonitor.error='';
        try{
          const res=await this.ccFetchJson('/api/fund-lab/live?benchmark='+(this.fundMonitor.benchmark||'SPY'),{tab:'funds',retries:3,backoff:600});
          if(!res.ok){
          ...
          if(this.fundMonitor.data?.cards){
            ...
            if(this.fundMonitor.data.cards.length){
              ...
              const ctrl=cards.find(c=>c.controls_capital)||cards.find(c=>c.gate_status==='ACTIVE')||cards[0];
              const strip=(c)=>({id:c.id,display_name:c.display_name,gate_status:c.gate_status,stance:c.stance,mode:c.mode,controls_capital:!!c.controls_capital,regime_fit:c.regime_fit,excess_return_pct:c.excess_return_pct,max_drawdown_pct:c.max_drawdown_pct,equity_curve_20:c.equity_curve_20||[],evidence_badge:c.evidence_badge||'model_backtest'});
              this.today7.sleeve_summary={stance:'Active: '+(ctrl?.display_name||'')+' · '+(ctrl?.stance||'NEUTRAL')+' · '+(ctrl?.gate_status||'—'),cards:cards.slice(0,3).map(strip),active_today:ctrl?strip(ctrl):null,fund_manager:ctrl?{active_sleeve_id:ctrl.id,active_sleeve_name:ctrl.display_name,stance:ctrl.stance,mode:ctrl.mode,controls_capital:!!ctrl.controls_capital,regime_fit:ctrl.regime_fit}:null};
            }
          }
        }catch(e){this.fundMonitor.error=this.surfaceFetchErrorLine(e.message||'fund fetch failed','funds');this.fundMonitor.data=null}
        finally{this.fundMonitor.loading=false}
      },

      async fetchRs(){
        this.rsPanel.loading=true;this.rsPanel.error='';
        try{
        ...
        }catch(e){this.rsPanel.error=e.message||'RS fetch failed';this.rsPanel.data=null}
        finally{this.rsPanel.loading=false}
      },
      ...
      rejectionsPageIntro(){
        ...
        const trend=(r.trend||r.trend_regime||'SIDEWAYS').toString().toUpperCase();
        const tb=(r.tradeability||this.today7?.tradeability||'WAIT').toString().toUpperCase();
        return '呢頁只答點解被擋住。當前 '+trend+' · '+tb+' 盤面下，未同時通過 leadership、timing、contradiction 同 board-quality gates 嘅名稱，預設都唔應得 capital。先睇 blocker clusters，再決定要監察咩改善。';
      },
      ...
      rejectionsBlockerBadge(sig){
        const m={laggard:'Laggard',contradiction:'Contradiction',board_quality:'Board / regime',timing:'Timing',context:'Context'};
      ...
      rejectionsFallbackPrimary(sig){
        ...
        if(raw.indexOf('Weak setup — monitor only')>=0){
          if((sig.conflict||'').toLowerCase().indexOf('laggard')>=0) return 'Blocked — not leading enough for this regime (inferred from conflict).';
          if((sig.conflict||'').indexOf('contradict')>=0) return 'Blocked — too many contradictions for deployment (inferred from conflict).';
      ...
      async fetchRejections(){
        const tab='notrade';
        this.rejectionsPanel.loading=true;this.rejectionsPanel.error='';
        this.surfaceFetchHints[tab]={...(this.surfaceFetchHints[tab]||{}),loading:true,error:'',failed_fetch:false};
        try{
          ...
          if(!ntR.ok){
            ...
            this.surfaceFetchHints[tab]={loading:false,error:this.opsDegradedLine(degraded),failed_fetch:true};
            return;
          }
        ...
        finally{this.rejectionsPanel.loading=false}
      },

      async fetchConviction(){
        const tk=(this.dos.ticker||'').trim().toUpperCase();
        if(!tk){this.dos.conviction=null;return;}
        ...
        }catch(e){console.warn('conviction fetch failed',e);this.dos.conviction=null}
        finally{this.dos.convictionLoading=false}
      },
      ...
      async fetchCommandBoard(){
        this.cmd.loading=true;this.cmd.error='';
        try{
        ...
        }catch(e){this.cmd.error=e.message||'board fetch failed'}
        finally{this.cmd.loading=false}
      },

      async fetchDecision(ticker){
        const tk=(ticker||'').trim().toUpperCase();
        if(!tk)return;
        this.cmd.activeTicker=tk;this.cmd.loading=true;this.cmd.error='';this.cmd.agentError='';
        try{
          ...
          this.cmd.agent={regime_gate:card.regime_allows_trading?'PASS':'BLOCK',conviction_tier:card.grade,rr_multiple:card.risk_reward,summary:card.action_reason,council:card.expert_council,journal:{persisted:false},agents:{critic:{notes:[]}}};
          try{
        ...
        }catch(e){this.cmd.error=e.message;this.cmd.decision=null}
        finally{this.cmd.loading=false}
      },
      ...
      async ibkrFetchStatus(opts){
        ...
        try{
          ...
          if(!r.ok){
            let errBody={};
            try{errBody=await r.json();}catch(_x){
              const txt=await r.text().catch(()=>'');
              try{errBody=JSON.parse(txt);}catch(_y){errBody={error:txt};}
            }
            ...
            this.ibkrApplyStatusFetchFailure(
              r.status,
      ...
      async autoScheduleExperiments(){
        ...
        }catch(e){this._handleAutoScheduleError(e);}
      },
    }}
