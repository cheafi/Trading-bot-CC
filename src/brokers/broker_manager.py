...
class BrokerManager:
    ...
    async def initialize(self):
        ...
        if settings.has_ib:
            try:
                ...
                msg = str(e)
                # Common when init runs in a background thread without its own loop
                if "no current event loop" in msg.lower():
                    logger.info(
                        "IB (ib_insync) skipped: no event loop in this thread. "
                        "Use /api/ibkr/* (ibapi) for live broker — this warning is cosmetic."
                    )
                else:
    ...
    async def cancel_order(
        self, order_id: str, broker: Optional[BrokerType] = None
    ) -> bool:
        """Cancel an order."""
        target_broker = self._brokers.get(broker) if broker else self.active_broker
    ...
    async def get_aggregated_portfolio(self) -> Dict:
        ...
        return {
            ...
            "brokers_connected": len(
                [b for b in self._brokers.values() if b.is_connected]
            ),
        }
