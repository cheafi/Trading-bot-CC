...
class FutuBroker(BaseBroker):
            ...
            if ret == 0 and len(data) > 0:
                for _, row in data.iterrows():
                    pos = Position(
                        ...
                        current_price=float(row.get("market_val", 0))
                        / max(int(row.get("qty", 1)), 1),
    ...
    def _place_order_sync(self, order: OrderRequest) -> OrderResult:
        ...
        try:
            ...
            trade_ctx = (
                self._trade_ctx_us if order.market == Market.US else self._trade_ctx_hk
            )
            ...
            # Map trade side
            trade_side = (
                self._futu.TrdSide.BUY
                if order.side == OrderSide.BUY
                else self._futu.TrdSide.SELL
            )
            ...
            if ret == 0 and len(data) > 0:
                ...
                return OrderResult(success=False, message=f"Order failed: {data}")
    ...
    async def get_market_snapshot(
        self, tickers: List[str], market: Market = Market.US
    ) -> List[Quote]:
        ...
        return await loop.run_in_executor(
            None, self._get_market_snapshot_sync, tickers, market
        )

    def _get_market_snapshot_sync(
        self, tickers: List[str], market: Market
    ) -> List[Quote]:
    ...
    async def subscribe_realtime(
        ...
        try:
            ...
            ret, data = self._quote_ctx.subscribe(
                futu_tickers, [self._futu.SubType.QUOTE], subscribe_push=True
            )
