...
class IBBroker(BaseBroker):
            ...
            for order in open_orders:
                ...
                if status is None or order_status == status:
                    orders.append({
                        ...
                        'ticker': order.contract.symbol if hasattr(order, 'contract') else '',
                        'side': 'buy' if order.action == 'BUY' else 'sell',
                        'quantity': int(order.totalQuantity),
                        'filled_qty': int(trade.orderStatus.filled) if trade else 0,
                        'price': float(order.lmtPrice) if hasattr(order, 'lmtPrice') else 0,
    ...
    async def get_contract_details(self, ticker: str, market: Market = Market.US) -> Optional[Dict]:
