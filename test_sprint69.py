...
def test_find_similar_cases_strategy_match():
    ...
    trades = [
        {"ticker": "A", "strategy_id": "breakout",
         "pnl_pct": 5, "r_multiple": 1.0,
         "regime_at_entry": "UPTREND", "setup_grade": "B+",
         "direction": "LONG", "hold_days": 10},
        {"ticker": "B", "strategy_id": "momentum",
         "pnl_pct": -2, "r_multiple": -0.5,
         "regime_at_entry": "UPTREND", "setup_grade": "B",
         "direction": "LONG", "hold_days": 5},
    ]
    cases = find_similar_cases(
        "breakout", trades=trades
    )
    assert len(cases) == 1
...
def test_find_similar_cases_regime_boost():
    ...
    from src.engines.historical_analog import find_similar_cases
    trades = [
        {"ticker": "X", "strategy_id": "momentum",
         "pnl_pct": 8, "r_multiple": 1.5,
         "regime_at_entry": "UPTREND", "setup_grade": "A",
         "direction": "LONG", "hold_days": 15},
        {"ticker": "Y", "strategy_id": "momentum",
         "pnl_pct": 3, "r_multiple": 0.5,
         "regime_at_entry": "SIDEWAYS", "setup_grade": "B",
         "direction": "LONG", "hold_days": 8},
    ]
    cases = find_similar_cases(
        "momentum", regime="UPTREND", trades=trades
    )
...
def test_analog_summary():
    ...
    from src.engines.historical_analog import (
        analog_summary,
    )
    cases = [
        {"pnl_pct": 5, "r_multiple": 1.0},
...
def test_find_from_real_trades():
    ...
    from src.engines.historical_analog import (
        find_similar_cases, load_closed_trades,
    )
    trades = load_closed_trades()
    if trades:
        cases = find_similar_cases(
            "breakout", regime="UPTREND", trades=trades
        )
