...
def _running_in_repo_venv() -> bool:
    """True when this interpreter is the repo .venv/venv (not bare homebrew python3)."""
    ...
    for name in (".venv", "venv"):
        ...
        if candidate.is_dir() and active == candidate:
            return True
    return False


def _maybe_reexec_with_venv() -> None:
    """Re-exec with repo .venv when launched via system python3 (backend needs uvicorn)."""
    if os.environ.get("CC_INSTANT_VENV_REEXEC") == "1" or _running_in_repo_venv():
        return
    for rel in (".venv/bin/python3", ".venv/bin/python", "venv/bin/python3"):
        ...
        if not vpy.is_file():
            continue
        env = os.environ.copy()
        env["CC_INSTANT_VENV_REEXEC"] = "1"
        os.execve(
            str(vpy),
            [str(vpy), "-u", str(Path(__file__).resolve()), *sys.argv[1:]],
            env,
        )
...
def _stale_firm_health_bytes(path_only: str, reason: str) -> bytes | None:
    """Honest research-only stubs for Firm Health panels while backend warms."""
    from datetime import datetime, timezone

    now = datetime.now(timezone.utc).isoformat().replace("+00:00", "Z")
    warming = (
        f"{reason}. Showing empty research-only stub until the full API is ready — "
        "refresh Ops → Firm Health after /health mode=full."
    )
    base = {
        ...
        "degraded_reason": reason,
        "generated_at": now,
        "warming_note": warming,
    }
    if path_only == "/api/v7/belief-review/summary":
        return _encode_degraded(
            {
                **base,
                "status": "warming",
                "beliefs_due": 0,
                "beliefs_reviewed_mtd": 0,
                "forward_outcome_marks": 0,
                "headline": "Belief Review · warming — no items yet",
                "next_review": "Monthly · first business day",
                "items": [],
            },
            reason=reason,
        )
    if path_only == "/api/v7/research-queue":
        return _encode_degraded(
            {
                **base,
                "items": [],
                "category_budgets": {"Research": 60, "Portfolio": 30, "Market": 15},
                "total_budget_minutes": 0,
                "max_items": 12,
                "headline": "Research Queue · warming — queue empty",
            },
            reason=reason,
        )
    if path_only.startswith("/api/v7/attention-budget/summary"):
        return _encode_degraded(
            {
                **base,
                "status": "ok",
                "categories": [
                    {"id": "research", "label": "Research", "budget_minutes": 60},
                    {"id": "portfolio", "label": "Portfolio", "budget_minutes": 30},
                    {"id": "market", "label": "Market", "budget_minutes": 15},
                ],
                "categories_live": [
                    {
                        "id": "research",
                ...
                "total_budget_minutes": 105,
                "total_used_minutes": 0,
                "total_remaining_minutes": 105,
                "any_exceeded": False,
                "ciio_message": "",
            },
            reason=reason,
        )
    if path_only == "/api/v7/firm-cadence/summary":
        return _encode_degraded(
            {
                **base,
                "status": "warming",
                "headline": "Firm cadence · warming — rituals pending full API",
                "next_ritual": {"id": "daily_ic", "label": "Daily IC", "due": "—"},
                "rituals": [],
                "forward_outcome_marks": 0,
            },
            reason=reason,
        )
    if path_only.startswith("/api/v7/decision-journal/recent"):
        return _encode_degraded(
            {
                **base,
                "status": "empty",
                "headline": "Decision Journal · warming — no entries yet",
                "total_loaded": 0,
                "recent_count": 0,
                "stub_count_recent": 0,
                "entries": [],
    ...
    if path_only == "/api/v7/override-journal/summary":
        return _encode_degraded(
            {
                **base,
                "total": 0,
                "by_advice_class": {},
                "recent": [],
                "cooldown": {"in_cooldown": False, "cooldown_hours": 24},
                "as_of": now,
            },
            reason=reason,
        )
    if path_only == "/api/v7/calibration/report":
        return _encode_degraded(
            {
                **base,
                "headline": "Calibration · warming — no forward marks yet",
                "recommendation": "Accumulate journal + forward outcomes before quarterly review.",
                "sample": {"forward_marks": 0, "win_rate": None},
            },
            reason=reason,
        )
    if path_only == "/api/v7/weekly-ic/digest":
        return _encode_degraded(
            {
                **base,
                "cadence": "weekly",
                ...
                "sections": [],
                "daily_ic": None,
                "headline": "Weekly IC · warming — digest pending full API",
            },
            reason=reason,
        )
    if path_only == "/api/v7/usage-log/summary":
        return _encode_degraded(
            {
                **base,
                "total_events": 0,
                "by_surface": {},
                "deletion_candidates": [],
                "headline": "Usage log · warming — no events yet",
            },
            reason=reason,
        )
    return None
...
def _degraded_response(path_only: str, reason: str, full_path: str = "") -> bytes | None:
    ...
    firm_health = _stale_firm_health_bytes(path_only, reason)
    if firm_health is not None:
        return firm_health
    if path_only in (
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def _handle(self):
        ...
        if self.path in ("/health", "/api/health"):
            ...
            if not listening:
                ...
                from src.services.operator_state_contract import build_warmup_module_checklist
                ...
                health_payload = _stamp_instant_degraded(
                    {
                        **health_payload,
                        "degraded": True,
                        "display_mode": "LOADING",
                        "warmup_module_checklist": build_warmup_module_checklist(
                            shell_ok=True,
                            cached_board_ok=fallback_n > 0,
                            market_data_ok=False,
                            dossier_core_ok=False,
                            enrichments_ok=False,
                            broker_ok=False,
                        ),
                        **extra,
                    },
    ...
    def _proxy(self) -> bool:
        ...
        try:
            ...
            path_only = self.path.split("?", 1)[0]
            if e.code == 404:
                stub = _stale_firm_health_bytes(
                    path_only,
                    "backend route not loaded — restart API container after deploy",
                )
                if stub is not None:
                    self._send_json(stub, status=200)
                    return True
            if e.code >= 500 and not path_only.startswith("/api/ibkr/"):
                return False
            body = b""
            try:
                body = e.read()
...
def _acquire_single_instance() -> bool:
    ...
    from src.core.state_paths import runtime_state_dir

    lock_path = runtime_state_dir() / "cc_instant.lock"
